...
| `condition`                 | Dialogue condition evaluated with the attacking monster as `u` and its target as `npc`. The attack is skipped when it is false. `u_has_flag` sees both the monster type's JSON flags and generic flags granted by effects. Legacy effect requirements remain supported and are also enforced.
...
| `condition`             | Dialogue condition evaluated with the casting monster as `u` and its target as `npc` (unless `allow_no_target` is true).
| `forbidden_effects_any` | Array of effect IDs, if the monster has any one the attack can't trigger.
| `forbidden_effects_all` | Array of effect IDs, if the monster has every effect the attack can't trigger.
| `required_effects_any`  | Array of effect IDs, the monster needs any one for the attack to trigger.
| `required_effects_all`  | Array of effect IDs, the monster needs every effect for the attack to trigger.
...
| `condition`          | Dialogue condition evaluated with the leaping monster as `u`.
