...
Each sound effect is identified by an id, a variant or an array of variants, and a season. If a sound effect is played with a variant that does not exist in the json files, but a variant "default" exists, then the "default" variant is played instead. Some sound effects may not use the "default" variant. The file name of the sound effect is relative to the soundpack directory, so if the file name is set to "sfx.wav" and your soundpack is in `data/sound/mypack`, the file must be placed at `data/sound/mypack/sfx.wav`.
...
Melee sound effects depend on which attack is used. For instance, attacking with a knife will make a stabbing sound, while punching or kicking with unarmed combat or martial arts will use an unarmed or default melee sound.
