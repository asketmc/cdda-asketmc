...
## Export vehicle prototypes using the debug menu

The debug menu can generate a prototype from a vehicle in the game.  Stand on a vehicle tile and
choose **Vehicle -> Export vehicle as prototype**.  The generated JSON is written under the user
directory's `export_dir` folder and should be run through the JSON formatter before being added to
the game data.

This exports installed parts and their variants, turret ammunition, tank fuel, vehicle-bound loot
zones, the vehicle blueprint, and simple top-level cargo items.  Nested item state, comestibles,
and containers cannot be represented faithfully by vehicle prototype item spawns and are reduced
to their top-level item IDs.  Review generated files before using them as game data, and replace
the placeholder `id` and `name` fields.
