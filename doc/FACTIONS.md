...
`"food_supply"` | integer, the number of kilocalories available to the faction.
`"consumes_food"` | boolean, optional and false by default. If true, camp meals reduce this faction's food supply; otherwise the stored amount is an inexhaustible abstraction.
...
`"share my stuff"` | Members of the acting faction will not object if members of the object faction take items owned by the acting faction. It also grants access to the acting faction's camp food and water.
`"share public goods"` | Grants the object faction access to public water at the acting faction's camps, but not its food.
