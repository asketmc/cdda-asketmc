...
"item": "wheel",              // The item used to install this part, and the item obtained when
                              // removing this part.
"location": "under",          // (Optional) One of the checks used when determining if a part
                              // can be installed on a given tile. A part cannot be installed
                              // if an existing part occupies the same location.
...
```jsonc
{
  ...
  "z_order": 5,              // Highest Z on a tile is drawn; -1 is never drawn; default 0
...
(Optional) Mass of the furniture while it is tied down on a vehicle. Defaults to 1000 kilograms.
