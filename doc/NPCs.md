...
#### Variable Object
`variable_object`: This is either an object, an `arithmetic` [expression](#compare-integers-and-arithmetics) or array describing a variable name. It can either describe an int, a time duration or a string. If it is an array it must have 2 values the first of which will be a minimum and the second will be a maximum, the value will be randomly between the two. If it is an int `default` is an int which will be the value returned if the variable is not defined. If is it a duration then `default` can be either an int or a string describing a time span. `u_val`, `npc_val`, `global_val`, `context_val`, or `var_val` can be used for the variable name element. `context_val` describes a value scoped to the current dialogue or EOC chain. `var_val` resolves a variable name stored in a context variable; prefixes `u_`, `npc_`, `global_`/`g_`, `context_`/`_`, and `v_` select its scope. Child EOCs inherit a copy of the context, but their changes do not flow back to the parent. If this is a duration `infinite` will be accepted to be a virtually infinite value(it is actually more than a year, if longer is needed a code change to make this a flag or something will be needed).
...
## Dialogue State
Dialogue context variables exist only for the current dialogue or EOC activation. They are useful for passing event fields and temporary values through nested EOCs without writing permanent character or global state.
...
`u_add_trait, npc_add_trait: `string or [variable object](#variable-object), (*optional* `variant: `string or [variable object](#variable-object)) | Your character or the NPC will gain the trait. If `variant` is supplied, that cosmetic mutation variant is selected; omitting it preserves normal variant selection. An unknown explicit variant fails without adding a random variant.
`u_activate_trait, npc_activate_trait: `string or [variable object](#variable-object) | Activate an active mutation already possessed by your character or the NPC.
...
`u_deactivate_trait, npc_deactivate_trait: `string or [variable object](#variable-object) | Deactivate an active mutation already possessed by your character or the NPC.
...
`set_condition`: string or [variable object](#variable-object), `condition`: a dialogue condition object | Store a named dialogue condition. It is available to nested EOCs in the same way as a context variable.
...
`u_prevent_death`, `npc_prevent_death` | Restores any dead vital body part to 1 HP. An NPC death is canceled when this runs from its `NPC_DEATH` EOC or `character_dies` event before cleanup.
...
`if: `dialogue condition, `then: `effect, (*optional* `else: `effect`) | Runs `then` when the dialogue condition is true, otherwise runs `else` when provided. Both branches may contain one effect or an array of effects, including nested `if` effects.
...
`u_run_inv_eocs or npc_run_inv_eocs : ` string or [variable object](#variable-object), (*optional* `true_eocs: effect_on_condition_array`), (*optional* `false_eocs: effect_on_condition_array`), (*optional* `search_data: search_data_array`), (*optional* `title:` string or [variable object](#variable-object)) | Runs EOCs on matching items carried by u or npc, with the item as the beta talker and the character as alpha. `"all"` runs `true_eocs` for every match; `"random"` runs them for one random match; `"manual"` and `"manual_mult"` let the player choose one or several matches from an inventory menu whose heading is set by `title`. If no item matches, a manual selector is canceled, or a variable resolves to an invalid mode, `false_eocs` runs once with the original dialogue; invalid modes also emit a debug message. Each `search_data` object may restrict `id`, `category`, `material`, `flags`, `worn_only`, and `wielded_only`; multiple objects are alternatives.
`u_map_run_item_eocs or npc_map_run_item_eocs : ` string or [variable object](#variable-object), (*optional* `loc:` [variable object](#variable-object)), (*optional* `min_radius:` int or [variable object](#variable-object)), (*optional* `max_radius:` int or [variable object](#variable-object)), (*optional* `true_eocs: effect_on_condition_array`), (*optional* `false_eocs: effect_on_condition_array`), (*optional* `search_data: search_data_array`), (*optional* `title:` string or [variable object](#variable-object)) | Like the inventory form, but searches map items in the selected radius around `loc` (or alpha by default). The selected character is alpha and the item is beta. No match or cancel runs `false_eocs` once. `worn_only` and `wielded_only` are rejected because map items cannot be worn or wielded.
`map_spawn_item: ` string or [variable object](#variable-object), (*optional* `loc:` [variable object](#variable-object)), (*optional* `count:` int or [variable object](#variable-object)), (*optional* `container:` string or [variable object](#variable-object)), (*optional* `use_item_group:` bool) | Places items on the map at `loc`, or at alpha's position when omitted. `container` places the result inside that item; when not everything fits, the empty or partially filled container is still placed. `use_item_group` interprets the main value as an item group and ignores outer `count` and `container`, preserving the group's own collection results.
...
`"map_terrain_with_flag"`, `loc` | string or [variable object](#variable-object) | `true` if terrain at the mandatory global map-square `loc` has the named flag. Off-bubble checks read existing saved submaps and return `false` for ungenerated terrain; evaluating the condition never generates map content.
`"map_furniture_with_flag"`, `loc` | string or [variable object](#variable-object) | `true` if furniture at the mandatory global map-square `loc` has the named flag. Off-bubble checks read existing saved submaps and return `false` for ungenerated terrain; evaluating the condition never generates map content.
...
`"get_condition"` | string or [variable object](#variable-object) | Evaluate the named condition stored in the current dialogue context. Missing names evaluate to false.
...
`"u_val": "var"` | Custom variable. `"var_name"`, `"type"`, and `"context"` must also be specified. `npc_val`, `global_val`, `context_val`, and `var_val` support the same `"var"` form. If `default` is given as either an int or a variable_object then that value will be used if the variable is empty. If `default_time` is used the default is parsed as a time string such as "10 hours". Otherwise 0 is used.
