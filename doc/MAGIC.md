...
| `REGEN_STAMINA` | 
| `THIRST` | 
| `TOTAL_WEIGHT` | Affects total character weight, including worn items and bionics.
| `WEAPON_DISPERSION` | 
...
Vehicle parts accept an `enchantments` array using the same inline or referenced enchantment
syntax as other enchantment containers.  Effects applied to a vehicle also contribute their
enchantments.  Vehicle enchantments support `ALWAYS`, `ACTIVE`, and `INACTIVE` conditions;
0.G does not have a vehicle dialogue talker, so dialogue conditions are not evaluated.  Numeric
values may be constants or global variables; values that require a character dialogue actor are
ignored on vehicles.

| Vehicle status value | Description
|---                    |---
| `TOTAL_WEIGHT` | Affects total vehicle weight.
| `FUEL_USAGE` | Affects engine fuel use.
| `TURNING_DIFFICULTY` | Affects vehicle handling difficulty.
