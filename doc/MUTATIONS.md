...
```json
{
  ...
  "personality_score": {                               // Optional. Defines cosmetic flavor traits for NPCs which can be viewed during dialogue with the NPC.
    "min_aggression": -10,                             // Traits with a defined personality_score are automatically applied to NPCs with qualifying personalities.
    "max_aggression": 10,                              // NPC personality values are integers in the range [-10, 10].
    "min_bravery": -10,                                // All minimum and maximum values are optional; the values shown here are the defaults.
    "max_bravery": 10,
    "min_collector": -10,                              // For example, min_bravery -5 matches bravery -5 through 10, but not -6.
    "max_collector": 10,
    "min_altruism": -10,
    "max_altruism": 10
  },
  ...
  "transform": { "target": "BIOLUM1",         // Trait_id of the mutation this one will transform into.
                 ...
                 "safe": false                // If true, use normal mutation prerequisites and conflict handling without consuming vitamins.  A failed transform restores the source mutation and charges no moves.
  },
  ...
  "processed_eocs": [ "eoc_id_1" ],           // List of effect_on_conditions that run whenever an active mutation pays its periodic cost.
  ...
  "activated_is_setup": false,                 // Keep an activated-EOC mutation powered so processed_eocs can run; otherwise activation EOCs are one-shot.
...
Activation, processing, and deactivation EOCs receive the mutation ID in the
dialogue context variable `this`.  The `u_activate_trait`/`npc_activate_trait`
and `u_deactivate_trait`/`npc_deactivate_trait` dialogue effects can control an
existing active mutation without adding or removing the trait itself.
