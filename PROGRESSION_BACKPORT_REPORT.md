...
Each ordinary base-game LMOE parent guarantees one hidden `lmoe_guns` cache through a palette marker that No Hope deliberately suppresses. Shared storage stays unchanged, excluding occupied and Whately LMOEs. The autodoc finale guarantees `bionics_common`; other finales retain guaranteed rewards.
