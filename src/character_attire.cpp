...
Character::wear( item_location item_wear, bool interactive )
{
    ...
    bool removed_by_event = false;
    auto result = wear_item( to_wear_copy, interactive, true, &removed_by_event );
    if( !result ) {
        if( removed_by_event ) {
            if( was_weapon ) {
    ...
    // Preserve the original empty-wield-state event when a wielded item is worn.
    // The wear event itself already carries the newly worn item as its beta talker.
    if( was_weapon ) {
        get_event_bus().send<event_type::character_wields_item>( getID(), item().typeId() );
    }
        ...
        bool *removed_by_event )
{
    ...
    item &new_item = new_item_it != worn.end() ? *new_item_it : worn.back();
    safe_reference<item> worn_item_reference = new_item.get_safe_reference();
    item_location worn_item( guy, &new_item );
    cata::event event = cata::event::make<event_type::character_wears_item>( guy.getID(), guy.last_item );
    get_event_bus().send_with_talker( &guy, &worn_item, event );
    if( !worn_item_reference ) {
        if( removed_by_event != nullptr ) {
            *removed_by_event = true;
        }
        return cata::nullopt;
    }
...
cata::optional<std::list<item>::iterator> Character::wear_item( const item &to_wear,
        bool interactive, bool do_calc_encumbrance, bool *removed_by_event )
{
    ...
    return worn.wear_item( *this, to_wear, interactive, do_calc_encumbrance, true, false,
                           removed_by_event );
}
