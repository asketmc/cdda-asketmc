...
struct item_search_data {
    ...
    explicit item_search_data( const JsonObject &jo )
        : id( jo.get_string( "id", "" ) ),
          category( jo.get_string( "category", "" ) ),
          material( jo.get_string( "material", "" ) ),
          worn_only( jo.get_bool( "worn_only", false ) ),
          wielded_only( jo.get_bool( "wielded_only", false ) ) {
        for( const std::string &flag : jo.get_string_array( "flags" ) ) {
            flags.emplace_back( flag );
        }
    }

    bool matches( const Character &guy, const item_location &loc ) const {
        if( loc.get_item() == nullptr ) {
            return false;
        }
...
struct eoc_entry {
    ...
    cata::optional<str_or_var<T>> var;
};

template<class T>
static std::vector<eoc_entry<T>> load_eoc_vector_id_and_var( const JsonObject &jo,
        const std::string &member )
{
    std::vector<eoc_entry<T>> entries;
    const auto load_entry = [&entries, &member]( const JsonValue & value ) {
        eoc_entry<T> entry;
        if( value.test_object() ) {
            JsonObject object = value.get_object();
            object.allow_omitted_members();
            if( !object.has_member( "id" ) ) {
                entry.var = get_str_or_var<T>( value, member, false );
            }
        }
        if( !entry.var ) {
            entry.id = effect_on_conditions::load_inline_eoc( value, "" );
        }
    ...
    if( jo.has_array( member ) ) {
        ...
        load_entry( jo.get_member( member ) );
    }
    return entries;
}
...
{
    std::unique_ptr<talker> u_talker = get_talker_for( static_cast<const Creature &>( u ) );
    std::unique_ptr<talker> me_talker = get_talker_for( static_cast<const Creature &>( me ) );
    parse_tags( phrase, *u_talker, *me_talker, item_type );
}

void parse_tags( std::string &phrase, const talker &u, const talker &me,
                 const itype_id &item_type )
{
    ...
    do {
        ...
        if( tag == "<yrwp>" ) {
            phrase.replace( fa, l, u_weapon ? remove_color_tags( u_weapon->tname() ) : _( "fists" ) );
        } else if( tag == "<mywp>" ) {
...
void dialogue::set_value( const std::string &key, const std::string &value ) const
{
    context[key] = value;
}

void dialogue::remove_value( const std::string &key ) const
{
    context.erase( key );
}
...
std::string dialogue::get_value( const std::string &key ) const
{
    ...
    return iter == context.end() ? "" : iter->second;
}
...
void dialogue::set_conditional( const std::string &key, const dialogue_conditional &value ) const
{
    conditionals[key] = value;
}

bool dialogue::evaluate_conditional( const std::string &key ) const
{
    const auto iter = conditionals.find( key );
    if( iter == conditionals.end() ) {
        return false;
    }
    if( !evaluating_conditionals.insert( key ).second ) {
        debugmsg( "Recursive stored condition '%s' failed closed.", key );
        return false;
    }
    const on_out_of_scope clear_guard( [this, &key]() {
        evaluating_conditionals.erase( key );
    } );
    return iter->second( *this );
}

const dialogue_conditional_map &dialogue::get_conditionals() const
{
    return conditionals;
}
...
dialogue::dialogue( std::unique_ptr<talker> alpha_in,
                    ...
                    const std::unordered_map<std::string, std::string> &context_in )
    : dialogue( std::move( alpha_in ), std::move( beta_in ), {}, context_in )
{
}

dialogue::dialogue( std::unique_ptr<talker> alpha_in,
                    std::unique_ptr<talker> beta_in,
                    const dialogue_conditional_map &conditionals_in,
                    const std::unordered_map<std::string, std::string> &context_in )
{
        ...
        bool is_npc )
{
    ...
    str_or_var<T> new_variant;
    if( jo.has_member( "variant" ) ) {
        new_variant = get_str_or_var<T>( jo.get_member( "variant" ), "variant", true );
    } else {
        new_variant.str_val = "";
    }
    function = [is_npc, new_trait, new_variant]( const T & d ) {
        const trait_id trait( new_trait.evaluate( d ) );
        const std::string variant_id = new_variant.evaluate( d );
        const mutation_variant *variant = trait->variant( variant_id );
        if( !variant_id.empty() && variant == nullptr ) {
            debugmsg( "Mutation %s has no variant %s", trait.c_str(), variant_id );
            return;
        }
...
template<class T>
void talk_effect_fun_t<T>::set_activate_trait( const JsonObject &jo, const std::string &member,
        bool is_npc )
{
    str_or_var<T> new_trait = get_str_or_var<T>( jo.get_member( member ), member, true );
...
template<class T>
void talk_effect_fun_t<T>::set_deactivate_trait( const JsonObject &jo, const std::string &member,
        bool is_npc )
{
    str_or_var<T> old_trait = get_str_or_var<T>( jo.get_member( member ), member, true );
    function = [is_npc, old_trait]( const T & d ) {
        d.actor( is_npc )->deactivate_mutation( trait_id( old_trait.evaluate( d ) ) );
    };
}
...
static void map_add_item( item &it, const tripoint_abs_ms &target_pos )
{
    map &here = get_map();
    if( here.inbounds( target_pos ) ) {
        here.add_item_or_charges( here.getlocal( target_pos ), it );
    } else {
        tinymap target_bay;
...
template<class T>
void talk_effect_fun_t<T>::set_map_spawn_item( const JsonObject &jo, const std::string &member )
{
    ...
    const bool use_item_group = jo.get_bool( "use_item_group", false );
    int_or_var<T> count = get_int_or_var<T>( jo, "count", false, 1 );
    cata::optional<var_info> loc_var;
    if( jo.has_object( "loc" ) ) {
        loc_var = read_var_info( jo.get_object( "loc" ) );
    }

    function = [item_name, count, container_name, use_item_group, loc_var]( const T & d ) {
        const itype_id iname( item_name.evaluate( d ) );
        const int item_count = count.evaluate( d );
        const cata::optional<tripoint_abs_ms> target_pos =
            get_tripoint_from_var_checked<T>( loc_var, d );
        if( !target_pos ) {
            return;
        }
        const std::string container_id = container_name.evaluate( d );

        if( use_item_group ) {
            for( item &new_item : item_group::items_from( item_group_id( iname.c_str() ),
                    calendar::turn ) ) {
                map_add_item( new_item, *target_pos );
            }
            return;
        }
        ...
        item new_item( iname, calendar::turn );
        if( container_id.empty() ) {
            if( new_item.count_by_charges() ) {
                new_item.charges = item_count;
                map_add_item( new_item, *target_pos );
            } else {
            ...
            if( new_item.count_by_charges() ) {
                new_item.charges = item_count;
                if( !container.put_in( new_item, item_pocket::pocket_type::CONTAINER ).success() ) {
                    debugmsg( "map_spawn_item could not put %s into %s; placing the empty container.",
                              new_item.typeId().c_str(), container.typeId().c_str() );
                }
            } else {
                if( !new_item.ammo_default().is_null() ) {
                    new_item.ammo_set( new_item.ammo_default() );
                ...
                for( int i = 0; i < item_count; ++i ) {
                    if( !container.put_in( new_item,
                                           item_pocket::pocket_type::CONTAINER ).success() ) {
                        debugmsg( "map_spawn_item could put only %d of %d %s into %s; placing the partially filled container.",
                                  i, item_count, new_item.typeId().c_str(), container.typeId().c_str() );
                        break;
                    }
                }
            }
            map_add_item( container, *target_pos );
        }
    };
}
        ...
        bool is_npc )
{
    ...
    function = [iov_min_radius, iov_max_radius, var_name, outdoor_only, target_params,
        ...
        write_var_value( type, var_name, d.actor( type == var_type::npc ), get_dialogue_context( d ),
                                        ...
                                        bool is_npc )
{
    ...
    function = [message, outdoor_only, sound, snippet, same_snippet, type_string, popup_msg,
        ...
        const talker &alpha = d.has_alpha ? *d.actor( false ) : *default_talker;
        const talker &beta = d.has_beta ? *d.actor( true ) : *default_talker;
        ...
        bool is_npc )
{
    ...
    function = [is_npc, dov, act]( const T & d ) {
        Character *target = d.actor( is_npc )->get_character();
        if( target ) {
            // An EOC assignment replaces the activity that invoked it.  The completed or
            // interrupted activity must not be suspended into the normal resume backlog.
            target->assign_activity( player_activity( activity_id( act.evaluate( d ) ),
                                     to_moves<int>( dov.evaluate( d ) ) ), false, false );
        }
    };
}
...
void talk_effect_fun_t<T>::set_set_string_var( const JsonObject &jo, const std::string &member )
{
    ...
    function = [values, var]( const T & d ) {
        ...
        write_var_value( var.type, var.name, d.actor( var.type == var_type::npc ),
                         get_dialogue_context( d ), values[index].evaluate( d ) );
    };
}

template<class T>
void talk_effect_fun_t<T>::set_set_condition( const JsonObject &jo, const std::string &member )
{
    str_or_var<T> name = get_str_or_var<T>( jo.get_member( member ), member, true );
    dialogue_conditional stored_condition;
    read_condition<dialogue>( jo, "condition", stored_condition, false );
    function = [name, stored_condition]( const T & d ) {
        d.set_conditional( name.evaluate( d ), stored_condition );
    };
}
...
void talk_effect_fun_t<T>::set_run_eocs( const JsonObject &jo,
        const std::string &member )
{
    std::vector<eoc_entry<T>> entries = load_eoc_vector_id_and_var<T>( jo, member );
    if( entries.empty() ) {
        jo.throw_error( "Invalid input for run_eocs" );
    }
    function = [entries]( const T & d ) {
        dialogue newDialog = copy_dialogue( d );
        for( const eoc_entry<T> &entry : entries ) {
            const effect_on_condition_id eoc =
                entry.var ? effect_on_condition_id( entry.var->evaluate( newDialog ) ) : entry.id;
            if( !eoc.is_valid() ) {
                debugmsg( "run_eocs resolved invalid effect_on_condition id %s", eoc.c_str() );
                continue;
            }
            eoc->activate( newDialog );
        }
    ...
    if( local ) {
        function = [eocs, unique_ids, npc_must_see, npc_range, is_npc]( const T & d ) {
            ...
            for( npc *target : available ) {
                for( const effect_on_condition_id &eoc : eocs ) {
                    dialogue newDialog( get_talker_for( target ), nullptr, d.get_conditionals(),
                                        d.get_context() );
                    eoc->activate( newDialog );
                }
            }
            ...
            for( const str_or_var<T> &target : unique_ids ) {
                if( g->unique_npc_exists( target.evaluate( d ) ) ) {
                    for( const effect_on_condition_id &eoc : eocs ) {
                        ...
                        if( npc ) {
                            dialogue newDialog( get_talker_for( npc ), nullptr, d.get_conditionals(),
                                                d.get_context() );
                            eoc->activate( newDialog );
                        } else {
...
template<class T>
void talk_effect_fun_t<T>::set_run_inv_eocs( const JsonObject &jo,
        const std::string &member, bool is_npc )
{
    str_or_var<T> option = get_str_or_var<T>( jo.get_member( member ), member );
    if( jo.has_string( member ) ) {
        const std::string value = jo.get_string( member );
        if( value != "all" && value != "random" && value != "manual" &&
            value != "manual_mult" ) {
            jo.throw_error( member +
                            " must be \"all\", \"random\", \"manual\", or \"manual_mult\"" );
        }
    }

    std::vector<effect_on_condition_id> true_eocs = load_eoc_vector( jo, "true_eocs" );
    std::vector<effect_on_condition_id> false_eocs = load_eoc_vector( jo, "false_eocs" );
    std::vector<item_search_data> search;
    for( const JsonValue &entry : jo.get_array( "search_data" ) ) {
        search.emplace_back( entry.get_object() );
    }
    str_or_var<T> title;
    if( jo.has_member( "title" ) ) {
        title = get_str_or_var<T>( jo.get_member( "title" ), "title", true );
    } else {
        title.str_val = "";
    }

    function = [option, true_eocs, false_eocs, search, is_npc, title]( const T & d ) {
        ...
        std::vector<item_location> matching_items;
        for( const item_location &loc : guy->all_items_loc() ) {
            bool matches = search.empty();
            for( const item_search_data &datum : search ) {
                if( datum.matches( *guy, loc ) ) {
                    matches = true;
                    break;
                }
            }
            if( matches ) {
                matching_items.push_back( loc );
            }
        }

        const auto run_item_eocs = [&d, guy]( item_location &loc,
        const std::vector<effect_on_condition_id> &eocs ) {
            for( const effect_on_condition_id &eoc : eocs ) {
                if( loc.get_item() == nullptr ) {
                    break;
                }
                dialogue item_dialogue( get_talker_for( static_cast<Creature *>( guy ) ),
                                        get_talker_for( loc ), d.get_conditionals(), d.get_context() );
                eoc->activate( item_dialogue );
            }
        };

        const std::string selection = option.evaluate( d );
        if( selection != "all" && selection != "random" && selection != "manual" &&
            selection != "manual_mult" ) {
            debugmsg( "Invalid inventory item EOC selection mode: %s", selection );
            run_eoc_vector( false_eocs, d );
            return;
        }
        if( selection == "all" ) {
            ...
            for( item_location &loc : matching_items ) {
                run_item_eocs( loc, true_eocs );
            }
            ...
            std::shuffle( matching_items.begin(), matching_items.end(), rng_get_engine() );
            run_item_eocs( matching_items.back(), true_eocs );
        } else if( selection == "manual" || selection == "manual_mult" ) {
            if( matching_items.empty() ) {
                run_eoc_vector( false_eocs, d );
                return;
            }

            const auto filter = [&matching_items]( const item_location & candidate ) {
                return std::find( matching_items.begin(), matching_items.end(), candidate ) !=
                       matching_items.end();
            };

            if( selection == "manual" ) {
                item_location selected = game_menus::inv::titled_filter_menu(
                                             filter, *guy, title.evaluate( d ) );
                if( selected ) {
                    run_item_eocs( selected, true_eocs );
                } else {
                    run_eoc_vector( false_eocs, d );
                }
            } else {
                ...
                for( drop_location selected_item : selected ) {
                    run_item_eocs( selected_item.first, true_eocs );
                }
            }
        } else {
            debugmsg( "Invalid inventory EOC selection mode: %s", selection );
            run_eoc_vector( false_eocs, d );
        }
    };
}
...
template<class T>
void talk_effect_fun_t<T>::set_map_run_item_eocs( const JsonObject &jo,
        const std::string &member, bool is_npc )
{
    str_or_var<T> option = get_str_or_var<T>( jo.get_member( member ), member );
    if( jo.has_string( member ) ) {
    ...
    std::vector<item_search_data> search;
    for( const JsonValue &entry : jo.get_array( "search_data" ) ) {
        JsonObject search_entry = entry.get_object();
        if( search_entry.get_bool( "worn_only", false ) ||
            search_entry.get_bool( "wielded_only", false ) ) {
            search_entry.throw_error(
                "worn_only and wielded_only are only valid for inventory item EOC selectors" );
        }
        search.emplace_back( search_entry );
    }
    str_or_var<T> title;
    if( jo.has_member( "title" ) ) {
        title = get_str_or_var<T>( jo.get_member( "title" ), "title", true );
    ...
    int_or_var<T> min_radius = get_int_or_var<T>( jo, "min_radius", false, 0 );
    int_or_var<T> max_radius = get_int_or_var<T>( jo, "max_radius", false, 0 );

    function = [option, true_eocs, false_eocs, search, is_npc, title, loc_var,
                min_radius, max_radius]( const T & d ) {
        Character *guy = d.actor( is_npc )->get_character();
        if( guy == nullptr ) {
            run_eoc_vector( false_eocs, d );
            return;
        ...
        map &here = get_map();
        const tripoint center = here.getlocal( get_tripoint_from_var<T>( loc_var, d ) );
        const int minimum = std::max( 0, min_radius.evaluate( d ) );
        const int maximum = std::max( 0, max_radius.evaluate( d ) );
        std::vector<item_location> matching_items;
        if( minimum <= maximum ) {
            for( const tripoint &pos : here.points_in_radius( center, maximum ) ) {
                if( !here.inbounds( pos ) || rl_dist( center, pos ) < minimum ) {
                    continue;
                }
        ...
        if( selection != "all" && selection != "random" && selection != "manual" &&
            selection != "manual_mult" ) {
            debugmsg( "Invalid map item EOC selection mode: %s", selection );
            run_eoc_vector( false_eocs, d );
            return;
        }
        if( selection == "all" ) {
            ...
            if( selection == "manual" ) {
                inventory_pick_selector selector( *guy, preset );
                selector.set_title( title.evaluate( d ) );
                ...
                item_location selected = selector.execute();
                if( selected ) {
                    run_item_eocs( selected, true_eocs );
                } else {
                    run_eoc_vector( false_eocs, d );
                ...
                inventory_multiselector selector( *guy, preset );
                selector.set_title( title.evaluate( d ) );
                selector.set_display_stats( false );
                selector.clear_items();
                for( const tripoint &pos : here.points_in_radius( center, maximum ) ) {
                ...
                const drop_locations selected = selector.execute();
                if( selected.empty() ) {
                    run_eoc_vector( false_eocs, d );
                    return;
                }
...
template<class T>
void talk_effect_fun_t<T>::set_if( const JsonObject &jo,
                                   const std::string &member )
{
    std::function<bool( const T & )> condition;
    read_condition<T>( jo, member, condition, false );

    talk_effect_t<T> then_effect;
    then_effect.load_effect( jo, "then" );

    talk_effect_t<T> else_effect;
    if( jo.has_member( "else" ) ) {
        else_effect.load_effect( jo, "else" );
    }

    function = [condition, then_effect, else_effect]( const T & d ) {
        if( condition( d ) ) {
            then_effect.apply( d );
        } else {
            else_effect.apply( d );
...
template<class T>
void talk_effect_fun_t<T>::set_foreach( const JsonObject &jo, const std::string &member )
{
    const std::string foreach_type = jo.get_string( member );
    if( foreach_type != "ids" && foreach_type != "item_group" &&
        foreach_type != "monstergroup" && foreach_type != "array" ) {
        jo.throw_error( "foreach must be \"ids\", \"item_group\", \"monstergroup\", or \"array\"" );
    }

    const var_info iterator = read_var_info( jo.get_object( "var" ) );
    talk_effect_t<T> effect;
    effect.load_effect( jo, "effect" );
    std::string target;
    std::vector<str_or_var<T>> array;
    if( foreach_type == "array" ) {
        if( !jo.has_array( "target" ) ) {
            jo.throw_error( "array foreach requires an array target" );
        }
        for( const JsonValue &entry : jo.get_array( "target" ) ) {
            array.emplace_back( get_str_or_var<T>( entry, "target" ) );
        }
    } else {
        target = jo.get_string( "target" );
        if( foreach_type == "ids" && target != "flag" && target != "trait" &&
            target != "vitamin" ) {
            jo.throw_error( "ids foreach target must be \"flag\", \"trait\", or \"vitamin\"" );
        }
    }

    function = [foreach_type, iterator, effect, target, array]( const T & d ) {
        std::vector<std::string> values;
        if( foreach_type == "ids" ) {
            if( target == "flag" ) {
                for( const json_flag &flag : json_flag::get_all() ) {
                    values.push_back( flag.id.str() );
                }
            } else if( target == "trait" ) {
                for( const mutation_branch &mutation : mutation_branch::get_all() ) {
                    values.push_back( mutation.id.str() );
                }
            } else {
                for( const std::pair<const vitamin_id, vitamin> &vitamin_entry : vitamin::all() ) {
                    values.push_back( vitamin_entry.first.str() );
                }
            }
        } else if( foreach_type == "item_group" ) {
            for( const itype *item_type : item_group::every_possible_item_from(
                     item_group_id( target ) ) ) {
                values.push_back( item_type->get_id().str() );
            }
        } else if( foreach_type == "monstergroup" ) {
            for( const mtype_id &monster : MonsterGroupManager::GetMonstersFromGroup(
                     mongroup_id( target ), true ) ) {
                values.push_back( monster.str() );
            }
        } else {
            for( const str_or_var<T> &entry : array ) {
                values.push_back( entry.evaluate( d ) );
            }
        }

        talker *iterator_actor = d.actor( iterator.type == var_type::npc );
        if( ( iterator.type == var_type::u || iterator.type == var_type::npc ) &&
            iterator_actor == nullptr ) {
            debugmsg( "foreach iterator requires a missing dialogue actor" );
            return;
        }
        for( const std::string &value : values ) {
            write_var_value( iterator.type, iterator.name, iterator_actor,
                             get_dialogue_context( d ), value );
            effect.apply( d );
        }
    };
}
...
void talk_effect_t<T>::parse_sub_effect( const JsonObject &jo )
{
    ...
    } else if( jo.has_member( "map_spawn_item" ) ) {
        subeffect_fun.set_map_spawn_item( jo, "map_spawn_item" );
    ...
    } else if( jo.has_member( "u_map_run_item_eocs" ) ) {
        subeffect_fun.set_map_run_item_eocs( jo, "u_map_run_item_eocs", false );
    } else if( jo.has_member( "npc_map_run_item_eocs" ) ) {
        subeffect_fun.set_map_run_item_eocs( jo, "npc_map_run_item_eocs", true );
    } else if( jo.has_array( "weighted_list_eocs" ) ) {
        subeffect_fun.set_weighted_list_eocs( jo, "weighted_list_eocs" );
    } else if( jo.has_member( "if" ) ) {
        subeffect_fun.set_if( jo, "if" );
    } else if( jo.has_member( "switch" ) ) {
        subeffect_fun.set_switch( jo, "switch" );
    } else if( jo.has_string( "foreach" ) ) {
        subeffect_fun.set_foreach( jo, "foreach" );
...
#undef WRAP
    ...
    if( effect_id == "u_prevent_death" || effect_id == "npc_prevent_death" ) {
        const bool is_npc = effect_id == "npc_prevent_death";
        talk_effect_fun_t<T> prevent_death_effect( [is_npc]( const T & d ) {
            Character *ch = d.actor( is_npc )->get_character();
            if( ch != nullptr ) {
                ch->prevent_death();
            }
        } );
        set_effect( prevent_death_effect );
        return;
    }
