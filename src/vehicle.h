...
struct vehicle_part {
        ...
        void load_furniture( map &here, const tripoint &from );
        void unload_furniture( map &here, const tripoint &to );
    ...
    private:
        ...
        item base;
        std::vector<item> tools; // real tools attached to a VEH_TOOLS station
...
// Exact vehicle storage bound to a temporary multi-pocket item pocket.
...
class vehicle
{
    ...
    public:
        ...
        void add_effect( const effect_source &source, const efftype_id &eff_id,
                         const time_duration &duration, bool permanent = false, int intensity = 0 );
        void remove_effect( const efftype_id &eff_id );
        bool has_effect( const efftype_id &eff_id ) const;
        bool has_visible_effect() const;
        ...
        ret_val<void> can_mount( const point &dp, const vpart_id &id,
                                 bool allow_noinstall = false ) const;
        ...
        */
        ...
        // Total power draw from battery chargers during the current turn.
        // Cleared every turn during idle().
        int recharge_epower_this_turn = 0; // NOLINT(cata-serialize)
        ...
        /** @return true if this vehicle or a connected vehicle has stored battery power. */
        ...
        */
        ...
        bool has_available_electric_engine() const;
        ...
        std::vector<item> &get_tools( vehicle_part &part );
        const std::vector<item> &get_tools( const vehicle_part &part ) const;
        /** Build powered pseudo copies of static and attached tools for crafting and interaction. */
        std::map<item, input_event> prepare_tools( const vehicle_part &part ) const;
        /** Fill a pseudo tool from connected batteries or a matching vehicle tank. */
        int prepare_tool( item &tool ) const;
        /** Invoke or advance a temporary pseudo tool backed by a vehicle. */
        static bool use_vehicle_tool( vehicle &veh, const tripoint &vp_pos,
                                      const itype_id &tool_type, bool no_invoke = false );
        /** Concrete ammo item and available amount for a tool. */
        std::pair<itype_id, int> tool_ammo_available( const itype_id &tool_type ) const;
    ...
    public:
        ...
        */
        ...
        void use_tiedown_furniture( int part, const tripoint &pos );
        ...
        static std::map<std::string, multimag_pocket_state> prepare_multimag_pockets(
            vehicle &veh, item &tool, const gun_mode_id &mode,
            const itype_id &preferred_primary = itype_id::NULL_ID() );
        static std::map<std::string, multimag_pocket_state> prepare_multimag_pockets(
            vehicle &veh, item &tool );
        static void drain_back_multimag( vehicle &veh, const item &tool,
