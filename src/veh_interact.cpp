...
player_activity veh_interact::serialize_activity()
{
    ...
    if( sel_cmd == 'p' ) {
        if( !parts_here.empty() ) {
            ...
            const int displayed_index = veh->part_displayed_at( part_here.mount() );
            if( displayed_index >= 0 ) {
                return veh_shape( *veh ).start( veh->global_part_pos3( displayed_index ) );
            }
        }
...
void orient_part( vehicle *veh, const vpart_info &vpinfo, int partnum,
                  const cata::optional<point> &part_placement )
{
...
bool veh_interact::update_part_requirements()
{
    ...
    // The cursor can point at an empty tile immediately beside the vehicle.  In that case
    // cpart is -1, but the tile is still a valid place to extend the frame.  Use the cursor's
    // mount coordinate directly instead of indexing a non-existent part.
...
void veh_interact::do_repair()
{
    ...
    while( true ) {
        ...
        if( pt.is_broken() ) {
            ...
            if( pt.has_flag( vehicle_part::carried_flag ) ) {
                nmsg += colorize( _( "\nUnrack this vehicle before replacing the part.\n" ), c_red );
                ok = false;
            }
        ...
        if( vpr.part().is_reactor() || vpr.part().is_turret() ) {
            ...
            if( vpr.part().is_turret() ) {
                if( vpr.part().get_base().uses_firing_requirements() ) {
                    auto multimag_details = [this]( const vehicle_part & pt,
                    const catacurses::window & w, int y ) {
                        const turret_data turret = veh->turret_query( pt );
                        int row = y + 1;
                        for( const multimag_display_pocket &display : turret.multimag_display_state() ) {
                            const itype *ammo_type = display.ammo_itype.is_null() ? nullptr :
                                                     item::find_type( display.ammo_itype );
                            const std::string ammo_name = ammo_type != nullptr ?
                                                          ammo_type->nname( std::max( display.effective_qty, 1 ) ) : "-";
                            const nc_color row_color = display.effective_qty < display.per_use_qty ?
                                                       c_red : c_light_gray;
                            trim_and_print( w, point( 8, row ), getmaxx( w ) - 14, row_color,
                                            "%s", display.pocket_id );
                            right_print( w, row, 1, ammo_type != nullptr ? ammo_type->color : c_light_gray,
                                         string_format( "%s%s   %5i",
                                                        display.vehicle_bound ? "[v] " : "    ",
                                                        ammo_name, display.effective_qty ) );
                            ++row;
                        }
                    };
                    auto multimag_rows = [this]( const vehicle_part & pt ) {
                        const turret_data turret = veh->turret_query( pt );
                        return static_cast<int>( turret.multimag_display_state().size() );
                    };
                    overview_opts.emplace_back( "5_TURRET", &vpr.part(), selectable,
                                                selectable ? next_hotkey( hotkey ) : input_event(),
                                                multimag_details );
                    overview_opts.back().extra_rows = multimag_rows;
                } else {
                    overview_opts.emplace_back( "5_TURRET", &vpr.part(), selectable,
                                                selectable ? next_hotkey( hotkey ) : input_event(),
                                                details_ammo );
...
void veh_interact::display_overview()
{
    ...
    for( int idx = overview_offset; idx != static_cast<int>( overview_opts.size() ); ++idx ) {
        ...
        y += 1 + ( overview_opts[idx].extra_rows ? overview_opts[idx].extra_rows( pt ) : 0 );
...
void veh_interact::display_veh()
{
    ...
    std::vector<int> structural_parts = veh->all_parts_at_location( vpart_location_structure );
