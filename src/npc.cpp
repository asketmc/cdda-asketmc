...
void npc::generate_personality_traits()
{
    ...
    for( const mutation_branch &mdata : mutation_branch::get_all() ) {
        if( mdata.personality_score ) {
            const cata::value_ptr<mut_personality_score> &required = mdata.personality_score;
            // This is a bounds check repeated for every personality score.
            if( required->min_aggression <= ur.aggression &&
                ur.aggression <= required->max_aggression &&
                required->min_bravery <= ur.bravery &&
                ur.bravery <= required->max_bravery &&
                required->min_collector <= ur.collector &&
                ur.collector <= required->max_collector &&
                required->min_altruism <= ur.altruism &&
                ur.altruism <= required->max_altruism ) {
...
void npc::starting_weapon( const npc_class_id &type )
{
    ...
    if( weapon ) {
        ...
        cata::event event = cata::event::make<event_type::character_wields_item>( getID(),
                            weapon->typeId() );
        get_event_bus().send_with_talker( this, &weapon, event );
    }
}
...
bool npc::wield( item &it )
{
    ...
    // EVENT EOCs may remove the newly wielded item.  Dispatch after all local
    // uses of the item_location so such removal cannot invalidate this path.
    cata::event event = cata::event::make<event_type::character_wields_item>( getID(),
                        weapon->typeId() );
    get_event_bus().send_with_talker( this, &weapon, event );
    return true;
...
npc_follower_rules::npc_follower_rules()
{
    ...
    set_flag( ally_rule::heal_others );
