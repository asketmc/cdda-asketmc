...
int Character::fire_gun( const tripoint &target, int shots, item &gun )
{
    ...
    // Explicit multi-pocket items are capped by the least-stocked required
    // pocket.  Legacy guns keep the exact 0.G aggregate-ammo calculation.
    if( gun.uses_firing_requirements() ) {
        shots = gun.feasible_shots( gun.gun_get_mode_id(), shots );
    ...
    while( curshot != shots ) {
        ...
        for( std::pair<Creature *const, std::pair<int, int>> &hit_entry : targets_hit ) {
            if( monster *const m = hit_entry.first->as_monster() ) {
                cata::event event = cata::event::make<event_type::character_ranged_attacks_monster>(
                                        getID(), gun_id, m->type->id );
                get_event_bus().send_with_talker( this, m, event );
            } else if( Character *const c = hit_entry.first->as_character() ) {
                cata::event event = cata::event::make<event_type::character_ranged_attacks_character>(
                                        getID(), gun_id, c->getID(), c->get_name() );
                get_event_bus().send_with_talker( this, c, event );
            }
            if( multishot ) {
        ...
        if( gun.uses_firing_requirements() ) {
            Character *carrier = gun.has_flag( flag_VEHICLE ) ? nullptr : this;
            if( gun.consume_shots( gun.gun_get_mode_id(), 1, pos(), carrier ) != 1 ) {
...
int time_to_attack( const Character &p, const itype &firing )
{
    ...
    return std::max( info.min_time,
                     static_cast<int>( std::round( info.base_time - info.time_reduction_per_level *
                                       p.get_skill_level_with_progress( skill_used ) ) ) );
}
...
std::string target_ui::uitext_fire() const
{
    if( mode == TargetMode::Throw || mode == TargetMode::ThrowBlind ) {
        ...
        return to_translation( "[Hotkey] to select", "to select" ).translated();
    } else {
...
void target_ui::panel_gun_info( int &text_y )
{
    ...
    if( status == Status::OutOfAmmo ) {
    ...
    } else if( mode == TargetMode::TurretManual && turret != nullptr &&
        ...
        for( const multimag_display_pocket &display : turret->multimag_display_state() ) {
            ...
            const std::string ammo_name = ammo_type != nullptr ?
                                          ...
                                          display.pocket_id;
            const nc_color ammo_color = ammo_type != nullptr ? ammo_type->color : c_light_gray;
            const std::string source = display.vehicle_bound ? _( "vehicle" ) : _( "loaded" );
            str = string_format( _( "%s: %s (%s %d, need %d)" ), display.pocket_id,
                                 colorize( ammo_name, ammo_color ), source,
                                 display.effective_qty, display.per_use_qty );
            print_colored_text( w_target, point( 1, text_y++ ), clr, clr, str );
        }
    } else if( ammo ) {
