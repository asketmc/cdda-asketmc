...
class layer_variant
{
    public:
        ...
        point offset;
        int total_weight;
        // Optional suffix used by modern layering.json contexts (for example _hoisted).
        std::string append_suffix;
};

/** Select a deterministic weighted sprite for item/field layering. */
std::string pick_layer_variant_sprite( const layer_variant &variant, unsigned int seed );

/** Build the sprite lookup variant used by a layered item. */
std::string layered_item_variant( const layer_variant &layer, const std::string &item_variant );

/** Build a tile id from either a normal item variant or a literal sprite suffix. */
std::string tile_id_with_variant( const std::string &id, const std::string &variant,
                                  bool allow_literal_suffix );

/** Calculate the amount of tall-sprite retraction for the configured occlusion mode. */
int calculate_occlusion_retract( int mode, float distance, float min_distance,
                                 float max_distance );

/** Convert source-tile vertical height to the current screen-pixel scale. */
int scaled_zlevel_height( int source_height, int rendered_tile_width, int source_tile_width );
/** Apply the shared sprite/fog vertical transform to a screen Y coordinate. */
int zlevel_screen_y( int base_y, int source_height, int rendered_tile_width,
                     int source_tile_width );
/** Resolve fog opacity from the same scaled separation used for sprite drawing. */
int zlevel_fog_alpha( bool isometric, int scaled_height );
/** Resolve lower-Z draw depth; legacy isometric sets without spacing stay single-Z. */
int effective_3d_draw_depth( bool isometric, int requested_depth, int scaled_height );
