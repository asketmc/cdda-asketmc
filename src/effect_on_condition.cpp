...
std::size_t eoc_factory_generation = 0;
} // namespace
...
void effect_on_condition::load( const JsonObject &jo, const std::string & )
{
    ...
    } else if( jo.has_member( "required_event" ) ) {
        jo.throw_error( "required_event is only valid for EVENT effect_on_conditions" );
    }
}
...
{
    // Context flows down into an EOC, but mutations stay local to this activation.
    dialogue d_eoc = copy_dialogue( d );
    bool retval = false;
    ...
    if( global && run_for_npcs ) {
        for( npc &guy : g->all_npcs() ) {
            dialogue d_npc( get_talker_for( guy ), nullptr, d_eoc.get_conditionals(),
                            d_eoc.get_context() );
...
{
    if( !has_cached || cached_generation != eoc_factory_generation ) {
        event_EOCs.clear();
        for( event_type event = static_cast<event_type>( 0 ); event < event_type::num_event_types;
             event = static_cast<event_type>( static_cast<int>( event ) + 1 ) ) {
            event_EOCs[event] = {};
        }

        for( const effect_on_condition &eoc : effect_on_conditions::get_all() ) {
            if( eoc.type == eoc_type::EVENT ) {
                event_EOCs[eoc.required_event].emplace_back( eoc.id );
            }
        }
        has_cached = true;
        cached_generation = eoc_factory_generation;
    }

    const std::vector<effect_on_condition_id> &event_eocs = event_EOCs[e.type()];
    if( event_eocs.empty() ) {
        return;
    }
    ...
    if( !alpha ) {
        ...
        static const std::vector<std::string> potential_alphas = {
            "avatar_id", "character", "attacker", "killer", "npc"
        };
        for( const std::string &potential_key : potential_alphas ) {
            const cata_variant value = e.get_variant_or_void( potential_key );
            if( value == cata_variant() ) {
                continue;
            }
            const character_id potential_id = value.get<cata_variant_type::character_id>();
            if( !potential_id.is_valid() ) {
                continue;
            }
            saw_character_id = true;
            if( potential_id == get_avatar().getID() ) {
                alpha = get_talker_for( get_avatar() );
                break;
            }
            if( npc *alpha_character = g->find_npc( potential_id ) ) {
                alpha = get_talker_for( alpha_character );
                break;
            }
        }
        ...
        if( !alpha ) {
            if( saw_character_id ) {
                debugmsg( "EVENT EOCs for %s could not resolve any referenced character; event ignored.",
                          io::enum_to_string( e.type() ) );
                return;
            }
            alpha = get_talker_for( get_avatar() );
        }
    ...
    for( const auto &value : e.data() ) {
        context["npctalk_var_" + value.first] = value.second.get_string();
    }
    dialogue event_dialogue( std::move( alpha ), std::move( beta ), context );

    for( const effect_on_condition_id &eoc : event_eocs ) {
        dialogue d = copy_dialogue( event_dialogue );
        eoc->activate( d );
    }
}
