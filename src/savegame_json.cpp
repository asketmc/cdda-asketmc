...
obj_type_name = { { "OBJECT_NONE", "OBJECT_ITEM", "OBJECT_ACTOR", "OBJECT_PLAYER",
        ...
        "OBJECT_TERRAIN", "OBJECT_FURNITURE", "OBJECT_FURNITURE_ON_VEHICLE"
    }
};
...
void faction::deserialize( const JsonObject &jo )
{
    ...
    if( jo.has_int( "food_supply" ) ) {
        // Legacy saves stored kilocalories as a single integer.
        int64_t legacy_kcal = 0;
        jo.read( "food_supply", legacy_kcal );
        constexpr int64_t max_calories = std::numeric_limits<int>::max();
        constexpr int64_t max_exact_legacy_kcal = max_calories / 1000;
        if( legacy_kcal < 0 ) {
            DebugLog( D_WARNING, D_GAME ) << "Legacy faction '" << id.str()
                                          << "' has a negative food supply; clamping it to zero.";
            food_supply.calories = 0;
        } else if( legacy_kcal > max_exact_legacy_kcal ) {
            DebugLog( D_WARNING, D_GAME ) << "Legacy faction '" << id.str() << "' food supply of "
                                          << legacy_kcal << " kcal exceeds the nutrients limit; "
                                          << "saturating it at " << max_calories << " calories.";
            food_supply.calories = static_cast<int>( max_calories );
        } else {
            food_supply.calories = static_cast<int>( legacy_kcal * 1000 );
        }
    } else if( !jo.read( "fac_food_supply", food_supply ) ) {
        food_supply.calories = 100 * 1000;
    }
...
void basecamp::deserialize( const JsonObject &data )
{
    ...
    if( !data.read( "owner", owner ) ) {
        owner = faction_id( "your_followers" );
    }
    data.read( "name", name );
