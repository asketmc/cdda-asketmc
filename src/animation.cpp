...
void game::draw_cursor_unobscuring( const tripoint &p ) const
{
    const tripoint rp = relative_view_pos( *this, p );
    mvwputch_inv( w_terrain, ( rp.xy() + point_north_east ), c_cyan, "↙" );
    mvwputch_inv( w_terrain, ( rp.xy() + point_south_east ), c_cyan, "↖" );
    mvwputch_inv( w_terrain, ( rp.xy() + point_north_west ), c_cyan, "↘" );
    mvwputch_inv( w_terrain, ( rp.xy() + point_south_west ), c_cyan, "↗" );
#if defined(TILES)
    tilecontext->init_draw_cursor( p );
#endif
}
