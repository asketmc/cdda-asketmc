    ...
    input_context *inp_ctxt, const draw_data_t &data )
{
    ...
    const std::string efficient_key = inp_ctxt != nullptr ?
                                      inp_ctxt->get_desc( "CHOOSE_DESTINATION" ) : "W";
    const std::string direct_key = inp_ctxt != nullptr ?
                                   inp_ctxt->get_desc( "CHOOSE_DESTINATION_DIRECT" ) : "T";
    mvwprintz( wbar, point( 1, 13 ), c_magenta, _( "Press %s to preview efficient route." ),
               efficient_key );
    mvwprintz( wbar, point( 1, 14 ), c_magenta, _( "Press %s to preview direct route." ),
               direct_key );
    mvwprintz( wbar, point( 1, 15 ), c_magenta, _( "Press route button again to confirm." ) );
    if( inp_ctxt != nullptr ) {
        int y = 16;
                                 ...
                                 const draw_data_t &data = draw_data_t() )
{
    ...
    do {
        ...
        if( const cata::optional<tripoint> vec = ictxt.get_direction( action ) ) {
            ...
            const bool direct = action == "CHOOSE_DESTINATION_DIRECT";
