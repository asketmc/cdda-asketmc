...
void game::load_npcs()
{
    ...
    for( const auto &temp : overmap_buffer.get_npcs_near_player( radius ) ) {
        ...
        // A death EOC can keep a marked NPC alive.  Such an NPC still needs to
        // be activated and loaded like every other nearby overmap NPC.
...
void game::validate_camps()
{
    ...
    // Synchronize the avatar index with every overmap already in memory.  This restores
    // camps loaded before avatar deserialization and camps retained when changing characters.
    for( const tripoint_abs_omt &camp_pos : overmap_buffer.get_loaded_camp_locations() ) {
        u.camps.insert( camp_pos );
    }
}
...
bool game::is_game_over()
{
    ...
    if( u.is_dead_state() ) {
        ...
        get_event_bus().send<event_type::character_dies>( u.getID() );
        if( !u.is_dead_state() ) {
            return false;
        }
    ...
    do {
        if( action == "ADD_ZONE" ) {
            ...
            if( action == "UP" ) {
                ...
                stuff_changed = stuff_changed || zones_changed;
...
look_around_result game::look_around( look_around_params looka_params )
{
    return look_around( looka_params.show_window, looka_params.center, looka_params.start_point,
                        looka_params.has_first_point, looka_params.select_zone, looka_params.peeking,
                        false, tripoint_zero, looka_params.change_lv );
}
...
bool game::walk_move( const tripoint &dest_loc, const bool via_ramp, const bool furniture_move )
{
    ...
    if( grabbed && u.get_grab_type() == object_type::FURNITURE ) {
        ...
        if( vp_grabbed && vp_grabbed->has_loaded_furniture() ) {
            shifting_furniture = !pushing && !pulling;
        } else {
            grabbed = false;
        }
...
bool game::can_move_furniture( tripoint fdest, const tripoint &dp )
{
    ...
    if( pulling_furniture && !is_empty( u.pos() + dp ) ) {
        return false;
    }
    if( has_floor && !m.has_flag( ter_furn_flag::TFLAG_FLAT, fdest ) && !is_ramp_or_road ) {
        return false;
    }
    if( m.has_furn( fdest ) ) {
        return false;
    }
    const optional_vpart_position destination_vehicle = m.veh_at( fdest );
    if( destination_vehicle && !destination_vehicle->can_load_furniture() ) {
        return false;
    }
    return !has_floor || m.tr_at( fdest ).is_null();
}
...
bool game::grabbed_furn_move( const tripoint &dp )
{
    ...
    if( destination_vehicle ) {
        const cata::optional<vpart_reference> loader =
            destination_vehicle->part_with_feature( "FURNITURE_TIEDOWN", true );
        if( !loader ) {
            return true;
        }
        u.grab( object_type::NONE );
        loader->part().load_furniture( m, fpos );
        destination_vehicle->vehicle().invalidate_mass();
        add_msg( _( "You load the furniture onto the vehicle." ) );
        const tripoint new_grab_point = fdest -
                                        ( u.pos() + ( shifting_furniture ? tripoint_zero : dp ) );
        if( std::abs( new_grab_point.x ) < 2 && std::abs( new_grab_point.y ) < 2 ) {
            u.grab( object_type::FURNITURE_ON_VEHICLE, new_grab_point );
        }
...
bool game::grabbed_move( const tripoint &dp, const bool via_ramp, const bool stairs_move )
{
    if( u.get_grab_type() == object_type::NONE ) {
        return false;
    }
    ...
    if( u.get_grab_type() == object_type::VEHICLE ) {
        return stairs_move ? grabbed_veh_move_stairs( dp ) : grabbed_veh_move( dp );
    }
    ...
    if( u.get_grab_type() == object_type::FURNITURE_ON_VEHICLE ) {
        u.assign_activity( player_activity( move_furniture_activity_actor( dp, via_ramp, true ) ) );
        return true;
    }
...
void game::vertical_move( int movez, bool force, bool peeking )
{
    ...
    const cata::optional<tripoint> old_grabbed_pos = u.grab_point != tripoint_zero ?
            cata::optional<tripoint>( old_pos + u.grab_point ) : cata::nullopt;
    point submap_shift;
    ...
    if( old_grabbed_pos ) {
        const tripoint shifted_grabbed_pos =
            *old_grabbed_pos - tripoint( sm_to_ms_copy( submap_shift ), 0 );
        u.grab_point = shifted_grabbed_pos - u.pos();
        const cata::optional<tripoint> dir = choose_direction(
                _( "Select a direction to push the grabbed object towards." ) );
        if( !dir ) {
            u.grab( object_type::NONE );
        } else if( dir->xy() == point_zero ) {
            add_msg( _( "You need to push the grabbed object away from the stairs." ) );
            u.grab( object_type::NONE );
        } else {
            grabbed_move( *dir - u.grab_point, false, true );
        }
    }
