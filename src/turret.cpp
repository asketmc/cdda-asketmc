...
const itype *turret_data::ammo_data() const
{
    ...
    if( part->base.uses_firing_requirements() ) {
        item prepared( part->base );
        vehicle::prepare_multimag_pockets( *veh, prepared, prepared.gun_get_mode_id(),
                                           ammo_current() );
        return prepared.ammo_data();
    }
    if( uses_vehicle_tanks_or_batteries() ) {
...
std::set<itype_id> turret_data::ammo_options() const
{
    ...
    if( part->base.uses_firing_requirements() ) {
        ...
        if( entries == nullptr ) {
            static const gun_mode_id default_mode( "DEFAULT" );
            entries = gun.type->firing_requirements.for_mode( default_mode );
        }
        if( entries == nullptr ) {
            return opts;
        }
        for( const pocket_consumption_entry &entry : *entries ) {
            const item_pocket *pocket = gun.pocket_by_id( entry.pocket );
            if( pocket == nullptr || gun.pocket_accepts_battery( pocket ) ) {
                continue;
            }
            ...
            if( data->type == item_pocket::pocket_type::MAGAZINE ) {
                for( const std::pair<const ammotype, int> &restriction : data->ammo_restriction ) {
                    for( const vpart_reference &tank_ref : veh->get_avail_parts( VPFLAG_FLUIDTANK ) ) {
                        const itype_id ammo = tank_ref.part().ammo_current();
                        if( !ammo.is_null() && ammo->ammo &&
                            ammo->ammo->type == restriction.first && tank_ref.part().ammo_remaining() > 0 ) {
                            opts.insert( ammo );
                        }
                    }
                }
            } else if( data->type == item_pocket::pocket_type::MAGAZINE_WELL ) {
                const item *magazine = pocket->magazine_current();
                if( magazine != nullptr && magazine->ammo_data() != nullptr ) {
                    opts.insert( magazine->ammo_data()->get_id() );
                }
            }
        }
        return opts;
...
bool turret_data::can_unload() const
{
    ...
    if( part->base.uses_firing_requirements() ) {
        for( const std::pair<const gun_mode_id, std::vector<pocket_consumption_entry>> &mode :
             part->base.type->firing_requirements.per_mode ) {
            for( const pocket_consumption_entry &entry : mode.second ) {
                if( part->base.ammo_remaining_in_pocket( entry.pocket ) > 0 ) {
                    return true;
                }
            }
        }
...
{
    std::vector<multimag_display_pocket> result;
    if( !veh || !part || !part->base.uses_firing_requirements() ) {
        return result;
    }
    const item &gun = part->base;
    ...
    const std::map<std::string, multimag_pocket_state> bindings =
        vehicle::prepare_multimag_pockets( *veh, prepared, mode, ammo_current() );
    for( const pocket_consumption_entry &entry : *entries ) {
        multimag_display_pocket display;
        display.pocket_id = entry.pocket;
        display.local_qty = gun.ammo_remaining_in_pocket( entry.pocket );
        display.effective_qty = prepared.ammo_remaining_in_pocket( entry.pocket );
        display.per_use_qty = prepared.effective_consumption_qty( entry );
        display.vehicle_bound = bindings.count( entry.pocket ) != 0;
        const item_pocket *pocket = prepared.pocket_by_id( entry.pocket );
        if( pocket != nullptr ) {
            for( const item *contained : pocket->all_items_top() ) {
                if( contained == nullptr ) {
                    continue;
                }
                const itype *ammo = contained->ammo_data();
                display.ammo_itype = ammo != nullptr ? ammo->get_id() : contained->typeId();
                if( !display.ammo_itype.is_null() ) {
                    break;
                }
            }
        }
...
turret_data::status turret_data::query() const
{
    ...
    if( gun.uses_firing_requirements() ) {
        ...
        vehicle::prepare_multimag_pockets( *veh, prepared, prepared.gun_get_mode_id(),
                                           ammo_current() );
        return prepared.feasible_shots( prepared.gun_get_mode_id(), 1 ) >= 1 ?
...
void turret_data::prepare_fire( Character &you )
{
    ...
    item &gun = part->base;
    if( gun.uses_firing_requirements() ) {
        mm_bindings_ = vehicle::prepare_multimag_pockets( *veh, gun, gun.gun_get_mode_id(),
                       ammo_current() );
        return;
    }
...
void turret_data::post_fire( Character &you, int shots )
{
    ...
    item &gun = part->base;
    if( gun.uses_firing_requirements() ) {
        vehicle::drain_back_multimag( *veh, gun, mm_bindings_ );
        for( const std::pair<const std::string, multimag_pocket_state> &binding : mm_bindings_ ) {
            item_pocket *pocket = gun.pocket_by_id( binding.first );
            if( pocket != nullptr ) {
                pocket->clear_items();
            }
        }
