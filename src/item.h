...
class item : public visitable
{
    public:
        ...
        */
        ...
        /** Multi-pocket consumption helpers.  These are active only when the
         * item type declares firing_requirements/consumption_per_use. */
        ...
        int ammo_consume_in_pocket( const std::string &id, int qty, const tripoint &pos );
        bool pocket_accepts_battery( const item_pocket *pocket ) const;
        const item_pocket *primary_ammo_pocket() const;
        int effective_consumption_qty( const pocket_consumption_entry &entry ) const;
        int feasible_shots( const gun_mode_id &mode, int requested ) const;
        int feasible_tool_uses( int requested = 1 ) const;
        int consume_shots( const gun_mode_id &mode, int shots, const tripoint &pos,
                           Character *carrier );
        int consume_tool_uses( int uses, const tripoint &pos, Character *carrier );
