...
// Empty for every legacy item.  Tools use DEFAULT; guns may define a cost per
// firing mode.  Keeping this opt-in is what preserves 0.G consumption paths.
struct firing_requirement_set {
    ...
    const std::vector<pocket_consumption_entry> *for_mode( const gun_mode_id &mode ) const {
        const auto found = per_mode.find( mode );
        return found == per_mode.end() ? nullptr : &found->second;
    }

    void deserialize_firing_requirements( const JsonObject &jo, const std::string &member );
    void deserialize_consumption_per_use( const JsonObject &jo, const std::string &member );
};
...
struct itype_variant_data {
    ...
    // Expand snippets in this variant description once when the item is created.
...
struct itype {
        ...
        */
        ...
        // Explicit per-pocket consumption.  Absent on all existing 0.G items.
        ...
        // Expand snippets in the description once and save the result on each item.
        bool expand_snippets = false;
    ...
    public:
        ...
        /** First concrete ammo item used by a tool, or the null id for tools without ammo. */
        itype_id tool_slot_first_ammo() const;
