                            ...
                            bool cumulative, bool camera, int penalty )
{
    ...
    if( !fov_3d ) {
        ...
        cast_zlight<float, sight_calc, sight_check, accumulate_transparency>(
            ...
            directions_to_cast );
        // Multiple remote cameras accumulate into one cache.  Processing that
        // pooled cache from one camera origin would erase visibility supplied
        // by another camera, so ledge post-processing is player-view only.
        if( !camera ) {
            seen_cache_process_ledges( seen_caches, floor_caches, cata::nullopt );
        }
    }
...
void map::seen_cache_process_ledges( array_of_grids_of<float> &seen_caches,
                                     ...
                                     const cata::optional<tripoint> &override_p ) const
{
    const Character &player_character = get_player_character();
    const tripoint origin = override_p ? *override_p : player_character.pos();
        ...
        for( int smy = 0; smy < my_MAPSIZE; ++smy ) {
            for( int sx = 0; sx < SEEX; ++sx ) {
                for( int sy = 0; sy < SEEY; ++sy ) {
                    ...
                    for( int z = origin.z - 1; z >= min_z; --z ) {
                        const int cache_z = z + OVERMAP_DEPTH;
                        if( ( *seen_caches[cache_z] )[x][y] == 0.0f ) {
                            break;
                        }
                        if( ( *floor_caches[cache_z] )[x][y] ) {
                            const tripoint p( x, y, z );
                            const int coverage = override_p ? ledge_coverage( origin, p ) :
                                                 ledge_coverage( player_character, p );
                            if( coverage > 100 ) {
                                ( *seen_caches[cache_z] )[x][y] = 0.0f;
                            }
                            break;
                        }
                    }
