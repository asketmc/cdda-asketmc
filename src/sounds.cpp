...
std::vector<sfx::sound_effect_choice> sfx::gun_sound_candidates(
    const std::string &weapon_id, const std::string &ammo_type, const std::string &suffix,
    const std::string &legacy_suppressed_variant )
{
    std::vector<sound_effect_choice> result = { { "fire_gun" + suffix, weapon_id } };
    if( !ammo_type.empty() ) {
        result.push_back( { "fire_ammo" + suffix, ammo_type } );
    }
    if( suffix == "_suppressed" && !legacy_suppressed_variant.empty() ) {
        result.push_back( { "fire_gun", legacy_suppressed_variant } );
    }
    result.push_back( { "fire_gun" + suffix, "default" } );
    result.push_back( { "fire_gun", "default" } );
    return result;
}

std::string sfx::melee_sound_fallback_variant( const std::string &skill,
        const int weapon_volume )
{
    if( skill == "bashing" ) {
        return weapon_volume > 8 ? "big_bash" : "small_bash";
    }
    if( skill == "cutting" ) {
        return weapon_volume > 6 ? "big_cutting" : "small_cutting";
    }
    if( skill == "stabbing" ) {
        return weapon_volume > 4 ? "big_stabbing" : "small_stabbing";
    }
    if( skill == "unarmed" ) {
        return "unarmed";
    }
...
std::string sfx::melee_hit_sound_id( const bool target_is_monster,
                                     const std::string &material )
{
    return target_is_monster && material == "steel" ? "melee_hit_metal" : "melee_hit_flesh";
}
...
void sfx::generate_gun_sound( const Character &source_arg, const item &firing )
{
    ...
    const std::vector<sound_effect_choice> candidates = gun_sound_candidates(
                weapon_id.str(), ammo_type.is_null() ? "" : ammo_type.str(), selected_sound,
                itype_weapon_fire_suppressed.str() );
    bool played = false;
    for( std::size_t i = 0; i + 1 < candidates.size(); ++i ) {
        const sound_effect_choice &candidate = candidates[i];
        if( has_exact_variant_sound( candidate.id, candidate.variant, seas_str, indoors, night ) ) {
            play_variant_sound( candidate.id, candidate.variant, seas_str, indoors, night,
    ...
    if( !played ) {
        const sound_effect_choice &fallback = candidates.back();
        play_variant_sound( fallback.id, fallback.variant, seas_str, indoors, night,
                            heard_volume, angle, 0.8, 1.2 );
    }
...
{
struct sound_thread {
    sound_thread( const item &weapon, const tripoint &source, const tripoint &target,
...
void sfx::generate_melee_sound( const item &weapon, const tripoint &source,
...
sfx::sound_thread::sound_thread( const item &weapon, const tripoint &source,
                                 const tripoint &target,
                                 const bool hit,
    ...
    , material( material )
{
    ...
    weapon_volume = !weapon.is_null() ? weapon.volume() / units::legacy_volume_factor : 0;
}
...
void sfx::sound_thread::operator()() const
{
    ...
    const std::string skill_variant = melee_sound_fallback_variant( weapon_skill.str(),
                                      weapon_volume );
    ...
    if( hit ) {
        ...
        const std::string melee_hit_material = melee_hit_sound_id( targ_mon, material );
...
void sfx::do_footstep( const Character &ch )
{
    ...
    if( std::chrono::duration_cast<std::chrono::milliseconds> ( sfx_time ).count() > 400 ) {
        int heard_volume = sfx::get_heard_volume( ch.pos() );

        if( heard_volume <= 0 ) {
            return;
        }

        units::angle heard_angle = ch.is_npc() ? sfx::get_heard_angle( ch.pos() ) : 0_degrees;
        const auto terrain = here.ter( ch.pos() ).id();
        ...
        auto veh_displayed_part = here.veh_at( ch.pos() ).part_displayed();
...
void sfx::generate_melee_sound( const item &, const tripoint &, const tripoint &,
                                bool, bool,
                                const std::string & ) { }
