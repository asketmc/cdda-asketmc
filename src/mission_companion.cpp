...
static const std::array < miss_data, Camp_Have_Meal + 1 > miss_info = { {
        {
            //  No_Mission
            "",
            no_translation( "" )
...
mission_id mission_id_of( const std::string &str )
{
    ...
    for( int i = No_Mission + 1; i <= Camp_Have_Meal; i++ ) {
...
{
    npc_ptr comp = preselected_choice != nullptr ? preselected_choice :
                   companion_choose( required_skills, silent_failure );
    if( comp == nullptr ) {
        return comp;
    }
