...
using dialogue_conditional = std::function<bool( const dialogue & )>;
using dialogue_conditional_map = std::unordered_map<std::string, dialogue_conditional>;
...
struct dialogue {
        ...
        dialogue( std::unique_ptr<talker> alpha_in, std::unique_ptr<talker> beta_in,
                  const dialogue_conditional_map &conditionals,
                  const std::unordered_map<std::string, std::string> &context );
        talker *actor( bool is_beta ) const;

        void set_value( const std::string &key, const std::string &value ) const;
        void remove_value( const std::string &key ) const;
        std::string get_value( const std::string &key ) const;
        const std::unordered_map<std::string, std::string> &get_context() const;
        void set_conditional( const std::string &key, const dialogue_conditional &value ) const;
        bool evaluate_conditional( const std::string &key ) const;
        const dialogue_conditional_map &get_conditionals() const;
    ...
    private:
        ...
        // Dialogue-scoped variables are copied into child EOCs but never back into their parent.
        mutable std::unordered_map<std::string, std::string> context;
        // Named conditions follow the same child-only scope as dialogue context variables.
        mutable dialogue_conditional_map conditionals;
        // Protect named conditions from direct or mutual recursive evaluation.
        mutable std::unordered_set<std::string> evaluating_conditionals;
        /**
