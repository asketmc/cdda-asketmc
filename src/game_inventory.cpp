...
item_location game_menus::inv::titled_filter_menu( const item_location_filter &filter, Character &you,
        const std::string &title, int radius, const std::string &none_message )
{
...
drop_locations game_menus::inv::ebooksave( Character &who, item_location &ereader )
{
    ...
    const auto unique_books = []( const auto &locations ) {
        std::set<itype_id> selected_types;
        std::vector<item_location> books;
        for( const auto &entry : locations ) {
            if( selected_types.insert( entry.first->typeId() ).second ) {
    ...
    auto make_raw_stats = [&available_charges, &ereader, &unique_books](
                              const std::vector<std::pair<item_location, int>> &locs
    ) {
        const std::vector<item_location> books = unique_books( locs );
        ...
        const inventory_selector::stat charge_stat = display_stat( "", required_charges,
                available_charges, int_to_str );
        const std::string charges = charge_stat[1] + charge_stat[2] + charge_stat[3];
    ...
    std::set<itype_id> selected_types;
    selected.remove_if( [&selected_types]( const drop_location &entry ) {
        return !selected_types.insert( entry.first->typeId() ).second;
    } );
    return selected;
}
...
class attach_vehicle_tool_inventory_preset : public inventory_selector_preset
{
    public:
        explicit attach_vehicle_tool_inventory_preset( const std::set<itype_id> &allowed_types ) :
            allowed_types( allowed_types ) {
        }

        bool is_shown( const item_location &loc ) const override {
            return allowed_types.count( loc->typeId() ) != 0;
        }

        std::string get_denial( const item_location &loc ) const override {
            const item &candidate = *loc.get_item();
            if( !candidate.empty() || !candidate.toolmods().empty() ) {
                return _( "The tool must be empty and have no installed tool mods." );
            }
            return std::string();
        }
...
item_location game_menus::inv::veh_tool_attach( Character &you, const std::string &part_name,
        const std::set<itype_id> &allowed_types )
{
    return inv_internal( you, attach_vehicle_tool_inventory_preset( allowed_types ),
                         string_format( _( "Attach a tool to the %s" ), part_name ), 1,
                         string_format( _( "You have no compatible empty tools for the %s.\n\nAllowed tools:\n%s" ),
                                        part_name,
    enumerate_as_string( allowed_types, []( const itype_id & tool ) {
        return tool->nname( 1 );
    } ) ),
    string_format( _( "Choose a tool to attach to the %s" ), part_name ) );
}
...
{
    return install_bionic( you, you, patient, surgeon );
}
