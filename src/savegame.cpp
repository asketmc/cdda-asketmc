...
struct camp_migration_data {
    translation name;
    faction_id owner;
    bool require_full_terrain_neighborhood = false;
};

static std::map<oter_type_str_id, camp_migration_data> camp_migrations;

void overmap::load_camp_migration( const JsonObject &jo )
{
    const JsonObject migration = jo.get_object( "camp_migrations" );
    std::string name;
    oter_type_str_id terrain;
    faction_id owner;
    migration.read( "name", name );
    migration.read( "overmap_terrain", terrain );
    migration.read( "faction", owner );
    camp_migration_data data;
    data.name = translation::to_translation( name );
    data.owner = owner;
    migration.read( "require_full_terrain_neighborhood", data.require_full_terrain_neighborhood );
    camp_migrations[terrain] = std::move( data );
}

void overmap::reset_camp_migrations()
{
    camp_migrations.clear();
}

bool overmap::terrain_should_have_camp( const oter_type_str_id &terrain )
{
    return camp_migrations.count( terrain ) != 0;
}

void overmap::migrate_camps( const std::vector<tripoint_abs_omt> &points )
{
    for( const tripoint_abs_omt &camp_point : points ) {
        const tripoint_om_omt local = project_remain<coords::om>( camp_point ).remainder_tripoint;
        const oter_type_str_id terrain = ter( local )->get_type_id();
        const auto migration = camp_migrations.find( terrain );
        if( migration == camp_migrations.end() ) {
            continue;
        }

        if( migration->second.require_full_terrain_neighborhood ) {
            bool full_neighborhood = true;
            for( int x = -1; x <= 1 && full_neighborhood; ++x ) {
                for( int y = -1; y <= 1; ++y ) {
                    if( x == 0 && y == 0 ) {
                        continue;
                    }
                    const tripoint_om_omt neighbor = local + point( x, y );
                    if( !inbounds( neighbor ) || ter( neighbor )->get_type_id() != terrain ) {
        ...
        if( find_camp( camp_point.xy() ) ) {
            continue;
        }

        basecamp &new_camp = get_map().add_camp( camp_point, "faction_camp", false );
        new_camp.set_owner( migration->second.owner );
        new_camp.set_name( migration->second.name.translated() );
        new_camp.define_camp( camp_point, "faction_base_bare_bones_NPC_camp_0", false );
    }
}
    ...
    for( JsonMember om_member : jsobj ) {
        ...
        if( name == "layers" ) {
                    ...
                    for( int i = 0; i < OMAPX; i++ ) {
                        if( count == 0 ) {
                            ...
                            if( obsolete_terrain( tmp_ter ) ) {
                                ...
                                if( terrain_should_have_camp( tmp_otid->get_type_id() ) ) {
                                    for( int p = i; p < i + count; ++p ) {
                                        camps_to_place.emplace_back( project_combine(
                                                pos(), tripoint_om_omt( p, j, z - OVERMAP_DEPTH ) ) );
                                    }
                                }
                            } else {
            ...
            for( JsonObject camp_json : camps_json ) {
                ...
                get_player_character().camps.insert( new_camp.camp_omt_pos() );
...
{
    cata::ifstream fin( path.get_unrelative_path(), std::ios::binary );
    size_t json_offset = chkversion( fin );
...
void npc::import_and_clean( const JsonObject &data )
{
    ...
    stashed_outbounds_backlog_owned = defaults.stashed_outbounds_backlog_owned;
