...
static const std::string camp_crafting_ui_parameter = "CAMP_CRAFTING_UI";
...
void basecamp::get_available_missions_by_dir( mission_data &mission_key, const point &dir )
{
    ...
    std::map<recipe_id, translation> craft_recipes = recipe_deck( dir );
    {
        ...
        if( npc_list.size() < 3 ) {
            ...
            const mission_id ui_miss_id = { Camp_Crafting, camp_crafting_ui_parameter, dir };
            mission_key.add_start(
                ui_miss_id, dir_abbr + _( " Craft using full interface" ),
                _( "Open the normal crafting interface with every recipe known by an assigned "
                   "worker or provided by a camp expansion.  Batch size and the actual worker "
                   "are selected before the mission begins." ), true );
        }
...
void basecamp::get_available_missions( mission_data &mission_key )
{
    ...
    if( !by_radio ) {
        faction *camp_faction = fac();
        const int food_available = camp_faction == nullptr ? 0 : camp_faction->food_supply.kcal();
        {
            ...
            entry = string_format( _( "Notes:\n"
                                   ...
                                   food_available, food_available / 2500 );
            mission_key.add( { miss_id, false }, name_display_of( miss_id ),
                             entry );
        }
        {
            ...
            entry = _( "Notes:\n"
                       "Eat a prepared meal from the camp larder.  The meal carries the calories, "
                       "vitamins, and mutagenic nutrients represented by the stored food.\n"
                       "Difficulty: N/A\n"
                       "Risk: None\n" );
...
void basecamp::player_eats_meal()
{
    ...
    int kcal_to_eat = std::min( 2500, food_available );
    ...
    feed_workers( you, std::move( dinner ), true );
}
    ...
    switch( miss_id.id.id ) {
        ...
        case Camp_Crafting:
            if( miss_id.ret ) {
                ...
                if( miss_id.id.parameters == camp_crafting_ui_parameter ) {
                    start_crafting_with_ui( miss_id.id );
                } else {
                    start_crafting( recipe_group::get_building_of_recipe( miss_id.id.parameters ),
                                    miss_id.id );
                }
            }
            break;
...
{
    faction *camp_faction = fac();
    if( must_feed && ( camp_faction == nullptr ||
                       camp_faction->food_supply.kcal() < time_to_food( duration ) ) ) {
    ...
    if( comp != nullptr ) {
        ...
        if( must_feed ) {
            feed_workers( *comp, camp_food_supply( duration ) );
        }
        ...
        const std::map<skill_id, int> &required_skills )
{
    ...
    while( true ) { //  We'll break out of the loop when all the workers have been assigned
        ...
        if( must_feed && camp_faction->food_supply.kcal() <
            time_to_food( work_days * ( result.size() + 1 ) ) ) {
            if( result.empty() ) {
                popup( _( "You don't have enough food stored to feed your companion for this task." ) );
                return result;
            } else {
    ...
    if( result.empty() ) {
        ...
        if( must_feed ) {
            ...
            for( npc_ptr &comp : result ) {
                work_party.emplace_back( *comp );
            }
            feed_workers( work_party, camp_food_supply( work_days * result.size() ) );
        }
        return result;
    }
}
    ...
    for( auto &expansion : expansions ) {
        ...
        for( const tripoint &pos : expansion_map.points_in_rectangle( mapmin, mapmax ) ) {
            ...
            if( expansion_map.veh_at( pos ).has_value() &&
                ...
                for( const auto &entry : expansion_map.veh_at( pos )->get_tools() ) {
                    const item &tool = entry.first;
...
{
    faction *camp_faction = fac();
    if( camp_faction == nullptr || camp_faction->food_supply.kcal() < time_to_food( 3_hours ) ) {
        popup( _( "You don't have enough food stored to feed your companion." ) );
        return;
    }
...
void basecamp::start_crafting_with_ui( const mission_id &miss_id )
{
    ...
    for( const npc_ptr &worker : available_workers ) {
        ...
        for( const proficiency_id &proficiency : worker->known_proficiencies() ) {
            combined_worker.add_proficiency( proficiency, true );
        }
    }
    if( camp_recipes.size() == 0 ) {
        popup( _( "The camp has no recipes available." ) );
        return;
    }
    ...
    uilist choose_worker;
    choose_worker.title = _( "Choose a worker to craft" );
    std::vector<npc_ptr> candidates;
    for( const npc_ptr &worker : available_workers ) {
        const bool has_skills = worker->has_recipe_requirements( *making );
        const bool has_proficiencies = making->character_has_required_proficiencies( *worker );
        const bool can_craft = has_skills && has_proficiencies;
        std::string label = worker->disp_name();
        if( !has_skills ) {
            label += _( " (lacks required skills)" );
        } else if( !has_proficiencies ) {
            label += _( " (lacks required proficiencies)" );
        }
        const int candidate_index = static_cast<int>( candidates.size() );
        candidates.push_back( worker );
        choose_worker.addentry( candidate_index, can_craft, MENU_AUTOASSIGN, label );
    }
    choose_worker.query();
    if( choose_worker.ret < 0 || static_cast<size_t>( choose_worker.ret ) >= candidates.size() ) {
        return;
    }
    npc_ptr worker = candidates[choose_worker.ret];
    ...
    const time_duration work_days = base_camps::to_workdays( making->batch_duration( *worker,
                                    batch_size ) );
    const int kcal_needed = time_to_food( work_days );
    faction *camp_faction = fac();
    const int kcal_available = camp_faction == nullptr ? 0 : camp_faction->food_supply.kcal();
    if( !query_yn( _( "This job will cost %d kcal (the camp has %d kcal).  Continue?" ),
                   kcal_needed, kcal_available ) ) {
        return;
    }
    if( kcal_needed > kcal_available ) {
        popup( _( "The camp does not have enough stored food." ) );
        return;
    }
                              ...
                              const std::string &skill, int difficulty, const bool cancel )
{
    ...
    // Missions that are not fixed-time pay their food cost when the worker returns.
    int need_food = time_to_food( mission_time - reserve_time );
    faction *camp_faction = fac();
    if( camp_faction == nullptr || camp_faction->food_supply.kcal() < need_food ) {
...
void basecamp::recruit_return( const mission_id &miss_id, int score )
{
    ...
    while( true ) {
        ...
        description += string_format( _( "Faction Food:%9d days\n\n" ),
                                      camp_faction->food_supply.kcal() / 2500 );
...
int basecamp::recipe_batch_max( const recipe &making ) const
{
    ...
    for( size_t batch_size = 1000; batch_size > 0; batch_size /= 10 ) {
        for( int iter = 0; iter < max_checks; iter++ ) {
            ...
            if( can_make && camp_faction->food_supply.kcal() > food_req ) {
...
int basecamp::recruit_evaluation( int &sbase, int &sexpansions, int &sfaction, int &sbonus ) const
{
    ...
    faction *camp_faction = fac();
    sfaction = camp_faction == nullptr ? 0 :
               std::min( camp_faction->food_supply.kcal() / 10000, 10 );
...
nutrients basecamp::camp_food_supply( nutrients &change )
{
    ...
    if( change.calories < 0 && change.vitamins.empty() ) {
        const int requested_calories = -change.calories;
        const int available_calories = owner_faction->food_supply.calories;
        if( available_calories <= 0 ) {
            return consumed;
        }
        if( requested_calories >= available_calories ) {
            consumed = owner_faction->food_supply;
            const int missing_kcal = ( requested_calories - available_calories ) / 1000;
            if( owner_faction->consumes_food ) {
                owner_faction->food_supply = nutrients();
                faction *yours = get_player_character().get_faction();
                if( owner_faction == yours ) {
                    yours->likes_u -= missing_kcal / 1250;
                    yours->respects_u -= missing_kcal / 625;
                    yours->trusts_u -= missing_kcal / 625;
                }
            }
            return consumed;
        }

        const double fraction = static_cast<double>( requested_calories ) / available_calories;
        consumed = owner_faction->food_supply;
        consumed *= fraction;
        // Preserve the exact calorie charge while vitamins follow the larder's proportions.
        consumed.calories = requested_calories;
        if( owner_faction->consumes_food ) {
            owner_faction->food_supply -= consumed;
        }
        return consumed;
    }

    owner_faction->food_supply += change;
    return change;
}
...
{
    nutrients amount;
    amount.calories = change * 1000;
    return camp_food_supply( amount );
}

nutrients basecamp::camp_food_supply( time_duration work )
{
                             ...
                             nutrients food, bool is_player_meal )
{
    ...
    food /= workers.size();
    for( const std::reference_wrapper<Character> &worker_reference : workers ) {
        Character &worker = worker_reference.get();
        if( !allowed_access_by( worker ) ) {
            debugmsg( "Character %s tried to access food stores at camp %s without permission",
                      worker.disp_name(), name );
        }
        worker.add_msg_if_player( _( "You grab a prepared meal from storage and chow down." ) );
        const units::volume filling_volume = std::max(
                0_ml, worker.stomach.capacity( worker ) / 2 - worker.stomach.contains() );
        worker.stomach.ingest( food_summary{ 0_ml, filling_volume, food } );
        worker.update_stomach( calendar::turn, calendar::turn );
    }
}
...
bool basecamp::distribute_food()
{
    ...
    if( fac() == nullptr ) {
        popup( _( "This camp has no valid owner.  Aborting…" ) );
        return false;
    }

    const bool distribute_vitamins = query_yn(
            _( "Do you also wish to distribute comestibles without calories, such as multivitamins and mutagens?" ) );
    ...
    const auto consume_non_recursive = [&]( item & it, item * const container ) {
        ...
        nutrients from_item = getAverageJoe().compute_effective_nutrients( it ) * it.count();
        if( const use_function *drug_use = it.type->get_use( "consume_drug" ) ) {
            const consume_drug_iuse *drug = dynamic_cast<const consume_drug_iuse *>(
                                                drug_use->get_actor_ptr() );
            if( drug != nullptr ) {
                for( const auto &vitamin : drug->vitamins ) {
                    const int average_units = ( vitamin.second.first + vitamin.second.second ) / 2;
                    from_item.vitamins[vitamin.first] += average_units * it.count();
                }
            }
        }
        from_item *= rot_multip( it, container );
        if( from_item.kcal() <= 0 && ( !distribute_vitamins || from_item.vitamins.empty() ) ) {
            return false;
        }
        nutrients_to_add += from_item;
        return true;
    };
    ...
    if( nutrients_to_add.kcal() > 0 ) {
        popup( _( "You distribute %d kcal worth of food to your companions." ),
               nutrients_to_add.kcal() );
    } else {
        popup( _( "You distribute vitamins, drugs, and mutagens to your companions." ) );
    }
...
{
    const auto place_on_map = [this, &result]( map &target_bay,
                               const tripoint_abs_ms &storage_origin ) {
        const bool found_storage_tile = form_storage_zones( target_bay, storage_origin );
        ...
        apply_camp_ownership( target_bay, new_spot, ( found_storage_tile || by_radio ) ? 10 : 0 );
    };
