...
veh_menu_item &veh_menu_item::hotkey( const input_event &event )
{
    this->_hotkey_char = cata::nullopt;
    this->_hotkey_action = cata::nullopt;
    this->_hotkey_event = event;
    return *this;
}
...
void veh_menu::sort(
    const std::function<bool( const veh_menu_item &, const veh_menu_item & )> &comparer )
{
...
{
    last_selected = keep_last_selected ? last_selected : cata::nullopt;
    items.clear();
}
    ...
    private:
        ...
        void select( uilist *menu ) override {
            ...
            if( menu->selected < 0 || menu->selected >= static_cast<int>( points.size() ) ) {
                ...
                player_character.view_offset = points[menu->selected] - player_character.pos();
                player_character.view_offset.z = 0;
            }
            g->invalidate_main_ui_adaptor();
            if( on_select ) {
                on_select( menu->selected );
                map &m = get_map();
...
bool veh_menu::query()
{
    ...
    veh_menu_cb callback( locations );
    callback.on_select = [this]( const int selected ) {
        if( selected >= 0 && static_cast<size_t>( selected ) < items.size() &&
            items[selected]._on_select ) {
            items[selected]._on_select();
        }
    };
    ...
    if( last_selected.has_value() ) {
        menu.selected = std::min( last_selected.value(), static_cast<int>( items.size() ) - 1 );
    } else {
        const auto selected = std::find_if( items.begin(), items.end(),
        []( const veh_menu_item & item ) {
            return item._selected;
        } );
        menu.selected = selected == items.end() ? 0 :
                        static_cast<int>( std::distance( items.begin(), selected ) );
    }
    menu.query();
