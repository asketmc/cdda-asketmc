    ...
    if( veh && veh->cruise_on ) {
        ...
        for( size_t e = 0; e < veh->engines.size(); e++ ) {
            if( veh->is_engine_on( e ) &&
                !( veh->is_perpetual_type( e ) || veh->is_engine_type( e, fuel_type_battery ) ||
...
std::pair<std::string, nc_color> display::vehicle_battery_percent_text_color( const Character &u )
{
    ...
    vehicle *veh = display::vehicle_driven( u );
    if( veh ) {
        const int max_battery = veh->fuel_capacity( fuel_type_battery );
        const int cur_battery = veh->fuel_left( fuel_type_battery );
        if( max_battery != 0 ) {
            const int percent = cur_battery * 100 / max_battery;
