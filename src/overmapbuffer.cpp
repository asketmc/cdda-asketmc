...
basecamp &overmapbuffer::add_camp( const basecamp &camp )
{
    const point_abs_omt omt = camp.camp_omt_pos().xy();
    const overmap_with_local_coords om_loc = get_om_global( omt );
    om_loc.om->camps.push_back( camp );
    return om_loc.om->camps.back();
}

std::vector<tripoint_abs_omt> overmapbuffer::get_loaded_camp_locations() const
{
    std::vector<tripoint_abs_omt> result;
    for( const auto &om : overmaps ) {
        result.reserve( result.size() + om.second->camps.size() );
        for( const basecamp &camp : om.second->camps ) {
            result.push_back( camp.camp_omt_pos() );
        }
    }
    return result;
}
...
{
    const auto flatten = []( const int cost ) {
        return cost < 1 ? cost : static_cast<int>( std::log2( cost ) );
    };

    orig.road_cost = flatten( orig.road_cost );
    orig.field_cost = flatten( orig.field_cost );
    orig.dirt_road_cost = flatten( orig.dirt_road_cost );
    orig.trail_cost = flatten( orig.trail_cost );
    orig.forest_cost = flatten( orig.forest_cost );
    orig.small_building_cost = flatten( orig.small_building_cost );
    orig.shore_cost = flatten( orig.shore_cost );
    orig.swamp_cost = flatten( orig.swamp_cost );
    orig.water_cost = flatten( orig.water_cost );
    orig.air_cost = flatten( orig.air_cost );
    orig.other_cost = flatten( orig.other_cost );
    return orig;
}
