...
{
    int batch_size = 0;
    const std::pair<Character *, const recipe *> selection =
        select_crafter_and_crafting_recipe( batch_size, goto_recipe, this );
    Character *crafter = selection.first;
    const recipe *rec = selection.second;
    if( rec ) {
        std::string reason;
...
namespace
{
struct crafting_roll_data {
    double skill_dice = 0.0;
    int skill_sides = 0;
    int difficulty_dice = 0;
    int difficulty_sides = 0;
    double proficiency_malus = 1.0;
};

static crafting_roll_data crafting_roll_parameters( const Character &crafter,
        const recipe &making, bool announce_helper )
{
    ...
    const double primary_skill_level = crafter.get_skill_level_with_progress( making.skill_used ) + 1.0;
    const int primary_difficulty = making.difficulty + 1;

    float secondary_dice = 0.0f;
    int secondary_difficulty = 0;
    for( const auto &pr : making.required_skills ) {
        // Adjust skill and difficulty such that the lowest level is 1 not 0
        // This allows characters with 0 level skills to attempt higher level recipes
        secondary_dice += crafter.get_skill_level_with_progress( pr.first ) + 1.0f;
        secondary_difficulty += pr.second + 1;
    }

    // # of dice is 75% primary skill, 25% secondary (unless secondary is null)
    crafting_roll_data result;
    if( secondary_difficulty > 0 ) {
        result.skill_dice = primary_skill_level * 3 + secondary_dice;
    } else {
        result.skill_dice = primary_skill_level * 4;
    }
    // Even with no skill you should have "some" chance to complete a craft
    // this is equilavant to a quarter of a level
    if( result.skill_dice == 0 ) {
        result.skill_dice = 1;
    }

    for( const npc *np : crafter.get_crafting_helpers() ) {
        if( np->get_skill_level( making.skill_used ) >=
            crafter.get_skill_level( making.skill_used ) ) {
            // NPC assistance is worth half a skill level
            result.skill_dice += 2;
            if( announce_helper ) {
                crafter.add_msg_if_player( m_info, _( "%s helps with crafting…" ), np->get_name() );
            }
            break;
        }
    }
    ...
    if( crafter.has_flag( json_flag_HYPEROPIC ) && !crafter.worn_with_flag( flag_FIX_FARSIGHT ) &&
        ...
        result.skill_dice -= main_rank_penalty * 4;
    }
    ...
    result.skill_sides = 16 + crafter.get_int();

    if( secondary_difficulty > 0 ) {
        result.difficulty_dice = primary_difficulty * 3 + secondary_difficulty;
    } else {
        // Since skill level is * 4 also
        result.difficulty_dice = primary_difficulty * 4;
    }

    result.difficulty_sides = 24; // 16 + 8 (default intelligence)
    result.proficiency_malus = making.proficiency_failure_maluses( crafter );
    return result;
}

static std::vector<long double> dice_sum_distribution( int number, int sides )
{
    if( number <= 0 || sides <= 0 ) {
        return { 1.0L };
    }

    std::vector<long double> distribution( 1, 1.0L );
    for( int die = 0; die < number; ++die ) {
        std::vector<long double> next( distribution.size() + sides, 0.0L );
        for( size_t sum = 0; sum < distribution.size(); ++sum ) {
            const long double outcome = distribution[sum] / sides;
            for( int face = 1; face <= sides; ++face ) {
                next[sum + face] += outcome;
            }
        }
...
static std::vector<long double> fractional_dice_sum_distribution( double number, int sides )
{
    if( number <= 0.0 || sides <= 0 ) {
        return { 1.0L };
    }

    const int lower_count = static_cast<int>( std::floor( number ) );
    const long double upper_weight = number - lower_count;
    if( upper_weight == 0.0L ) {
        return dice_sum_distribution( lower_count, sides );
    }

    std::vector<long double> lower = dice_sum_distribution( lower_count, sides );
    std::vector<long double> upper = dice_sum_distribution( lower_count + 1, sides );
    lower.resize( upper.size(), 0.0L );
    for( size_t i = 0; i < upper.size(); ++i ) {
        upper[i] = lower[i] * ( 1.0L - upper_weight ) + upper[i] * upper_weight;
    }
    return upper;
}
} // namespace
...
double Character::crafting_success_roll( const recipe &making ) const
{
    ...
    const crafting_roll_data data = crafting_roll_parameters( *this, making, true );

    const double skill_roll = dice( roll_remainder( data.skill_dice ), data.skill_sides );
    const double diff_roll = dice( data.difficulty_dice, data.difficulty_sides );
    ...
    return ( skill_roll / diff_roll ) / data.proficiency_malus;
}

double Character::crafting_failure_chance( const recipe &making ) const
{
    if( has_trait( trait_DEBUG_CNF ) ) {
        return 0.0;
    }

    const crafting_roll_data data = crafting_roll_parameters( *this, making, false );
    const std::vector<long double> skill = fractional_dice_sum_distribution( data.skill_dice,
                                           data.skill_sides );
    const std::vector<long double> difficulty = dice_sum_distribution( data.difficulty_dice,
                                                data.difficulty_sides );

    std::vector<long double> skill_cumulative( skill.size(), 0.0L );
    for( size_t i = 0; i < skill.size(); ++i ) {
        skill_cumulative[i] = skill[i] + ( i == 0 ? 0.0L : skill_cumulative[i - 1] );
    }

    long double failure = 0.0L;
    for( size_t difficulty_roll = 1; difficulty_roll < difficulty.size(); ++difficulty_roll ) {
        const long double threshold = difficulty_roll * data.proficiency_malus;
        const long long largest_failing_roll = static_cast<long long>( std::ceil( threshold ) ) - 1;
        if( largest_failing_roll < 0 ) {
            continue;
        }
        const size_t index = std::min<size_t>( largest_failing_roll, skill_cumulative.size() - 1 );
        failure += difficulty[difficulty_roll] * skill_cumulative[index];
    }
    return clamp( static_cast<double>( failure ), 0.0, 1.0 );
}
    ...
    for( item &newit : newits ) {
        ...
        if( newit.made_of( phase_id::LIQUID ) ) {
            if( is_avatar() ) {
                ...
                liquid_handler::handle_npc_liquid( newit, *this );
            }
        ...
        for( item &bp : bps ) {
            ...
            if( bp.made_of( phase_id::LIQUID ) ) {
                if( is_avatar() ) {
                    ...
                    liquid_handler::handle_npc_liquid( bp, *this );
                }
            } else if( !loc ) {
...
std::vector<Character *> Character::get_crafting_group() const
{
    ...
    group.push_back( const_cast<Character *>( this ) );

    Character &avatar = get_player_character();
    const npc *self_npc = as_npc();
    if( self_npc != nullptr && getID() != avatar.getID() && self_npc->is_ally( avatar ) &&
        rl_dist( avatar.pos(), pos() ) < PICKUP_RANGE &&
        get_map().clear_path( pos(), avatar.pos(), PICKUP_RANGE, 1, 100 ) ) {
        group.push_back( &avatar );
    }

    const std::vector<npc *> allies = g->get_npcs_if( [this]( const npc & guy ) {
        return getID() != guy.getID() && guy.is_ally( *this ) &&
               rl_dist( guy.pos(), pos() ) < PICKUP_RANGE &&
               get_map().clear_path( pos(), guy.pos(), PICKUP_RANGE, 1, 100 );
    } );
    group.insert( group.end(), allies.begin(), allies.end() );
    return group;
}

static bool is_anyone_crafting( const item_location &itm, const Character *you = nullptr )
{
    const auto is_crafting = [ &itm, you ]( const Character & guy ) {
        return !( you && you->getID() == guy.getID() ) &&
               guy.activity.id() == ACT_CRAFT && !guy.activity.targets.empty() &&
               guy.activity.targets.back() == itm;
    };

    if( is_crafting( get_player_character() ) ) {
        return true;
    }
...
item_location npc::get_item_to_craft()
{
    ...
    visit_items( [ this, &to_craft ]( item * itm, item * ) {
        if( itm->get_var( "crafter", "" ) == name ) {
            item_location candidate( *this, itm );
            if( !is_anyone_crafting( candidate, this ) ) {
                to_craft = candidate;
    ...
    for( const tripoint &adj : here.points_in_radius( pos(), 1 ) ) {
        ...
        for( item &itm : here.i_at( adj ) ) {
            if( itm.get_var( "crafter", "" ) == name ) {
                item_location candidate( map_cursor( adj ), &itm );
                if( !is_anyone_crafting( candidate, this ) ) {
                    return candidate;
                }
            }
