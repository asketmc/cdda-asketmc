                            ...
                            bool debug ) const
{
    ...
    const auto activity_var = item_vars.find( "activity_var" );
    if( activity_var != item_vars.end() ) {
        info.emplace_back( "DESCRIPTION",
                           string_format( _( "* This item is currently used by %s" ),
                                          activity_var->second ) );
    }
...
{
    if( !is_gun() || ( !uses_firing_requirements() && ammo_required() && !ammo_remaining() ) ) {
        return 0;
    }
...
{
    return type != nullptr && !type->firing_requirements.empty();
}
...
item_pocket *item::pocket_by_id( const std::string &id )
{
    ...
    const std::vector<item_pocket *> matches = contents.get_pockets(
    [&id]( const item_pocket & pocket ) {
        return pocket.get_pocket_data() != nullptr &&
               pocket.get_pocket_data()->pocket_id == id;
    } );
...
const item_pocket *item::pocket_by_id( const std::string &id ) const
{
    ...
    const std::vector<const item_pocket *> matches = contents.get_pockets(
    [&id]( const item_pocket & pocket ) {
        return pocket.get_pocket_data() != nullptr &&
               pocket.get_pocket_data()->pocket_id == id;
    } );
...
{
    const item_pocket *pocket = pocket_by_id( id );
    if( pocket == nullptr ) {
        return 0;
    }
    if( pocket->is_type( item_pocket::pocket_type::MAGAZINE_WELL ) ) {
        const item *magazine = pocket->magazine_current();
        return magazine == nullptr ? 0 : magazine->ammo_remaining();
    }
    if( pocket->is_type( item_pocket::pocket_type::MAGAZINE ) ) {
        int total = 0;
        for( const item *contained : pocket->all_items_top() ) {
            if( contained != nullptr && !contained->has_flag( flag_CASING ) ) {
                total += contained->charges;
            }
        }
        return total;
    }
...
int item::ammo_consume_in_pocket( const std::string &id, int qty, const tripoint &pos )
{
    if( qty <= 0 ) {
        return 0;
    }
    item_pocket *pocket = pocket_by_id( id );
    if( pocket == nullptr ) {
        return 0;
    }
    if( pocket->is_type( item_pocket::pocket_type::MAGAZINE_WELL ) ) {
        item *magazine = pocket->magazine_current();
        return magazine == nullptr ? 0 : magazine->ammo_consume( qty, pos, nullptr );
    }
    if( pocket->is_type( item_pocket::pocket_type::MAGAZINE ) ) {
...
bool item::pocket_accepts_battery( const item_pocket *pocket ) const
{
    if( pocket == nullptr ) {
        return false;
    }
    if( pocket->ammo_types().count( ammo_battery ) != 0 ) {
        return true;
    }
    for( const itype_id &allowed : pocket->item_type_restrictions() ) {
        if( allowed->magazine && allowed->magazine->type.count( ammo_battery ) != 0 ) {
            return true;
        }
    }
    return false;
...
{
    const std::vector<const item_pocket *> ammo_pockets = contents.get_pockets(
    []( const item_pocket & pocket ) {
        return pocket.is_type( item_pocket::pocket_type::MAGAZINE ) ||
               pocket.is_type( item_pocket::pocket_type::MAGAZINE_WELL );
    } );
    if( ammo_pockets.empty() ) {
        return nullptr;
    }
    const auto matches_gun_ammo = [this]( const item_pocket * pocket ) {
        if( !type || !type->gun ) {
            return false;
        }
        for( const ammotype &ammo : pocket->ammo_types() ) {
            if( type->gun->ammo.count( ammo ) != 0 ) {
                return true;
            }
        }
        for( const itype_id &allowed : pocket->item_type_restrictions() ) {
            if( allowed->magazine ) {
                for( const ammotype &ammo : allowed->magazine->type ) {
                    if( type->gun->ammo.count( ammo ) != 0 ) {
                        return true;
                    }
                }
    ...
    const auto loaded = []( const item_pocket * pocket ) {
        return pocket->is_type( item_pocket::pocket_type::MAGAZINE_WELL ) ?
               pocket->magazine_current() != nullptr : !pocket->empty();
    };
    for( const item_pocket *pocket : ammo_pockets ) {
        if( matches_gun_ammo( pocket ) && loaded( pocket ) ) {
            return pocket;
        }
    }
    for( const item_pocket *pocket : ammo_pockets ) {
        if( matches_gun_ammo( pocket ) ) {
            return pocket;
        }
    }
    for( const item_pocket *pocket : ammo_pockets ) {
    ...
    return ammo_pockets.front();
}

int item::effective_consumption_qty( const pocket_consumption_entry &entry ) const
{
    double multiplier = 1.0;
    int modifier = 0;
    const item_pocket *primary = primary_ammo_pocket();
    if( is_gun() && pocket_by_id( entry.pocket ) == primary ) {
        for( const item *mod : gunmods() ) {
    ...
    const double result = entry.qty * multiplier + modifier;
    return result <= 0.0 ? 0 : std::max( 1, static_cast<int>( result ) );
}

static const std::vector<pocket_consumption_entry> *consumption_for_mode(
    const item &host, const gun_mode_id &mode )
{
    const std::vector<pocket_consumption_entry> *entries =
        host.type->firing_requirements.for_mode( mode );
    if( entries == nullptr ) {
        entries = host.type->firing_requirements.for_mode( gun_mode_DEFAULT );
    }
    return entries;
}

int item::feasible_shots( const gun_mode_id &mode, int requested ) const
{
    if( !uses_firing_requirements() || requested <= 0 ) {
        return uses_firing_requirements() ? 0 : requested;
    }
    const std::vector<pocket_consumption_entry> *entries = consumption_for_mode( *this, mode );
    if( entries == nullptr || entries->empty() ) {
        return 0;
    }
    std::map<std::string, int> required_by_pocket;
    for( const pocket_consumption_entry &entry : *entries ) {
        required_by_pocket[entry.pocket] += effective_consumption_qty( entry );
    }
    int possible = requested;
    for( const std::pair<const std::string, int> &requirement : required_by_pocket ) {
        if( requirement.second > 0 ) {
            possible = std::min( possible,
                                 ammo_remaining_in_pocket( requirement.first ) / requirement.second );
        }
    }
    return possible;
}

int item::feasible_tool_uses( int requested ) const
{
    return feasible_shots( gun_mode_DEFAULT, requested );
}

int item::consume_shots( const gun_mode_id &mode, int shots, const tripoint &pos,
                         Character * )
{
    const int allowed = feasible_shots( mode, shots );
    if( allowed <= 0 ) {
        return 0;
    }
    const std::vector<pocket_consumption_entry> *entries = consumption_for_mode( *this, mode );
    for( const pocket_consumption_entry &entry : *entries ) {
        const int required = effective_consumption_qty( entry ) * allowed;
        if( ammo_consume_in_pocket( entry.pocket, required, pos ) != required ) {
            debugmsg( "Unexpected multi-pocket consumption shortage on %s (%s)",
                      tname(), entry.pocket );
            return 0;
        }
    }
...
int item::consume_tool_uses( int uses, const tripoint &pos, Character *carrier )
{
    return consume_shots( gun_mode_DEFAULT, uses, pos, carrier );
}
...
bool item::ammo_sufficient( const Character *carrier, int qty ) const
{
    if( uses_firing_requirements() ) {
        return is_gun() ? feasible_shots( gun_get_mode_id(), qty ) >= qty :
               feasible_tool_uses( qty ) >= qty;
    }
    if( ammo_required() ) {
...
{
    if( uses_firing_requirements() ) {
        return consume_tool_uses( qty, pos, carrier );
    }
...
const itype *item::ammo_data() const
{
    if( uses_firing_requirements() && is_gun() ) {
        const item_pocket *primary = primary_ammo_pocket();
        if( primary != nullptr ) {
            if( primary->is_type( item_pocket::pocket_type::MAGAZINE_WELL ) ) {
                const item *magazine = primary->magazine_current();
                return magazine == nullptr ? nullptr : magazine->ammo_data();
            }
            const std::list<const item *> loaded = primary->all_items_top();
            return loaded.empty() ? nullptr : loaded.front()->ammo_data();
        }
    }
    const item *mag = magazine_current();
    if( mag ) {
...
{
    // empty() includes magazines and other non-container pockets, while
    // only_item() deliberately does not.  Use the same item-stack definition
    // as only_item() so a battery-powered empty vessel (for example a multi
    // cooker) is treated as an empty liquid container instead of calling
    // only_item() with zero eligible contents.
    if( left->num_item_stacks() == 0 ) {
        return false;
    } else if( right->num_item_stacks() == 0 ) {
        return true;
    } else {
