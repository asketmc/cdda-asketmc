...
class user_turn
{
    ...
    public:
        ...
        bool blink_timeout() {
            ...
            if( normal_input_blink_timeout_elapsed( elapsed_ms ) ) {
...
static void grab()
{
    ...
    if( const optional_vpart_position vp = here.veh_at( grabp ) ) {
        ...
        if( vp->has_loaded_furniture() ) {
            ...
            const furn_str_id furniture( loader ?
                                         loader->part().get_base().get_var( "tied_down_furniture" ) : "" );
            if( furniture.is_valid() && query_yn( _( "Grab %s on %s?" ), furniture->name(),
                                                  vp->vehicle().name ) ) {
                you.grab( object_type::FURNITURE_ON_VEHICLE, grabp - you.pos() );
                add_msg( m_info, _( "You grab the %s loaded on the %s." ), furniture->name(),
                         vp->vehicle().name );
                return;
            }
        }
