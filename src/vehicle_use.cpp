...
void vehicle::smash_security_system()
{
    ...
    if( c >= 0 && s >= 0 ) {
        ...
        const float skill = player_character.get_skill_level_with_progress( skill_mechanics );
...
void vehicle::display_effects()
{
    ...
    for( const std::pair<const efftype_id, effect> &entry : effects ) {
        if( entry.first->is_show_in_info() ) {
            effect_message += entry.second.disp_name() + ": " + entry.second.disp_desc() + "\n";
        }
    }
    if( !effect_message.empty() ) {
        add_msg( effect_message );
    }
}

static const std::string tied_down_furniture_var = "tied_down_furniture";

static void place_carried_furniture( map &here, item &base_copy )
{
    const furn_str_id tied_down_furniture( base_copy.get_var( tied_down_furniture_var ) );
    if( !tied_down_furniture.is_valid() ) {
        debugmsg( "Invalid stored furniture %s", base_copy.get_var( tied_down_furniture_var ) );
        base_copy.erase_var( tied_down_furniture_var );
        return;
    }

    creature_tracker &creatures = get_creature_tracker();
    const auto valid_furniture_destination = [&here, &creatures]( const tripoint & target ) {
        const bool has_floor = here.has_floor( target );
        const bool is_ramp_or_road = here.has_flag( ter_furn_flag::TFLAG_RAMP_DOWN, target ) ||
                                     here.has_flag( ter_furn_flag::TFLAG_RAMP_UP, target ) ||
                                     here.has_flag( ter_furn_flag::TFLAG_ROAD, target );
        if( !here.passable( target ) || creatures.creature_at<npc>( target ) != nullptr ||
            creatures.creature_at<monster>( target ) != nullptr ) {
            return false;
        }
        if( has_floor && !here.has_flag( ter_furn_flag::TFLAG_FLAT, target ) &&
            !is_ramp_or_road ) {
            return false;
        }
        return !here.has_furn( target ) && !here.veh_at( target ) &&
               ( !has_floor || here.tr_at( target ).is_null() );
    };
    const std::string fail_msg =
        _( "Can't put furniture there.  The destination is obstructed or unsuitable." );
    const cata::optional<tripoint> selected_tile = choose_adjacent_highlight(
                string_format( _( "Select where to place the %s." ), tied_down_furniture->name() ),
                fail_msg, valid_furniture_destination, false );
...
static void pickup_furniture_for_carry( map &here, vehicle_part &part, item &base_copy )
{
    const auto tile_with_furniture = [&here]( const tripoint & target ) {
        return here.has_furn( target );
    };
    const cata::optional<tripoint> selected_tile = choose_adjacent_highlight(
                _( "Select furniture to load into the vehicle." ), _( "No adjacent furniture." ),
                tile_with_furniture, false );
    if( !selected_tile ) {
        return;
    }
    ...
    if( !crane_lift && picked_up_furn->move_str_req > lifting_str_available ) {
        if( you.get_lift_assist() > 0 ) {
            add_msg( _( "Even working together, you are unable to lift the %s." ),
                     picked_up_furn->name() );
        } else {
            add_msg( _( "You aren't able to lift the %s on your own." ), picked_up_furn->name() );
        }
        return;
    }

    here.furn_clear( *selected_tile );
    base_copy.set_var( tied_down_furniture_var, picked_up_furn.str() );
}

void vehicle::use_tiedown_furniture( int part, const tripoint & )
{
    vehicle_part &vp = parts[part];
    if( vp.is_broken() || vp.removed ) {
        add_msg( _( "The %s is broken and cannot be used." ), vp.name() );
        return;
    }

    item base_copy( vp.get_base() );
    const bool will_unload_furniture = base_copy.has_var( tied_down_furniture_var );
    if( !get_items( part ).empty() && !will_unload_furniture ) {
    ...
    if( will_unload_furniture ) {
        place_carried_furniture( here, base_copy );
    } else {
        pickup_furniture_for_carry( here, vp, base_copy );
    }

    vp.set_base( base_copy );
    invalidate_mass();
}
...
void vpart_position::form_inventory( inventory &inv ) const
{
    ...
    for( const auto &entry : get_tools() ) {
        inv.provide_pseudo_item( entry.first );
    }
}

static const gun_mode_id gun_mode_DEFAULT( "DEFAULT" );

std::map<std::string, multimag_pocket_state> vehicle::prepare_multimag_pockets(
    vehicle &veh, item &tool )
{
    return prepare_multimag_pockets( veh, tool, gun_mode_DEFAULT, itype_id::NULL_ID() );
}

std::map<std::string, multimag_pocket_state> vehicle::prepare_multimag_pockets(
    vehicle &veh, item &tool, const gun_mode_id &mode, const itype_id &preferred_primary )
{
    std::map<std::string, multimag_pocket_state> result;
    ...
    if( entries == nullptr ) {
        entries = tool.type->firing_requirements.for_mode( gun_mode_DEFAULT );
    }
    if( entries == nullptr ) {
        return result;
    }

    int claimed_battery = 0;
    std::map<int, int> claimed_tank;
    for( const pocket_consumption_entry &entry : *entries ) {
        if( result.count( entry.pocket ) != 0 ) {
            continue;
        }
        item_pocket *pocket = tool.pocket_by_id( entry.pocket );
        if( pocket == nullptr || !pocket->empty() ) {
            continue;
        }
        ...
        if( tool.pocket_accepts_battery( pocket ) ) {
            const int battery_total = static_cast<int>( std::min<int64_t>(
                                          veh.fuel_left( itype_battery, true ),
                                          std::numeric_limits<int>::max() ) );
            const int available = std::max( 0, battery_total - claimed_battery );
            if( available <= 0 ) {
                continue;
            }
            bool inserted = false;
            if( data->type == item_pocket::pocket_type::MAGAZINE_WELL ) {
                itype_id magazine_type = data->default_magazine;
                if( magazine_type.is_null() && !data->item_id_restriction.empty() ) {
                    magazine_type = *data->item_id_restriction.begin();
                }
                if( !magazine_type.is_null() ) {
                    item magazine( magazine_type, calendar::turn_zero );
                    magazine.clear_items();
                    const int amount = std::min( available, magazine.ammo_capacity( ammo_battery ) );
                    if( amount > 0 ) {
                        magazine.ammo_set( itype_battery, amount );
                        inserted = pocket->insert_item( magazine ).success();
                        if( inserted ) {
                            claimed_battery += amount;
                        }
                    }
                }
            } else if( data->type == item_pocket::pocket_type::MAGAZINE ) {
                const int amount = std::min( available,
                                             pocket->remaining_ammo_capacity( ammo_battery ) );
                if( amount > 0 ) {
                    item battery( itype_battery, calendar::turn_zero, amount );
                    inserted = pocket->insert_item( battery ).success();
                    if( inserted ) {
                        claimed_battery += amount;
                    }
                }
            ...
            if( inserted ) {
                multimag_pocket_state state;
                state.kind = multimag_pocket_state::source_kind::BATTERY;
                state.initial_qty = tool.ammo_remaining_in_pocket( entry.pocket );
                result.emplace( entry.pocket, state );
            }
            continue;
        }

        if( data->type != item_pocket::pocket_type::MAGAZINE ||
            data->ammo_restriction.empty() ) {
            continue;
        }
        bool inserted = false;
        for( int pass = 0; pass < 2 && !inserted; ++pass ) {
            if( pass == 0 && preferred_primary.is_null() ) {
                continue;
            }
            const bool preference_only = pass == 0;
            for( const std::pair<const ammotype, int> &restriction : data->ammo_restriction ) {
                for( const vpart_reference &tank_ref : veh.get_avail_parts( VPFLAG_FLUIDTANK ) ) {
                    const itype_id tank_ammo = tank_ref.part().ammo_current();
                    if( tank_ammo.is_null() || !tank_ammo->ammo ||
                        tank_ammo->ammo->type != restriction.first ||
                        ( preference_only && tank_ammo != preferred_primary ) ) {
                        continue;
                    }
                    const int tank_index = static_cast<int>( tank_ref.part_index() );
                    const int available = std::max( 0, tank_ref.part().ammo_remaining() -
                                                    claimed_tank[tank_index] );
                    const int amount = std::min( available, restriction.second );
                    if( amount <= 0 ) {
                        continue;
                    }
                    item ammo( tank_ammo, calendar::turn_zero, amount );
                    if( pocket->insert_item( ammo ).success() ) {
                        claimed_tank[tank_index] += amount;
                        multimag_pocket_state state;
                        state.kind = multimag_pocket_state::source_kind::TANK;
                        state.vpart_index = tank_index;
                        state.initial_qty = tool.ammo_remaining_in_pocket( entry.pocket );
                        result.emplace( entry.pocket, state );
                        inserted = true;
                        break;
...
void vehicle::drain_back_multimag( vehicle &veh, const item &tool,
                                   const std::map<std::string, multimag_pocket_state> &bindings )
{
    for( const std::pair<const std::string, multimag_pocket_state> &binding : bindings ) {
        const int used = binding.second.initial_qty -
                         tool.ammo_remaining_in_pocket( binding.first );
        if( used <= 0 ) {
            continue;
        }
        if( binding.second.kind == multimag_pocket_state::source_kind::BATTERY ) {
            veh.discharge_battery( used );
        } else if( binding.second.vpart_index >= 0 ) {
            veh.drain( binding.second.vpart_index, used );
        }
    }
}

std::pair<itype_id, int> vehicle::tool_ammo_available( const itype_id &tool_type ) const
{
    if( !tool_type.is_valid() || !tool_type->tool ) {
        return { itype_id::NULL_ID(), 0 };
    }
    const itype_id ammo_type = tool_type->tool_slot_first_ammo();
    if( ammo_type.is_null() || !ammo_type->ammo ) {
        return { itype_id::NULL_ID(), 0 };
    }
    const int64_t amount = ammo_type->ammo->type == ammo_battery
                           ? fuel_left( itype_battery, true )
                           : fuel_left( ammo_type );
    return { ammo_type, static_cast<int>( std::min<int64_t>(
...
int vehicle::prepare_tool( item &tool ) const
{
    ...
    if( tool.uses_firing_requirements() ) {
        prepare_multimag_pockets( *const_cast<vehicle *>( this ), tool );
        return tool.ammo_remaining();
    }

    const std::pair<itype_id, int> source = tool_ammo_available( tool.typeId() );
    if( source.first.is_null() ) {
        return 0;
    }

    // This is a disposable working copy.  Remove its real magazine/ammo before
    // the pseudo adapter changes pockets, so item_contents never deletes loaded
    // contents (and the stored attached tool remains untouched).
    if( item *loaded_magazine = tool.magazine_current() ) {
        tool.remove_item( *loaded_magazine );
    }
    tool.ammo_unset();

    item magazine_mod( "pseudo_magazine_mod", calendar::turn_zero );
    magazine_mod.set_flag( STATIC( flag_id( "IRREMOVABLE" ) ) );
    if( !tool.put_in( magazine_mod, item_pocket::pocket_type::MOD ).success() ) {
        return 0;
    }

    item magazine( tool.magazine_default(), calendar::turn_zero );
    if( !magazine.is_null() ) {
        magazine.clear_items();
        if( !tool.put_in( magazine, item_pocket::pocket_type::MAGAZINE_WELL ).success() ) {
            return 0;
        }
    }

    const ammotype ammo_category = source.first->ammo->type;
    const int amount = std::min( source.second, tool.ammo_capacity( ammo_category ) );
    if( amount > 0 ) {
        tool.ammo_set( source.first, amount );
    }
    return amount;
}
                                ...
                                bool no_invoke )
{
    ...
    if( pseudo.uses_firing_requirements() ) {
        const std::map<std::string, multimag_pocket_state> bindings =
            vehicle::prepare_multimag_pockets( veh, pseudo );
        if( pseudo.feasible_tool_uses() < 1 ) {
            return false;
        }
        if( !no_invoke ) {
            get_player_character().invoke_item( &pseudo, vp_pos );
        }
        vehicle::drain_back_multimag( veh, pseudo, bindings );
        return true;
    }

    const std::pair<itype_id, int> source = veh.tool_ammo_available( tool_type );
    const int starting_ammo = veh.prepare_tool( pseudo );
    int required_ammo = pseudo.ammo_required();
    if( no_invoke ) {
        // The only deferred vehicle-tool bridge in 0.G is HACKSAW.  Respect its
        // per-method charge multiplier (repair kits deliberately use zero charges).
        const auto scale = pseudo.type->ammo_scale.find( "HACKSAW" );
        if( scale != pseudo.type->ammo_scale.end() ) {
            required_ammo *= scale->second;
        }
    }
    if( required_ammo > starting_ammo ) {
        return false;
    }

    if( no_invoke ) {
        if( required_ammo > 0 && !source.first.is_null() ) {
            veh.drain( source.first, required_ammo );
        }
        return true;
    }
    ...
    if( act.id() == ACT_REPAIR_ITEM &&
        ( tool_type == itype_welder || tool_type == itype_welder_crude ||
          tool_type == itype_welding_kit || tool_type == itype_soldering_iron ||
          tool_type == itype_small_repairkit || tool_type == itype_large_repairkit ) ) {
    ...
    const int used = starting_ammo - pseudo.ammo_remaining();
    if( used > 0 && !source.first.is_null() ) {
        veh.drain( source.first, used );
    }
    return true;
}
...
void vehicle::build_interact_menu( veh_menu &menu, const tripoint &p, bool with_pickup )
{
    ...
    if( is_locked && controls_here ) {
        ...
        .on_submit( [this] {
            ...
            const float skill = std::max(
                                    1.0f, get_player_character().get_skill_level_with_progress( skill_mechanics ) );
    ...
    for( const auto &entry : vp.get_tools() ) {
        const item &prepared_tool = entry.first;
        const input_event &hotkey = entry.second;
        const itype_id tool = prepared_tool.typeId();
        if( ( !hotkey.sequence.empty() && hotkey.sequence.front() == -1 ) || !tool->has_use() ) {
            continue; // passive tool
        }

        const bool enabled = prepared_tool.uses_firing_requirements()
                             ? prepared_tool.feasible_tool_uses() >= 1
                             : prepared_tool.ammo_required() == 0 ||
                             prepared_tool.ammo_remaining() >= prepared_tool.ammo_required();
    ...
    const cata::optional<vpart_reference> vp_furniture_tiedown =
        vp.avail_part_with_feature( "FURNITURE_TIEDOWN" );
    if( vp_furniture_tiedown ) {
        const size_t tiedown_idx = vp_furniture_tiedown->part_index();
        menu.add( _( "Tie down or remove furniture" ) )
        .hotkey( "USE_TIEDOWN_FURNITURE_VEH" )
        .on_submit( [this, tiedown_idx, vppos] {
            use_tiedown_furniture( tiedown_idx, vppos );
        } );
    }
    ...
    const cata::optional<vpart_reference> vp_toolstation =
        vp.avail_part_with_feature( "VEH_TOOLS" );
    if( vp_toolstation && vp_toolstation->info().get_toolkit_info() ) {
        const size_t toolstation_idx = vp_toolstation->part_index();
        const std::string toolstation_name = vp_toolstation->part().name( false );

        menu.add( string_format( _( "Attach a tool to %s" ), toolstation_name ) )
        .skip_locked_check()
        .on_submit( [this, toolstation_idx, toolstation_name] {
            Character &you = get_player_character();
            vehicle_part &station = part( toolstation_idx );
            std::set<itype_id> allowed = station.info().get_toolkit_info()->allowed_types;
            for( const auto &entry : prepare_tools( station ) ) {
                allowed.erase( entry.first.typeId() );
            }

            item_location selected = game_menus::inv::veh_tool_attach( you, toolstation_name, allowed );
            if( !selected ) {
                return;
            }

            item &selected_tool = *selected.get_item();
            you.add_msg_if_player( _( "You attach %s to %s." ),
                                   selected_tool.tname(), toolstation_name );
            station.tools.emplace_back( selected_tool );
            selected.remove_item();
            invalidate_mass();
            you.invalidate_crafting_inventory();
        } );

        const bool can_detach = !get_tools( part( toolstation_idx ) ).empty();
        menu.add( string_format( _( "Detach a tool from %s" ), toolstation_name ) )
        .enable( can_detach )
        .desc( can_detach ? std::string() :
               string_format( _( "There are no tools attached to %s." ), toolstation_name ) )
        .skip_locked_check()
        .on_submit( [this, toolstation_idx, toolstation_name] {
            veh_menu detach_menu( this,
                                  string_format( _( "Detach a tool from %s" ), toolstation_name ) );
            vehicle_part &station = part( toolstation_idx );
            for( item &attached : get_tools( station ) ) {
                detach_menu.add( attached.display_name() )
                .skip_locked_check()
                .skip_theft_check()
                .on_submit( [this, &station, &attached, toolstation_name] {
                    Character &you = get_player_character();
                    std::vector<item> &tools = get_tools( station );
                    const auto iter = std::find_if( tools.begin(), tools.end(),
                    [&attached]( const item &candidate ) {
                        return &candidate == &attached;
                    } );
                    if( iter == tools.end() ) {
                        return;
                    }
                    you.add_msg_if_player( _( "You detach %s from %s." ),
                                           iter->tname(), toolstation_name );
                    you.add_or_drop_with_msg( *iter );
                    tools.erase( iter );
                    invalidate_mass();
                    you.invalidate_crafting_inventory();
                } );
