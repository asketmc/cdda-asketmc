...
dialogue_data::query_tile_selector_for_test &dialogue_data::query_tile_selector_for_tests()
{
    static query_tile_selector_for_test selector;
    return selector;
}
...
template<class T>
cata::optional<tripoint_abs_ms> get_tripoint_from_var_checked( cata::optional<var_info> var,
        const T &d )
{
    if( !var.has_value() ) {
        return get_map().getglobal( d.actor( false )->pos() );
    }
    const std::string value = read_var_value<T>( var.value(), d );
    if( value.empty() ) {
        debugmsg( "Explicit EOC location variable '%s' is empty; effect failed closed.",
                  var->name );
        return cata::nullopt;
    }
    ...
    input >> std::ws;
    if( !input || !input.eof() ) {
        debugmsg( "Could not convert EOC location '%s' to tripoint; effect failed closed.", value );
        return cata::nullopt;
    }
    return tripoint_abs_ms( parsed );
}
...
std::string read_context_value( const dialogue &d, const std::string &name )
{
    return d.get_value( name );
}

const dialogue *get_dialogue_context( const dialogue &d )
{
    return &d;
}
...
{
    var_type scope = var_type::global;
    ...
    return var_info( scope, "npctalk_var_" + name );
}

static void write_var_value_internal( var_type type, const std::string &name, talker *talk,
                                      const dialogue *d, const std::string &value,
                                      std::unordered_set<std::string> &indirect_vars )
{
    ...
    switch( type ) {
        ...
        case var_type::context:
            if( d ) {
                ...
                debugmsg( "Context variable %s used outside a dialogue.", name );
            }
            break;
        case var_type::var:
            if( d ) {
                if( indirect_vars.size() >= 100 ) {
                    debugmsg( "Indirect variable chain exceeds 100 references at %s.", name );
                    break;
                }
                if( !indirect_vars.insert( name ).second ) {
                    debugmsg( "Indirect variable cycle detected at %s.", name );
                    break;
                }
                const std::string referenced_var = d->get_value( name );
                if( referenced_var.empty() ) {
                    debugmsg( "Indirect variable %s has no context value.", name );
                    indirect_vars.erase( name );
                    break;
                }
                const var_info resolved = process_variable( referenced_var );
                talker *resolved_talker = talk;
                if( resolved.type == var_type::u || resolved.type == var_type::npc ) {
                    resolved_talker = d->actor( resolved.type == var_type::npc );
                }
                write_var_value_internal( resolved.type, resolved.name, resolved_talker, d, value,
                                          indirect_vars );
                indirect_vars.erase( name );
            } else {
                debugmsg( "Indirect variable %s used outside a dialogue.", name );
            }
            break;
        case var_type::u:
        case var_type::npc:
...
{
    std::unordered_set<std::string> indirect_vars;
    write_var_value_internal( type, name, talk, d, value, indirect_vars );
}
...
template<class T>
void conditional_t<T>::set_query_tile( const JsonObject &jo, const std::string &member,
                                       bool is_npc )
{
    const std::string selection_type = jo.get_string( member );
    if( selection_type != "anywhere" && selection_type != "line_of_sight" &&
        selection_type != "around" ) {
        jo.throw_error( member + " must be \"anywhere\", \"line_of_sight\", or \"around\"" );
    }
    if( selection_type == "line_of_sight" && !jo.has_member( "range" ) ) {
        jo.throw_error( member + " line_of_sight selection requires range" );
    }
    const var_info target_var = read_var_info( jo.get_object( "target_var" ) );
    translation message;
    optional( jo, false, "message", message );
    const int_or_var<T> range = get_int_or_var<T>( jo, "range", false, 0 );
    const bool z_level = jo.get_bool( "z_level", false );

    condition = [selection_type, target_var, message, range, z_level, is_npc]( const T & d ) {
        const dialogue *dialogue_context = get_dialogue_context( d );
        talker *actor = d.actor( is_npc );
        Character *character = actor ? actor->get_character() : nullptr;
        avatar *you = character ? character->as_avatar() : nullptr;
        if( !dialogue_context || !you ) {
            return false;
        }

        const std::string translated_message = message.empty() ? "" : message.translated();
        const int evaluated_range = range.evaluate( d );
        cata::optional<tripoint> location;
        dialogue_data::query_tile_selector_for_test &test_selector =
            dialogue_data::query_tile_selector_for_tests();
        if( test_selector ) {
            location = test_selector( selection_type, *you, actor->pos(), translated_message,
                                      evaluated_range, z_level );
        } else if( selection_type == "anywhere" ) {
            if( !translated_message.empty() ) {
                static_popup popup;
                popup.on_top( true ).message( "%s", translated_message );
                tripoint center = actor->pos();
                const look_around_params params = { true, center, center, false, true, true, z_level };
                location = g->look_around( params ).position;
            } else {
                tripoint center = actor->pos();
                const look_around_params params = { true, center, center, false, true, true, z_level };
                location = g->look_around( params ).position;
            }
        } else if( selection_type == "line_of_sight" ) {
            if( !translated_message.empty() ) {
                static_popup popup;
                popup.on_top( true ).message( "%s", translated_message );
                const target_handler::trajectory trajectory =
                    target_handler::mode_select_only( *you, evaluated_range );
                if( !trajectory.empty() ) {
                    location = trajectory.back();
                }
            } else {
                const target_handler::trajectory trajectory =
                    target_handler::mode_select_only( *you, evaluated_range );
        ...
        } else if( selection_type == "around" ) {
            location = choose_adjacent( translated_message.empty() ? _( "Choose direction" ) :
                                        translated_message );
        } else {
            debugmsg( "Invalid tile selection type: %s", selection_type );
        }

        if( !location ) {
            return false;
        }
        const tripoint_abs_ms absolute_location = get_map().getglobal( *location );
        write_var_value( target_var.type, target_var.name,
                         d.actor( target_var.type == var_type::npc ), dialogue_context,
                         absolute_location.to_string() );
        return true;
    };
}
...
template<class T>
void conditional_t<T>::set_map_ter_furn_with_flag( const JsonObject &jo,
        const std::string &member )
{
    str_or_var<T> flag = get_str_or_var<T>( jo.get_member( member ), member, true );
    ...
    const bool terrain = member == "map_terrain_with_flag";
    condition = [terrain, flag, loc_var]( const T & d ) {
        map &here = get_map();
        const cata::optional<tripoint_abs_ms> target =
            get_tripoint_from_var_checked<T>( loc_var, d );
        if( !target ) {
            return false;
        }
        if( here.inbounds( *target ) ) {
            const tripoint loc = here.getlocal( *target );
            return terrain ? here.ter( loc )->has_flag( flag.evaluate( d ) ) :
                   here.furn( loc )->has_flag( flag.evaluate( d ) );
        }

        tripoint_abs_sm target_submap;
        point_sm_ms loc;
        std::tie( target_submap, loc ) = project_remain<coords::sm>( *target );
        const submap *sm = MAPBUFFER.lookup_submap( target_submap );
        if( sm == nullptr ) {
            return false;
        }
        return terrain ? sm->get_ter( loc.raw() )->has_flag( flag.evaluate( d ) ) :
               sm->get_furn( loc.raw() )->has_flag( flag.evaluate( d ) );
    };
}
...
template<class T>
void conditional_t<T>::set_get_condition( const JsonObject &jo, const std::string &member )
{
    str_or_var<T> conditional_to_get = get_str_or_var<T>( jo.get_member( member ), member, true );
    condition = [conditional_to_get]( const T & d ) {
        return evaluate_stored_condition( d, conditional_to_get.evaluate( d ) );
    };
}
    ...
    if( jo.has_member( "const" ) ) {
        ...
        if( weather_aspect == "temperature" ) {
            ...
            return []( const T & ) {
               ...
               jo.has_member( "var_val" ) ) {
        const bool is_npc = jo.has_member( "npc_val" );
        const std::string checked_value = is_npc ? jo.get_string( "npc_val" ) :
                                          jo.has_member( "global_val" ) ? jo.get_string( "global_val" ) :
                                          jo.has_member( "context_val" ) ? jo.get_string( "context_val" ) :
                                          jo.has_member( "var_val" ) ? jo.get_string( "var_val" ) : jo.get_string( "u_val" );
    ...
    if( temp_var ) {
        ...
        return [min, max]( const T & d, int input ) {
            write_var_value( var_type::global, "temp_var", d.actor( false ),
                             get_dialogue_context( d ), std::to_string( handle_min_max<T>( d,
                                     input, min,
                                     max ) ) );
        };
        ...
        if( weather_aspect == "temperature" ) {
            ...
            return [min, max]( const T & d, int input ) {
               ...
               jo.has_member( "global_val" ) || jo.has_member( "context_val" ) ||
               jo.has_member( "var_val" ) || jo.has_member( "faction_val" ) ||
               jo.has_member( "party_val" ) ) {
        ...
        if( checked_value == "strength_base" ) {
            ...
            return [is_npc, var_name, type, min, max]( const T & d, int input ) {
                write_var_value( type, var_name, d.actor( is_npc ), get_dialogue_context( d ),
                                 std::to_string( handle_min_max<T>( d, input, min, max ) ) );
            };
        } else if( checked_value == "time_since_var" ) {
...
void conditional_t<T>::set_u_has_camp()
{
    condition = []( const T & ) {
        ...
        faction *your_faction = you.get_faction();
        if( your_faction == nullptr ) {
            return false;
        }
        for( const tripoint_abs_omt &camp_pos : you.camps ) {
            cata::optional<basecamp *> camp = overmap_buffer.find_camp( camp_pos.xy() );
            if( camp && ( *camp )->get_owner() == your_faction->id ) {
                return true;
            }
        }
        return false;
...
conditional_t<T>::conditional_t( const JsonObject &jo )
{
    ...
    } else if( jo.has_string( "u_query_tile" ) ) {
        set_query_tile( jo, "u_query_tile" );
    } else if( jo.has_string( "npc_query_tile" ) ) {
        set_query_tile( jo, "npc_query_tile", is_npc );
