        ...
        const std::array<advanced_inv_area, NUM_AIM_LOCATIONS> &squares, bool is_re_enter )
{
    int i_location = ( get_option<bool>( "OPEN_DEFAULT_ADV_INV" ) &&
                     ...
                     save_state->area_idx;
    if( i_location < AIM_INVENTORY || i_location >= NUM_AIM_LOCATIONS ||
        i_location == AIM_PARENT ) {
        i_location = AIM_ALL;
    }
    const aim_location location = load_container_settings( static_cast<aim_location>( i_location ) );
...
aim_location advanced_inventory_pane::load_container_settings( aim_location requested_area )
{
    const int saved_container_base = save_state->container_base_loc;
    const bool valid_base = saved_container_base >= AIM_INVENTORY &&
                            saved_container_base < NUM_AIM_LOCATIONS &&
                            saved_container_base != AIM_CONTAINER &&
                            saved_container_base != AIM_PARENT;
    container = requested_area == AIM_CONTAINER ? save_state->container : item_location::nowhere;
    if( requested_area == AIM_CONTAINER && container && container->is_container() && valid_base ) {
        container_base_loc = static_cast<aim_location>( saved_container_base );
    } else {
        if( requested_area == AIM_CONTAINER ) {
            requested_area = valid_base ? static_cast<aim_location>( saved_container_base ) : AIM_ALL;
        }
        container = item_location::nowhere;
        container_base_loc = NUM_AIM_LOCATIONS;
    }
...
{
    if( get_area() == AIM_CONTAINER ) {
        return container ? container->get_remaining_capacity() : 0_ml;
    }
