...
{
    // Get settings before checking engine_on: a controller managing one combustion engine is
    // allowed to stop that engine while parked and start it again when the battery gets low.
    ...
    for( int i = 0; i < static_cast<int>( engines.size() ); ++i ) {
        ...
        if( ( is_electric || is_combustion_engine_type( i ) ) &&
            ...
            has_electric_engine = has_electric_engine || is_electric;
        }
    }
    ...
    // One combustion engine is useful as a stationary generator.  A lone electric motor is not.
    if( rotorcraft || c_engines.empty() || ( has_electric_engine && c_engines.size() == 1 ) ||
        ...
        if( player_in_control( player_character ) ) {
            if( rotorcraft ) {
                ...
                add_msg( _( "Smart controller cannot detect a controllable engine." ) );
            } else if( c_engines.size() == 1 ) {
                add_msg( _( "Smart controller detects only a single electric engine." ) );
                add_msg( _( "An electric engine does not need fuel optimization." ) );
            } else {
    ...
    // Without an electric traction motor, a lone combustion engine acts as an automatic
    // generator: stop it when parked with a sufficiently full battery, restart it at the low mark.
    if( c_engines.size() == 1 && !has_electric_engine ) {
        if( !discharge_forbidden_soft && is_stationary && engine_on && !autopilot_on &&
            ...
            const int generator = c_engines.front();
            // stop_engines() intentionally leaves the selected engine enabled.  A newly
            // installed engine is normally enabled too, so use the direct starter path in
            // that state and the state-transition overload only for a disabled engine.
            const bool started = is_part_on( engines[generator] )
                                 ? start_engine( generator )
                                 : start_engine( generator, true );
            if( started ) {
                engine_on = true;
                ...
                smart_controller_state = cata::nullopt;
                if( player_in_control( player_character ) ) {
                    add_msg( m_bad, _( "Smart controller failed to start the generator engine." ) );
                    add_msg( m_bad, _( "Smart controller is shutting down." ) );
                }
                return;
            }
        ...
        // There is no engine combination to optimize in the single-engine case.  Returning
        // also prevents the generic mask loop from undoing the start/stop decision above.
                         ...
                         bool just_detect, bool bash_floor )
{
    ...
    for( int p = 0; p < num_parts(); p++ ) {
        ...
        if( !parts.at( p ).is_fake &&
            info.location != vpart_location_structure && info.rotor_diameter() == 0 ) {
            continue;
        }
        empty = false;
    ...
    for( const rider_data &r : riders ) {
        ...
        if( psg ) {
            ...
            if( veh.part( ps ).info().has_flag( "SEAT_REQUIRES_BALANCE" ) ) {
                // Skateboard-like vehicles are much harder to stay on in a collision.
