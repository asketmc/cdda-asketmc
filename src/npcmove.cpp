...
{
    return vitamin_iron->severity( who.vitamin_get( vitamin_iron ) ) > 0 ||
           vitamin_vitC->severity( who.vitamin_get( vitamin_vitC ) ) > 0;
}
...
static bool wants_to_reload( const npc &who, const item &candidate )
{
    if( !candidate.is_reloadable() || !who.can_reload( candidate ) ) {
        return false;
    }
    ...
    // select_ammo() performs the final capacity and compatible-ammunition checks.
    // In particular, empty magazines have no ammo_data(), so they must reach it.
    return true;
}
...
npc_action npc::address_needs( float danger )
{
    ...
    if( get_thirst() > 80 ||
        ...
        if( activity && ( activity.id() == ACT_FORAGE || activity.id() == ACT_HARVEST ) ) {
...
static int needed_vitamins( const npc &who, const item &it )
{
    const nutrients nutrients = who.compute_effective_nutrients( it );
    int result = 0;
    const std::array<vitamin_id, 2> deficiency_vitamins = { vitamin_iron, vitamin_vitC };
    for( const vitamin_id &vitamin : deficiency_vitamins ) {
        if( vitamin.obj().severity( who.vitamin_get( vitamin ) ) <= 0 ) {
            continue;
        }
        const auto food_vitamin = nutrients.vitamins.find( vitamin );
        if( food_vitamin != nutrients.vitamins.end() ) {
            result += std::max( 0, food_vitamin->second );
        }
        for( const auto &method : it.type->use_methods ) {
            ...
            const auto drug_vitamin = drug->vitamins.find( vitamin );
            if( drug_vitamin != drug->vitamins.end() ) {
                const int average_rda = ( drug_vitamin->second.first +
                                          drug_vitamin->second.second ) / 2;
                result += std::max( 0, static_cast<int>( vitamin->RDA_to_default( average_rda ) ) );
            }
        }
    }
    return result;
...
static float rate_food( const npc &who, const item &it, int want_nutr, int want_quench )
{
    ...
    const int useful_vitamins = needed_vitamins( who, it );
    if( nutr <= 0 && quench <= 0 && useful_vitamins <= 0 ) {
        // Not food - may be salt, drugs etc.
        return 0.0f;
    }

    if( !it.type->use_methods.empty() &&
        ( useful_vitamins <= 0 || !safe_vitamin_use_methods( it ) ) ) {
    ...
    float weight = useful_vitamins > 0 ?
                   10.0f + std::min( 10.0f, useful_vitamins / 10.0f ) :
                   std::max( 1.0, 10.0 * relative_rot );
    ...
    if( who.has_effect( effect_nausea ) && it.is_food() ) {
        weight /= 10.0f;
    }
...
{
    Character &player_character = get_player_character();
    std::vector<basecamp *> nearby_camps;
    for( const tripoint_abs_omt &camp_pos : player_character.camps ) {
        if( rl_dist( camp_pos, global_omt_location() ) >= 3 ) {
            continue;
        }
        const overmap_with_local_coords om_loc = overmap_buffer.get_existing_om_global( camp_pos );
        if( !om_loc ) {
            continue;
        }
        const cata::optional<basecamp *> camp = om_loc.om->find_camp( camp_pos.xy() );
        if( camp ) {
            nearby_camps.push_back( *camp );
        }
    }
    if( nearby_camps.empty() ) {
        return false;
    }
    ...
    if( get_hunger() > 0 && current_kcals < kcal_threshold ) {
        ...
        bool found_accessible_larder = false;
        for( basecamp *bcp : nearby_camps ) {
            if( !bcp->allowed_access_by( *this ) ) {
                continue;
            }
            faction *camp_faction = bcp->get_owner_faction();
            if( camp_faction == nullptr ) {
                continue;
            }
            found_accessible_larder = true;
            int kcals_to_eat = std::min( desired_kcals, camp_faction->food_supply.kcal() );
            if( kcals_to_eat <= 0 ) {
                continue;
            }
        ...
        if( found_accessible_larder ) {
            // We need food but none of the nearby accessible camps have any to eat.
...
bool npc::consume_food()
{
    ...
    const auto inv_food = items_with( []( const item & itm ) {
        return itm.is_food() || itm.is_medication();
    } );
        ...
        if( map_items_accessible ) {
            for( item &candidate : here.i_at( p ) ) {
                if( here.has_flag( ter_furn_flag::TFLAG_LIQUIDCONT, p ) &&
                    ...
                    ( candidate.typeId() != itype_water_clean || candidate.poison > 0 ||
                      here.has_flag( ter_furn_flag::TFLAG_MURKY, p ) ||
...
bool npc::move_to_local_resource( const tripoint &target )
{
    ...
    std::vector<tripoint> destinations = closest_points_first( target, 1 );
    std::sort( destinations.begin(), destinations.end(), [this]( const tripoint &lhs,
    const tripoint &rhs ) {
        return rl_dist( pos(), lhs ) < rl_dist( pos(), rhs );
    } );
    map &here = get_map();
    for( const tripoint &destination : destinations ) {
        if( !here.passable( destination ) || destination == pos() ) {
            continue;
        }
        update_path( destination, true );
        if( path.empty() ) {
            continue;
        }
...
{
    if( !is_player_ally() || get_thirst() <= 0 ) {
        return false;
    }
    for( const tripoint &target : find_local_clean_water() ) {
        if( local_resource_adjacent( *this, target ) ) {
            ...
            if( water.typeId() == itype_water_clean && water.poison <= 0 &&
                ...
                const time_duration consume_time = get_consume_time( water );
                if( consume( water ) != trinary::NONE ) {
                    moves -= to_moves<int>( consume_time );
                    return true;
                }
            }
    ...
    for( const tripoint &target : find_local_harvest() ) {
        if( local_resource_adjacent( *this, target ) ) {
            ...
            if( here.ter( target ).obj().has_examine( iexamine::shrub_wildveggies ) ) {
                ...
                move_cost /= rng( std::max( 4, per_cur ), 4 + per_cur * 2 );
                assign_activity( player_activity( forage_activity_actor( move_cost ) ) );
                activity.placement = here.getglobal( target );
                activity.auto_resume = true;
            } else {
                assign_activity( player_activity( harvest_activity_actor( target, true ) ) );
            }
