...
cata::optional<int> iuse::crowbar( Character *p, item *it, bool, const tripoint &pos )
{
    ...
    if( it->has_flag( flag_PSEUDO ) ) {
        p->add_msg_if_player( m_info,
                              _( "A fixed or virtual tool can't be used for prying." ) );
        return cata::nullopt;
    }
...
cata::optional<int> iuse::hacksaw( Character *p, item *it, bool t, const tripoint &it_pos )
{
    if( !p || t ) {
        return cata::nullopt;
    }
    ...
    if( !it->has_flag( flag_PSEUDO ) ) {
        ...
        // Vehicle-mounted tools are temporary pseudo items.  Keep stable vehicle/type
        // identifiers in the activity instead of an item_location to a stack object.
        p->assign_activity(
            player_activity( hacksaw_activity_actor( pnt, it->typeId(), it_pos ) ) );
    }
...
cata::optional<int> iuse::tow_attach( Character *p, item *it, bool, const tripoint & )
{
    ...
    if( initial_state == "attach_first" ) {
        ...
        if( !target_vp ) {
            ...
            const ret_val<void> source_mount = source_veh->can_mount( vcoords, vpid, true );
            if( !source_mount.success() ) {
                p->add_msg_if_player( m_bad, _( "You can't attach the %1$s: %2$s" ),
                                      it->type_name(), source_mount.str() );
                return cata::nullopt;
            }
            vehicle_part source_part( vpid, "", vcoords, item( *it ) );
            vcoords = target_vp->mount();
            const ret_val<void> target_mount = target_veh->can_mount( vcoords, vpid, true );
            if( !target_mount.success() ) {
                p->add_msg_if_player( m_bad, _( "You can't attach the %1$s: %2$s" ),
                                      it->type_name(), target_mount.str() );
                return cata::nullopt;
            }
            source_veh->install_part( source_vp->mount(), source_part );
...
cata::optional<int> iuse::wash_items( Character *p, bool soft_items, bool hard_items )
{
    ...
    auto make_raw_stats = [available_water,
        ...
        const inventory_selector::stat water_stat = display_stat( "", required.water, available_water,
                to_string );
        const inventory_selector::stat cleanser_stat = display_stat( "", required.cleanser,
                available_cleanser, to_string );
        const std::string water = water_stat[1] + water_stat[2] + water_stat[3];
        const std::string cleanser = cleanser_stat[1] + cleanser_stat[2] + cleanser_stat[3];
...
cata::optional<int> iuse::ebooksave( Character *p, item *it, bool t, const tripoint & )
{
    ...
    p->assign_activity( player_activity( ebooksave_activity_actor( books, ereader ) ) );
    return cata::nullopt;
}
