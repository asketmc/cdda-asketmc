...
void iexamine::locked_object( Character &you, const tripoint &examp )
{
    ...
    // Vehicle doors have no terrain/furniture prying data, and the crowbar
    // activity cannot alter vehicle lock state.  Keep their interaction to
    // the supported lockpicking path.
    const bool can_pry = has_prying && locked_part < 0;
    ...
    if( can_pry && can_pick ) {
        ...
        action = can_pry ? act::pry : can_pick ? act::pick : act::none;
    }

    if( action == act::none ) {
        if( locked_part >= 0 ) {
            add_msg( m_info, _( "The %s is locked.  You could pick the lock with the right tool…" ),
                    target_name );
        } else {
...
void iexamine::bulletin_board( Character &you, const tripoint &examp )
{
    ...
    if( bcp ) {
        ...
        if( !temp_camp->allowed_access_by( you ) ) {
            you.add_msg_if_player( _( "You don't run this camp, so the board is useless to you." ) );
            return;
        }
...
bool iexamine_helper::climb_down_supported_ledge( Character &you, const tripoint &examp,
        const int height, const float weary_mult, const float fall_mod )
{
    if( height <= 0 || !you.can_climb_down_safely( examp, height ) ) {
        return false;
    }
...
void iexamine::ledge( Character &you, const tripoint &examp )
{
    ...
    switch( cmenu.ret ) {
        ...
        case 2: {
            ...
            if( !iexamine_helper::climb_down_supported_ledge(
                    you, examp, height, weary_mult, fall_mod ) ) {
                you.moves -= to_moves<int>( 1_seconds + 1_seconds * fall_mod ) * weary_mult;
                you.setpos( examp );
