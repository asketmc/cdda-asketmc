...
struct effect_on_condition {
    public:
        ...
        event_type required_event = event_type::num_event_types;
...
class eoc_events : public event_subscriber
{
    public:
        ...
        void notify( const cata::event &e, std::unique_ptr<talker> alpha,
                     std::unique_ptr<talker> beta ) override;
        void clear();

    private:
        std::map<event_type, std::vector<effect_on_condition_id>> event_EOCs;
        bool has_cached = false;
        std::size_t cached_generation = 0;
};
