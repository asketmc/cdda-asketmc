                              ...
                              vehicle &veh, int part )
{
    ...
    for( item it : items ) {
        ...
        if( veh.add_item( part, it ) ) {
            ...
            c.add_msg_if_player( m_warning, _( "Unable to fit %1$s in the %2$s's %3$s." ),
                                 it.tname(), veh.name, part_name );
            // Keep overflow with the character when practical.  Wield before using the ground as
            // the final fallback so a full cargo tile does not silently scatter important items.
            if( c.can_pickVolume( it ) &&
                c.can_pickWeight( it, !get_option<bool>( "DANGEROUS_PICKUPS" ) ) ) {
                c.i_add( it );
            } else if( !c.has_wield_conflicts( it ) && c.can_wield( it ).success() ) {
                c.wield( it );
            } else {
                const std::string ter_name = here.name( where );
                c.add_msg_if_player( m_warning, _( "The %1$s falls to the %2$s." ),
                                     it.tname(), ter_name );
        ...
        switch( reason ) {
            case item_drop_reason::deliberate:
                if( items_did_not_fit_count == 0 ) {
                    ...
                    c.add_msg_player_or_npc(
                        _( "You put some items in the %1$s's %2$s." ),
                        _( "<npcname> puts some items in the %1$s's %2$s." ),
                        veh.name, part_name
                    );
                }
                break;
...
int get_auto_consume_moves( Character &you, const bool food )
{
    ...
    struct auto_consume_candidate {
        item *source;
        int vehicle_part_index;
        bool is_vehicle_part_base;
    };
    ...
    for( const tripoint_abs_ms &loc : dest_set ) {
        ...
        std::vector<auto_consume_candidate> items_here;
        if( vp ) {
            ...
            if( index >= 0 ) {
                ...
                for( item &it : vehitems ) {
                    items_here.push_back( { &it, index, false } );
                }
            }

            // A vehicle-bound Auto Drink zone on a working faucet can draw from
            // any suitable liquid tank installed anywhere on the same vehicle.
            if( !food && vp->part_with_tool( itype_water_faucet ) ) {
                for( const vpart_reference &tank : veh.get_avail_parts( "FLUIDTANK" ) ) {
                    const int tank_index = static_cast<int>( tank.part_index() );
                    item_location tank_loc = veh.part_base( tank_index );
                    items_here.push_back( { tank_loc.get_item(), tank_index, true } );
                }
            }
        } else {
            map_stack mapitems = here.i_at( here.getlocal( loc ) );
            for( item &it : mapitems ) {
                items_here.push_back( { &it, -1, false } );
            }
        }
        for( const auto_consume_candidate &candidate : items_here ) {
            if( candidate.source == nullptr ) {
                continue;
            }
            item &comest = you.get_consumable_from( *candidate.source );
            ...
            if( !candidate.source->is_owned_by( you, true ) ) {
                // it aint ours.
                continue;
            }
            ...
            if( !food && candidate.source->is_watertight_container() &&
                comest.made_of( phase_id::SOLID ) ) {
                // it's frozen
                continue;
            }
            ...
            const item &move_cost_item = candidate.is_vehicle_part_base ? comest : *candidate.source;
            int consume_moves = -Pickup::cost_to_move_item( you, move_cost_item ) *
                                std::max( rl_dist( you.pos(), here.getlocal( loc ) ), 1 );
            consume_moves += to_moves<int>( you.get_consume_time( comest ) );
            item_location item_loc;
            if( vp ) {
                if( candidate.is_vehicle_part_base ) {
                    item_location tank_loc = vp->vehicle().part_base( candidate.vehicle_part_index );
                    item_loc = item_location( tank_loc, &comest );
                } else {
                    item_loc = item_location( vehicle_cursor( vp->vehicle(),
                                              candidate.vehicle_part_index ), &comest );
                }
            } else {
