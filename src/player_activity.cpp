...
void player_activity::do_turn( Character &you )
{
    ...
    if( !type->do_turn_EOC.is_null() ) {
        const std::uint64_t activity_generation = you.activity_generation;
        dialogue d( get_talker_for( you ), nullptr );
        if( type->do_turn_EOC->type == eoc_type::ACTIVATION ) {
            type->do_turn_EOC->activate( d );
        } else {
            debugmsg( "Activity do_turn_eoc must reference an ACTIVATION EOC." );
        }
        // An EOC may interrupt or replace the activity.
        if( type.is_null() || you.activity_generation != activity_generation ) {
            return;
        }
    }
    ...
    if( *this && moves_left <= 0 ) {
        ...
        const activity_id completed_type = id();
        if( !type->completion_EOC.is_null() ) {
            ...
            if( type->completion_EOC->type == eoc_type::ACTIVATION ) {
                ...
                debugmsg( "Activity completion_eoc must reference an ACTIVATION EOC." );
            }
            // A completion EOC may cancel or replace this activity.  Do not finish
            // the replacement, including a same-type replacement with fresh moves.
            if( type.is_null() || you.activity_generation != activity_generation ||
                id() != completed_type || moves_left > 0 ) {
                return;
            }
        }
        if( actor ) {
