    ...
    , fontblending( fontblending )
{
    ...
    */
    ...
    const std::string hinting = get_option<std::string>( "FONT_HINTING" );
    const int hinting_mode = hinting == "light" ? TTF_HINTING_LIGHT :
                             hinting == "mono" ? TTF_HINTING_MONO :
                             hinting == "none" ? TTF_HINTING_NONE : TTF_HINTING_NORMAL;
    TTF_SetFontHinting( font.get(), hinting_mode );
