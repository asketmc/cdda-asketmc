...
struct talk_effect_fun_t {
    ...
    public:
        ...
        void set_map_run_item_eocs( const JsonObject &jo, const std::string &member, bool is_npc );
        void set_queue_eocs( const JsonObject &jo, const std::string &member );
        void set_if( const JsonObject &jo, const std::string &member );
        void set_switch( const JsonObject &jo, const std::string &member );
        void set_foreach( const JsonObject &jo, const std::string &member );
        ...
        void set_map_spawn_item( const JsonObject &jo, const std::string &member );
...
template<class T>
static std::string read_context_value( const T &, const std::string & )
{
    return "";
}

template<class T>
static const dialogue *get_dialogue_context( const T & )
{
    return nullptr;
}

template<class T>
static std::string read_var_value_internal( const var_info &info, const T &d,
        std::unordered_set<std::string> &indirect_vars )
{
    ...
    switch( info.type ) {
        ...
        case var_type::context:
            ret_val = read_context_value( d, info.name );
            break;
        case var_type::var: {
            if( indirect_vars.size() >= 100 ) {
                debugmsg( "Indirect variable chain exceeds 100 references at %s.", info.name );
                break;
            }
            if( !indirect_vars.insert( info.name ).second ) {
                debugmsg( "Indirect variable cycle detected at %s.", info.name );
                break;
            }
            const std::string referenced_var = read_context_value( d, info.name );
            if( !referenced_var.empty() ) {
                ret_val = read_var_value_internal( process_variable( referenced_var ), d, indirect_vars );
            }
            indirect_vars.erase( info.name );
            break;
        }
        case var_type::u:
...
{
    std::unordered_set<std::string> indirect_vars;
    return read_var_value_internal( info, d, indirect_vars );
}
