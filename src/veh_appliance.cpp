...
void convert_to_appliance( const tripoint &p, const cata::optional<furn_str_id> &furn_set,
                           const cata::optional<ter_str_id> &ter_set,
                           const itype_id &appliance_item )
{
    map &here = get_map();
    if( furn_set ) {
        here.furn_set( p, *furn_set );
    }
    if( ter_set ) {
        here.ter_set( p, *ter_set );
    }
    place_appliance( p, vpart_appliance_from_item( appliance_item ) );
}
...
static void draw_connection_line( const tripoint &start, const tripoint &end )
{
    if( start == end ) {
        return;
    }
    const std::vector<tripoint> points = line_to( start, end );
#if defined(TILES)
    if( use_tiles ) {
...
{
    if( !showing_connections || veh->num_parts() == 0 ) {
        return;
    }

    const tripoint start = veh->global_part_pos3( 0 );
    const std::map<vehicle *, bool> connected = vehicle::enumerate_vehicles( { veh } );
    for( const std::pair<vehicle *const, bool> &entry : connected ) {
        vehicle *other = entry.first;
        if( other == veh || other->num_parts() == 0 ) {
            continue;
        }
        draw_connection_line( start, other->global_part_pos3( 0 ) );
    }

    for( const vpart_reference &part : veh->get_all_parts() ) {
        draw_connection_line( start, veh->global_part_pos3( part.part() ) );
    }
}
...
void veh_app_interact::draw_info()
{
    ...
    if( !veh->accessories.empty() || veh->has_part( "RECHARGE" ) ) {
        int rate = veh->total_accessory_epower_w() + veh->recharge_epower_this_turn;
...
{
    shared_ptr_fast<game::draw_callback_t> connection_cb =
        make_shared_fast<game::draw_callback_t>( [this]() {
        draw_appliance_connections( veh );
    } );
    g->add_draw_callback( connection_cb );
