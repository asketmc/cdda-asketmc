...
bool enchantment::is_active( const vehicle &, const bool active ) const
{
    if( active_conditions.second == condition::ALWAYS ) {
        return true;
    }
    ...
    // 0.G has no vehicle dialogue talker, so dialogue conditions cannot be
    // evaluated safely for a vehicle.
    return false;
}

bool enchantment::is_vehicle_relevant() const
{
    const auto is_vehicle_value = []( enchant_vals::mod value ) {
        return value == enchant_vals::mod::TOTAL_WEIGHT ||
               value == enchant_vals::mod::FUEL_USAGE ||
               value == enchant_vals::mod::TURNING_DIFFICULTY;
    };
    return std::any_of( values_add.begin(), values_add.end(),
    [&is_vehicle_value]( const std::pair<const enchant_vals::mod, int_or_var<dialogue>> &value ) {
        return is_vehicle_value( value.first );
    } ) || std::any_of( values_multiply.begin(), values_multiply.end(),
    [&is_vehicle_value]( const std::pair<const enchant_vals::mod, int_or_var<dialogue>> &value ) {
        return is_vehicle_value( value.first );
    } );
}
...
void enchant_cache::force_add( const enchantment &rhs, const vehicle & )
{
    // Vehicle enchantments in this 0.G backport accept only constants and global variables.
    // A topic-only talker keeps dialogue evaluation valid without binding it to an avatar/NPC.
    dialogue d( get_talker_for( std::vector<std::string>() ), nullptr );
    for( const std::pair<const enchant_vals::mod, int_or_var<dialogue>> &pair_values :
         rhs.values_add ) {
        const int_or_var<dialogue> &value = pair_values.second;
        const auto safe_without_talker = []( const int_or_var_part<dialogue> &part ) {
            return part.int_val.has_value() ||
                   ( part.var_val.has_value() && part.var_val->type == var_type::global );
        };
        if( safe_without_talker( value.min ) && ( !value.pair || safe_without_talker( value.max ) ) ) {
            values_add[pair_values.first] += value.evaluate( d );
        }
    }
    for( const std::pair<const enchant_vals::mod, int_or_var<dialogue>> &pair_values :
         rhs.values_multiply ) {
        ...
        if( safe_without_talker( value.min ) && ( !value.pair || safe_without_talker( value.max ) ) ) {
            values_multiply[pair_values.first] += 0.01 * value.evaluate( d );
        }
    }
}
