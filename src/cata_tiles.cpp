...
auto simple_point_hash = []( const auto &p ) -> unsigned int
{
    return static_cast<unsigned int>( p.x ) + static_cast<unsigned int>( p.y ) * 65536U;
};

} // namespace

std::string pick_layer_variant_sprite( const layer_variant &variant, const unsigned int seed )
{
    if( variant.total_weight <= 0 ) {
        return std::string();
    }

    int roll = static_cast<int>( seed % static_cast<unsigned int>( variant.total_weight ) );
    for( const std::pair<const std::string, int> &sprite : variant.sprite ) {
        roll -= sprite.second;
        if( roll < 0 ) {
            return sprite.first;
        }
...
std::string layered_item_variant( const layer_variant &layer, const std::string &item_variant )
{
    return item_variant + layer.append_suffix;
}

std::string tile_id_with_variant( const std::string &id, const std::string &variant,
                                  const bool allow_literal_suffix )
{
    if( variant.empty() ) {
        return id;
    }
    if( allow_literal_suffix && variant.front() == '_' ) {
        return id + variant;
    }
    return id + "_var_" + variant;
}

int calculate_occlusion_retract( const int mode, const float distance,
                                 const float min_distance, const float max_distance )
{
    ...
    const float distance_slope = distance_range <= 0.0f ? 100.0f : 1.0f / distance_range;
    return static_cast<int>( 100.0f *
                             ( 1.0f - clamp( ( distance - min_distance ) * distance_slope,
                                             0.0f, 1.0f ) ) );
}

int scaled_zlevel_height( const int source_height, const int rendered_tile_width,
                          const int source_tile_width )
{
    if( source_height == 0 || rendered_tile_width <= 0 || source_tile_width <= 0 ) {
        return 0;
    }
    const int scaled_magnitude = std::max( 1, std::abs( source_height ) * rendered_tile_width /
                                          source_tile_width );
    return source_height < 0 ? -scaled_magnitude : scaled_magnitude;
}

int zlevel_screen_y( const int base_y, const int source_height, const int rendered_tile_width,
                     const int source_tile_width )
{
    return base_y - scaled_zlevel_height( source_height, rendered_tile_width, source_tile_width );
}

int zlevel_fog_alpha( const bool isometric, const int scaled_height )
{
    if( !isometric || scaled_height <= 0 ) {
        return 100;
    }
    return clamp( static_cast<int>( 255.0f - 255.0f * std::pow(
                                       155.0f / 255.0f, scaled_height / 100.0f ) ), 40, 150 );
}

int effective_3d_draw_depth( const bool isometric, const int requested_depth,
                             const int scaled_height )
{
    return isometric && scaled_height <= 0 ? 0 : requested_depth;
}
...
void cata_tiles::set_draw_scale( int scale )
{
    ...
    // Keep the accumulator in source-tile units.  Sprites and geometry fog
    // both convert it to screen pixels at their final Y transform.
    zlevel_height = tileset_ptr->get_zlevel_height();
    fog_alpha = zlevel_fog_alpha( is_isometric(), scaled_zlevel_height( zlevel_height,
                                  ...
                                  const bool pump_events )
{
    ...
    for( const JsonObject curr_info : config.get_array( "tile_info" ) ) {
        ...
        if( ts.tile_isometric && ts.zlevel_height <= 0 ) {
            DebugLog( D_WARNING, D_SDL ) << "Isometric tileset '" << tileset_id
                                         << "' has no positive zlevel_height; using legacy single-Z drawing.";
        }
    ...
    for( const JsonObject item : config.get_array( "variants" ) ) {
        if( item.has_member( "context" ) && ( item.has_array( "item_variants" ) ||
            ...
            if( item.has_string( "context" ) ) {
                ...
                // A one-element array denotes a terrain/furniture flag context.
                context = item.get_array( "context" ).next_value().get_string();
            } else {
                config.throw_error( "layering context must be a string or one-element string array" );
            }
            ...
            if( item.has_string( "append_variants" ) ) {
                ...
                if( append_suffix.empty() ) {
                    config.throw_error( "append_variants cannot be an empty string" );
                }
            }
                ...
                for( const JsonObject vars : item.get_array( "item_variants" ) ) {
                    if( vars.has_member( "item" ) && vars.has_member( "layer" ) ) {
                        ...
                        if( vars.has_array( "sprite" ) ) {
                            ...
                            // Modern layering data defaults an omitted sprite to the item id.
                            v.sprite.emplace( v.id, 1 );
                            total_weight = 1;
                        }
                       ...
                       color_block_overlay_container &color_blocks )
{
    ...
    std::vector<std::pair<std::size_t, std::size_t>> draw_rows;
    for( int row = min_row; row < max_row; row ++ ) {
        const std::size_t row_begin = draw_points.size();
        ...
        draw_rows.emplace_back( row_begin, draw_points.size() );
    }
    ...
    // Limit draw depth to the vertical vision setting.  Isometric tilesets use
    // their configured pixel separation between z-levels.
    const int max_draw_depth = effective_3d_draw_depth( is_isometric(), fov_3d_z_range,
                               zlevel_height );
    const int min_draw_z = std::max( center.z - max_draw_depth, -OVERMAP_DEPTH );
    ...
    if( max_draw_depth <= 0 ) {
        // Legacy draw mode
        for( const std::pair<std::size_t, std::size_t> &row_range : draw_rows ) {
            for( auto f : drawing_layers_legacy ) {
                for( std::size_t index = row_range.first; index < row_range.second; ++index ) {
                    tile_render_info &p = draw_points[index];
        ...
        // Start at the lowest configured visible z-level.  Bounding this range
        // is also the 0.G-compatible form of the #82240 isometric look-up fix:
        // never walk renderer data below the range prepared for this frame.
        int cur_zlevel = min_draw_z;
        do {
            const int base_height_3d = ( cur_zlevel - center.z ) * zlevel_height;

            // Sprite stacks are per map tile.  Reset every column participating
            // in this z-level before drawing its layers.
            for( int iter = min_draw_z; iter <= cur_zlevel; ++iter ) {
                for( tile_render_info &p : draw_points_3d[iter] ) {
                    p.height_3d = base_height_3d;
                }
            }
            ...
            for( auto f : drawing_layers ) {
                ...
                do {
                    for( tile_render_info &p : draw_points_3d[iter] ) {
                        ...
                        lit_level ll = p.ll;
                        std::array<bool, 5> invisible = p.invisible;
                        if( cur_zlevel != center.z ) {
                            const auto cached = here.ll_invis_cache.find( draw_loc );
                            if( cached == here.ll_invis_cache.end() ) {
                                const std::pair<lit_level, std::array<bool, 5>> ll_invis =
                                    calc_ll_invis( draw_loc );
                                ll = ll_invis.first;
                                invisible = ll_invis.second;
                                if( !test_mode ) {
                                    here.ll_invis_cache.emplace( draw_loc, ll_invis );
                                }
                            } else {
                                ll = cached->second.first;
                                invisible = cached->second.second;
                            }
                        }

                        if( f == &cata_tiles::draw_vpart_no_roof ||
                            f == &cata_tiles::draw_vpart_roof ) {
                            const int stacked_height_3d = p.height_3d;
                            p.height_3d = base_height_3d;
                            if( !( this->*f )( draw_loc, ll, p.height_3d, invisible ) ) {
                                p.height_3d = stacked_height_3d;
                            }
                        } else if( f == &cata_tiles::draw_critter_at ) {
                            if( !( this->*f )( draw_loc, ll, p.height_3d, invisible ) &&
                                do_draw_shadow && cur_zlevel == iter ) {
                                draw_critter_above( draw_loc, ll, p.height_3d, invisible );
                            }
                        } else {
                            ( this->*f )( draw_loc, ll, p.height_3d, invisible );
                        }
                    }
                    iter += 1;
                                  ...
                                  const int looks_like_jumps_limit ) const
{
    ...
    */
    ...
    if( !variant.empty() ) {
        auto tile_variant_with_season = find_tile_with_season(
                                            tile_id_with_variant( id, variant,
        ...
        const point &offset )
{
    ...
    if( retract < 0 && ( prevent_occlusion_transp || prevent_occlusion_retract ) ) {
        ...
        retract = calculate_occlusion_retract( prevent_occlusion, distance, d_min, d_max );
    ...
    if( !tt ) {
        ...
        if( category == TILE_CATEGORY::FURNITURE ) {
            ...
            if( vpid.is_valid() ) {
                ...
                // Vehicle rendering now passes the tileset index (0=N, 1=W,
                // 2=S, 3=E).  Convert it back only for the curses-symbol
                // fallback, whose tileray uses 0=E and increases clockwise.
                tileray face = tileray( 270_degrees - 90_degrees * rota );
    ...
    bool apply_night_vision_goggles, int retract, int &height_3d, const point &offset )
{
    ...
    const int source_tile_width = tileset_ptr->get_tile_width();
    const int base_y = p.y + ( tile_offset.y + offset.y ) * tile_width / source_tile_width;
    destination.y = zlevel_screen_y( base_y, height_3d, tile_width, source_tile_width );
                               ...
                               const std::array<bool, 5> &invisible )
{
    ...
    if( t && !invisible[0] ) {
        ...
        if( here.check_seen_cache( p ) && p.z == get_avatar().posz() ) {
                                 ...
                                 const std::array<bool, 5> &invisible )
{
    ...
    if( f && !invisible[0] ) {
        ...
        if( !( you.get_grab_type() == object_type::FURNITURE
            ...
            && here.check_seen_cache( p ) && p.z == get_avatar().posz() ) {
                ...
                if( itt != tileset_ptr->field_layer_data.end() ) {
                    ...
                    for( const layer_variant &layer_var : itt->second ) {
                        if( fld.id().str() == layer_var.id ) {
                            ...
                            const std::string sprite_to_draw =
                                pick_layer_variant_sprite( layer_var, simple_point_hash( p ) );
                ...
                for( const layer_variant &layer_var : itt->second ) {
                    for( const item &i : tile.get_items() ) {
                        if( i.typeId().str() == layer_var.id ) {
                            ...
                            const std::string sprite_to_draw =
                                pick_layer_variant_sprite( layer_var, i.seed );

                            variant = layered_item_variant(
                                          layer_var, i.has_itype_variant() ? i.itype_variant().id : "" );
                            // if we have found info on the item go through and draw its stuff
                             ...
                             const std::array<bool, 5> &invisible, bool roof )
{
    ...
    if( vp && !invisible[0] ) {
        ...
        if( !vp_id.empty() ) {
            ...
            // Keep storing the legacy degree value in map memory for save compatibility,
            // but pass the four-direction tileset index to the renderer.
            const int memory_rotation = std::round( to_degrees( veh.face.dir() ) );
            const int tile_rotation = angle_to_dir4( 270_degrees - veh.face.dir() );
            ...
            if( !veh.forward_velocity() && !veh.player_in_control( you )
                ...
                && here.check_seen_cache( p ) && p.z == get_avatar().posz() ) {
                you.memorize_tile( here.getabs( p ), vpname, subtile, memory_rotation );
            }
            if( !overridden ) {
                ...
                const bool ret =
                    draw_from_id_string( vpname, TILE_CATEGORY::VEHICLE_PART, empty_string, p,
                                         subtile, tile_rotation, ll, nv_goggles_activated, height_3d_temp );
                if( !roof ) {
                    height_3d = height_3d_temp;
                }
                const cata::optional<vpart_reference> tiedown =
                    vp.part_with_feature( "FURNITURE_TIEDOWN", true );
                if( ret && !roof && tiedown ) {
                    const std::string furniture =
                        tiedown->part().get_base().get_var( "tied_down_furniture" );
                    if( furn_str_id( furniture ).is_valid() ) {
                        draw_from_id_string( furniture, TILE_CATEGORY::FURNITURE, empty_string, p,
    ...
    if( overridden ) {
        ...
        if( vp2 ) {
            ...
            const int tile_rotation = angle_to_dir4( 270_degrees - rotation );
            ...
            const bool ret =
                draw_from_id_string( vpname, TILE_CATEGORY::VEHICLE_PART, empty_string, p, subtile,
                                     tile_rotation, lit_level::LIT, false, height_3d_temp );
            if( !roof ) {
                height_3d = height_3d_temp;
            }
            if( ret && draw_highlight ) {
        ...
        int height_3d_temp = height_3d;
        const int tile_rotation = angle_to_dir4( 270_degrees -
                                  units::from_degrees( t.rotation ) );
        return draw_from_id_string(
                   t.tile, TILE_CATEGORY::VEHICLE_PART, empty_string, p, t.subtile, tile_rotation,
...
void cata_tiles::draw_zlevel_overlay( const tripoint &p, const lit_level ll, int &height_3d )
{
    ...
    draw_rect.y = zlevel_screen_y( screen.y, height_3d, tile_width,
