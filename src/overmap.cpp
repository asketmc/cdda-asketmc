...
struct fixed_overmap_special_data : overmap_special_data {
    ...
    void check( const std::string &context ) const override {
        ...
        for( const overmap_special_terrain &elem : terrains ) {
            ...
            if( elem.camp_owner ) {
                if( !elem.camp_owner->is_valid() ) {
                    debugmsg( "In %s, camp at %s has invalid owner %s", context,
                              pos.to_string(), elem.camp_owner->c_str() );
                }
                if( elem.camp_name.empty() ) {
                    debugmsg( "In %s, camp at %s is missing camp_name", context,
                              pos.to_string() );
                }
            } else if( !elem.camp_name.empty() ) {
                debugmsg( "In %s, camp_name at %s has no camp owner", context,
                          pos.to_string() );
            }
    ...
    special_placement_result place(
        ...
        for( const overmap_special_terrain &elem : terrains ) {
            ...
            if( elem.camp_owner ) {
                const tripoint_abs_omt camp_loc = project_combine( om.pos(), location );
                basecamp &new_camp = get_map().add_camp( camp_loc, "faction_camp", false );
                new_camp.set_owner( *elem.camp_owner );
                new_camp.set_name( elem.camp_name.translated() );
                new_camp.define_camp( camp_loc,
                                      "faction_base_bare_bones_NPC_camp_0", false );
            }
