...
void handle_npc_liquid( item liquid, Character &who )
{
    ...
    map &here = get_map();
    const tripoint char_pos = who.pos();
    // Look for containers on adjacent tiles, in adjacent vehicle cargo, and on the character.
    for( const tripoint &pos : closest_points_first( char_pos, 1 ) ) {
        ...
        if( cargo ) {
            for( item &it : cargo->vehicle().get_items( cargo->part_index() ) ) {
                if( it.is_watertight_container() ) {
                    container_locs.emplace_back(
                        vehicle_cursor( cargo->vehicle(), cargo->part_index() ), &it );
                }
            }
        }
    }
    ...
    for( item_location &container_loc : container_locs ) {
        const bool is_carried = container_loc.where_recursive() == item_location::type::character;
        // Open containers are safe to fill while resting on the map or in
        // stationary vehicle cargo, but not while carried by the NPC.
        const bool allow_buckets = !is_carried;
        const int capacity = container_loc->get_remaining_capacity_for_liquid( liquid, allow_buckets );
        capacities.push_back( capacity );

        int priority = 0;
        if( capacity > 0 ) {
            // Filling an already compatible container avoids tying up another container.
    ...
    const int num_containers = static_cast<int>( container_locs.size() );
    while( liquid.charges > 0 ) {
        ...
        for( int idx = 0; idx < num_containers; ++idx ) {
            ...
            } else if( container_locs[idx]->base_volume() !=
                       container_locs[best_idx]->base_volume() ) {
        ...
        const int inserted = container_locs[best_idx]->fill_with( liquid, amount );
        liquid.charges -= inserted;
        if( inserted > 0 ) {
            // Invalidate the owning character/map/vehicle caches as well as
            // the container's own contents cache.
            container_locs[best_idx].on_contents_changed();
        }
