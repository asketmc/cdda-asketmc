...
void sfx::load_sound_effects( const JsonObject &jsobj )
{
    ...
    const std::vector<std::string> files = jsobj.get_string_array( "files" );
...
// Returns a sound effect matching the given id and variant without falling
// back to the default variant.
bool sfx::has_exact_variant_sound( const std::string &id, const std::string &variant,
                                   const std::string &season,
                                   const cata::optional<bool> &is_indoors,
                                   const cata::optional<bool> &is_night )
{
    return sfx_resources.sound_effects.find_no_fallback(
               id, variant, season, is_indoors, is_night ) != nullptr;
}
