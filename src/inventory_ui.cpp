...
ret_val<void> can_insert_item_directly( const item_location &container,
                                        const item_location &candidate )
{
    if( !container || !candidate || container == candidate ||
        container.eventually_contains( candidate ) || candidate.eventually_contains( container ) ) {
        return ret_val<void>::make_failure( _( "a container can't contain itself" ) );
    }

    item direct_candidate( *candidate );
    if( direct_candidate.count_by_charges() ) {
        direct_candidate.charges = 1;
    }

    ret_val<void> result = container->can_contain_directly( direct_candidate );
    if( !result.success() ) {
        return result;
    }
    if( container->all_pockets_rigid() ) {
        return result;
    }
    if( direct_candidate.count_by_charges() ) {
        if( container.max_charges_by_parent_recursive( direct_candidate ) <= 0 ) {
            return ret_val<void>::make_failure( _( "item can't fit through a parent container" ) );
        }
    } else if( !container.parents_can_contain_recursive( &direct_candidate ) ) {
        return ret_val<void>::make_failure( _( "item can't fit through a parent container" ) );
    }
    return result;
}
...
std::string inventory_holster_preset::get_denial( const item_location &it ) const
{
    ...
    if( it->made_of( phase_id::LIQUID ) && !holster->is_watertight_container() ) {
        return _( "container is not watertight" );
    }
    if( it->is_bucket_nonempty() ) {
        return _( "item would spill" );
    }

    const ret_val<void> can_contain = can_insert_item_directly( holster, it );
    if( !can_contain.success() ) {
        return can_contain.str().empty() ? _( "item can't be stored there" ) : can_contain.str();
    }
    return {};
}
