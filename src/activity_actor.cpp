    ...
    if( qual < 2 ) {
        if( !testing ) {
            debugmsg( "Item %s with 'HACKSAW' use action requires SAW_M quality of at least 2.",
                      type ? type.value().c_str() : tool->typeId().c_str() );
        }
        act.set_to_null();
        return;
    }
...
void hacksaw_activity_actor::do_turn( player_activity &/*act*/, Character &who )
{
    ...
    if( !veh_pos ) {
        ...
        if( !vp || !type || !vp->part_with_tool( type.value() ) ) {
            if( !testing ) {
        ...
        if( vehicle::use_vehicle_tool( veh, veh_pos.value(), type.value(), true ) ) {
            ...
            if( who.is_avatar() ) {
                who.add_msg_if_player( m_bad, _( "Your %1$s ran out of charges." ),
                                       type.value()->nname( 1 ) );
            } else { // who.is_npc()
                add_msg_if_player_sees( who.pos(), _( "%1$s %2$s ran out of charges." ), who.disp_name( false,
        ...
        const Character &/*who*/ ) const
{
    const hacksaw_activity_actor &actor = static_cast<const hacksaw_activity_actor &>
                                          ( other );
    if( veh_pos ) {
        return actor.target == target && actor.veh_pos == veh_pos && actor.type == type;
    }
    return actor.target == target && !actor.veh_pos && actor.tool.operator == ( tool );
}
...
{
    const auto first_unavailable = std::remove_if( books.begin(), books.end(),
    []( const item_location & book ) {
        return !book;
    } );
    if( first_unavailable != books.end() ) {
        add_msg_debug( debugmode::DF_ACT_EBOOK, "Skipping unavailable books while scanning" );
        const int completed_moves = std::max( 0, act.moves_total - act.moves_left );
        books.erase( first_unavailable, books.end() );
        act.moves_left = std::min( act.moves_left, to_moves<int>( required_time( books ) ) );
        act.moves_total = completed_moves + act.moves_left;
    }
    if( books.empty() ) {
        act.moves_left = 0;
        return;
    ...
    const int completed_pages = std::max( 0, act.moves_total - act.moves_left ) /
                                to_moves<int>( time_per_page );
    if( act.values.empty() ) {
        // Old saves had no progress counter.  Treat already elapsed pages as
        // accounted for so loading cannot cause an immediate duplicate drain.
        act.values.push_back( completed_pages );
    }
    const int charges_due = completed_pages / pages_per_charge -
                            act.values.front() / pages_per_charge;
    if( charges_due > 0 ) {
        if( !ereader->ammo_sufficient( &who, charges_due ) ) {
            add_msg_if_player_sees(
                who,
                _( "%1$s %2$s ran out of batteries." ),
                who.disp_name( true, true ),
        ...
        ereader->ammo_consume( ereader->ammo_required() * charges_due, who.pos(), &who );
    }
    act.values.front() = completed_pages;

    // ACT_EBOOKSAVE is speed-based, so moves_left already contains the exact
    // speed-adjusted progress made this turn.  Complete every book boundary
    // crossed by that progress without resetting or discarding the remainder.
    while( !books.empty() ) {
        const int current_book_moves = to_moves<int>(
                                           pages_in_book( books.back()->typeId() ) * time_per_page );
        const int moves_after_current_book =
            to_moves<int>( required_time( books ) ) - current_book_moves;
        if( act.moves_left > moves_after_current_book ) {
            break;
        }
        completed_scanning_current_book( who );
    }
}

void ebooksave_activity_actor::completed_scanning_current_book( Character &who )
{
...
{
    return lhs.where() == item_location::type::container &&
           rhs.where() == item_location::type::container &&
...
static std::list<item> obtain_activity_items(
    ...
    contents_change_handler &handler,
    Character &who, bool &bulk_unload )
{
    std::list<item> res;
    ...
    for( ; it != items.end(); ++it ) {
        ...
        if( !loc ) {
            // some items could have been destroyed by e.g. monster attack
            // An invalid item also breaks the contiguous bulk-unload sequence.
            bulk_unload = false;
            continue;
        }
        ...
        int consumed_moves;
        if( bulk_unload ) {
            consumed_moves = std::max(
                                 who.item_handling_cost( *loc, true, 0, it->count(), true ), 1 );
        } else {
        ...
        // Save whether the following item shares this container before the
        // current location is invalidated by removal.
        const auto next = std::next( it );
        bulk_unload = next != items.end() && next->loc() &&
                      is_bulk_unload( loc, next->loc() );
...
static void stop_furniture_grab( Character &who )
{
    avatar *you = dynamic_cast<avatar *>( &who );
    if( you != nullptr ) {
        you->grab( object_type::NONE );
    }
}

static bool transfer_loaded_furniture( vpart_position &from, vpart_position &to )
{
    const cata::optional<vpart_reference> from_loader =
        from.part_with_feature( "FURNITURE_TIEDOWN", true );
    const cata::optional<vpart_reference> to_loader =
        to.part_with_feature( "FURNITURE_TIEDOWN", true );
    if( !from_loader || !to_loader ) {
        return false;
    }

    item from_base( from_loader->part().get_base() );
    item to_base( to_loader->part().get_base() );
    const std::string furniture = from_base.get_var( "tied_down_furniture" );
    if( furniture.empty() || to_base.has_var( "tied_down_furniture" ) ) {
        return false;
    }

    from_base.erase_var( "tied_down_furniture" );
    to_base.set_var( "tied_down_furniture", furniture );
    from_loader->part().set_base( from_base );
    to_loader->part().set_base( to_base );
    from.vehicle().invalidate_mass();
    to.vehicle().invalidate_mass();
    return true;
}

bool move_furniture_activity_actor::can_move_furniture_on_vehicle_to( map &here,
        const tripoint &dest ) const
{
    ...
    const optional_vpart_position destination_vehicle = here.veh_at( dest );
    if( destination_vehicle && !destination_vehicle->can_load_furniture() ) {
        return false;
    }
    return g->can_move_furniture( dest, dp );
}

bool move_furniture_activity_actor::move_furniture_on_vehicle( Character &who ) const
{
    map &here = get_map();
    const tripoint position = who.pos() + who.grab_point;
    optional_vpart_position source = here.veh_at( position );
    if( !source || !source->has_loaded_furniture() ) {
        add_msg( m_warning, _( "The grabbed furniture has been lost." ) );
        stop_furniture_grab( who );
        return true;
    }

    const tripoint destination = position + dp;
    if( !can_move_furniture_on_vehicle_to( here, destination ) ) {
        add_msg( m_warning, _( "Can't drag to there." ) );
        return true;
    }

    const bool pulling = dp.xy() == -who.grab_point.xy();
    const bool pushing = dp.xy() == who.grab_point.xy();
    const bool shifting = !pushing && !pulling;
    optional_vpart_position destination_vehicle = here.veh_at( destination );
    if( destination_vehicle ) {
        if( !transfer_loaded_furniture( *source, *destination_vehicle ) ) {
            add_msg( m_warning, _( "Can't load the furniture there." ) );
            return true;
        }
    } else {
        const cata::optional<vpart_reference> loader =
            source->part_with_feature( "FURNITURE_TIEDOWN", true );
        if( !loader ) {
            stop_furniture_grab( who );
            return true;
        }
        loader->part().unload_furniture( here, destination );
        source->vehicle().invalidate_mass();
        avatar *you = dynamic_cast<avatar *>( &who );
        if( you != nullptr ) {
            you->grab( object_type::FURNITURE, who.grab_point );
        }
    }

    if( shifting ) {
        const tripoint new_grab_point = who.grab_point + dp;
        if( std::abs( new_grab_point.x ) < 2 && std::abs( new_grab_point.y ) < 2 ) {
            who.grab_point = new_grab_point;
        } else {
            stop_furniture_grab( who );
        }
        return true;
...
void move_furniture_activity_actor::start( player_activity &act, Character &who )
{
    int moves = 0;
    if( on_vehicle ) {
        const optional_vpart_position source = get_map().veh_at( who.pos() + who.grab_point );
        const cata::optional<vpart_reference> loader = source ?
                source->part_with_feature( "FURNITURE_TIEDOWN", true ) : cata::nullopt;
        if( !loader ) {
            act.set_to_null();
            stop_furniture_grab( who );
            return;
        }
        const furn_str_id furniture( loader->part().get_base().get_var( "tied_down_furniture" ) );
        if( !furniture.is_valid() ) {
            debugmsg( "Tried dragging invalid furniture %s", furniture.str() );
            act.set_to_null();
            stop_furniture_grab( who );
            return;
        }
        moves = 50;
        if( furniture->move_str_req > who.get_arm_str() ) {
            moves = std::max<int>( 3000, furniture->move_str_req * 10 +
                                   std::pow( furniture->move_str_req, 2.0 ) * 10 );
        }
    } else {
        moves = g->grabbed_furn_move_time( dp );
    }
    act.moves_left = moves;
    act.moves_total = moves;
}
...
{
    const bool furniture_stayed_put = on_vehicle ? move_furniture_on_vehicle( who ) :
                                      g->grabbed_furn_move( dp );
    if( !furniture_stayed_put ) {
...
void move_furniture_activity_actor::serialize( JsonOut &jsout ) const
{
    ...
    jsout.member( "on_vehicle", on_vehicle );
    jsout.end_object();
}

std::unique_ptr<activity_actor> move_furniture_activity_actor::deserialize( JsonValue &jsin )
{
    move_furniture_activity_actor actor = move_furniture_activity_actor( tripoint_zero, false, false );

    JsonObject data = jsin.get_object();

    data.read( "dp", actor.dp );
    data.read( "via_ramp", actor.via_ramp );
    data.read( "on_vehicle", actor.on_vehicle );
...
void insert_item_activity_actor::finish( player_activity &act, Character &who )
{
    ...
    if( holstered_item.first ) {
        ...
        const bool can_insert = can_insert_item_directly( holster, holstered_item.first ).success();
...
void mop_activity_actor::finish( player_activity &act, Character &who )
{
    ...
    const bool will_mop = one_in( who.is_blind() ? 3 : 1 );
    if( will_mop ) {
        map &here = get_map();
