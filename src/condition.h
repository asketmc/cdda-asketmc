...
{
using query_tile_selector_for_test = std::function<cata::optional<tripoint>(
        const std::string &, avatar &, const tripoint &, const std::string &, int, bool )>;

// Interactive tile selection is not available in headless tests.  Tests can
// temporarily replace this callback to exercise the complete EOC condition.
query_tile_selector_for_test &query_tile_selector_for_tests();
...
const std::unordered_set<std::string> complex_conds = { {
        ...
        "u_has_cash", "u_are_owed", "u_query", "npc_query", "u_query_tile", "npc_query_tile",
        ...
        "u_compare_time_since_var", "npc_compare_time_since_var", "is_weather", "map_terrain_with_flag",
        "map_furniture_with_flag", "mod_is_loaded", "one_in_chance", "x_in_y_chance",
        ...
        "compare_string", "roll_contested", "get_condition"
    }
};
} // namespace dialogue_data
...
{
    const auto copy_actor = [&d]( bool is_beta ) -> std::unique_ptr<talker> {
        if( ( is_beta && !d.has_beta ) || ( !is_beta && !d.has_alpha ) ) {
            return nullptr;
        }
        talker *source = d.actor( is_beta );
        if( Creature *creature = source->get_creature() ) {
            return get_talker_for( creature );
        }
        if( item_location *it = source->get_item() ) {
            return get_talker_for( it );
        }
        if( computer *comp = source->get_computer() ) {
            return get_talker_for( comp );
        }

        // Const character talkers intentionally expose their Creature only through
        // the const accessor.  Preserve that constness while rebuilding the dialogue.
        const talker *const_source = source;
        if( const Creature *creature = const_source->get_creature() ) {
            return get_talker_for( *creature );
        }
        if( item_location *it = const_source->get_item() ) {
            return get_talker_for( it );
        }
        return nullptr;
    };

    dialogue result( copy_actor( false ), copy_actor( true ), d.get_conditionals(),
                     d.get_context() );
    result.cur_item = d.cur_item;
    result.reason = d.reason;
    result.by_radio = d.by_radio;
    return result;
}

template<class T>
static bool evaluate_stored_condition( const T &, const std::string & )
{
    return false;
}

inline bool evaluate_stored_condition( const dialogue &d, const std::string &key )
{
    return d.evaluate_conditional( key );
}
...
struct conditional_t {
    ...
    public:
        ...
        void set_query_tile( const JsonObject &jo, const std::string &member, bool is_npc = false );
        ...
        void set_map_ter_furn_with_flag( const JsonObject &jo, const std::string &member );
