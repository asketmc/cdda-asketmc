...
class basecamp
{
    public:
        ...
        // food utility
        /// Changes the faction larder by @p change and returns the nutrients actually consumed.
        /// A negative calorie-only change consumes vitamins in the same proportion as calories.
        nutrients camp_food_supply( nutrients &change );
        /// Convenience overload for a change expressed in kilocalories.
        nutrients camp_food_supply( int change );
        /// Converts a work duration to its larder cost and consumes it.
        nutrients camp_food_supply( time_duration work );
        /// Evenly feeds abstracted larder nutrients to a work party.
        void feed_workers( const std::vector<std::reference_wrapper<Character>> &workers,
        ...
        /// Takes all food from camp food zones and adds its nutrients to the faction larder.
        ...
        /** Whether @p guy may use this camp's stores (or only its public water). */
        bool allowed_access_by( const Character &guy, bool water_request = false ) const;
        ...
        inline const std::vector<tripoint_abs_ms> &get_liquid_dumping_spots() const {
        ...
        void start_crafting_with_ui( const mission_id &miss_id );
    ...
    private:
        ...
        // CAMP_STORAGE tiles whose terrain or furniture can contain liquid.
