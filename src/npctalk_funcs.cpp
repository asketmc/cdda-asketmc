...
void talk_function::goto_location( npc &p )
{
    ...
    for( auto elem : player_character.camps ) {
        ...
        if( !temp_camp->allowed_access_by( p ) ) {
            continue;
        }
...
void talk_function::bionic_install_allies( npc &p )
{
    ...
    if( rl_dist( p.pos(), patient->pos() ) > 1 ) {
        popup( _( "%s needs to stand next to %s for the operation." ), patient->get_name(),
               p.get_name() );
        return;
    }
...
void talk_function::npc_rules_menu( npc &p )
{
    show_follower_rules_ui( p );
}
