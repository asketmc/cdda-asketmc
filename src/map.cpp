        ...
        if( !recharge_part.removed && !recharge_part.is_broken() && recharge_part.enabled &&
            ...
            for( item &n : cur_veh.get_items( part ) ) {
                ...
                if( n.ammo_capacity( ammo_battery ) > n.ammo_remaining() ||
                    ...
                    cur_veh.recharge_epower_this_turn -= recharge_part.info().bonus;
...
basecamp &map::add_camp( const tripoint_abs_omt &omt_pos, const std::string &name,
                         bool need_validate )
{
    ...
    basecamp &new_camp = overmap_buffer.add_camp( temp_camp );
...
{
    if( !zlevels || !inbounds( p ) || p.z <= -OVERMAP_DEPTH ) {
        return true;
    }

    // 0.G ramps use Z_TRANSPARENT rather than the later transparent-floor
    // cache flag.  Keep their existing see-through rendering while using the
    // floor cache for ordinary floors.
                        ...
                        const const_maptile &curr_maptile, const drawsq_params &params ) const
{
    ...
    if( veh != nullptr ) {
        ...
        const int tiedown = veh->part_with_feature( veh_part, "FURNITURE_TIEDOWN", true );
        if( tiedown >= 0 ) {
            const furn_str_id furniture( veh->part( tiedown ).get_base().get_var(
                                             "tied_down_furniture" ) );
            if( furniture.is_valid() ) {
                sym = furniture->symbol();
                tercol = furniture->color();
            }
        }
...
int map::ledge_coverage( const Creature &viewer, const tripoint &target_p ) const
{
    ...
    // Eye level from the ground, in map-grid units.
    ...
    if( const furn_id viewer_furn = furn( viewer_p ) ) {
                         ...
                         const float &eye_level ) const
{
    ...
    // Only the first ledge is needed for this coverage calculation.
    ...
    const float covered_z = abs_eye_z + eye_ledge_z_delta / dist_to_ledge_base * flat_dist;
    float coverage = ( covered_z - target_p.z * zlevel_to_grid_ratio ) * 100.0f;

    // #72293: avoid furniture lookups when the ledge cannot cover the target.
    if( coverage < 0.0f ) {
        return 0;
    }
    if( const furn_id target_furn = furn( target_p ) ) {
        coverage -= target_furn->coverage;
    }

    return static_cast<int>( std::max( coverage, 0.0f ) );
}
...
void map::build_map_cache( const int zlev, bool skip_lightmap )
{
    ...
    for( int z = minz; z <= maxz; z++ ) {
        // Remote levels only affect the active seen cache while 3D FOV is enabled.
