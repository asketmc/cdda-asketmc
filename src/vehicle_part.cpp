...
std::string vehicle_part::name( bool with_prefix ) const
{
    ...
    if( base.has_var( "tied_down_furniture" ) ) {
        const furn_str_id stored_furniture( base.get_var( "tied_down_furniture" ) );
...
{
    const cata::optional<vpart_reference> loader = part_with_feature( "FURNITURE_TIEDOWN", true );
    if( !loader ) {
        return false;
    }
    if( !loader->vehicle().get_items( static_cast<int>( loader->part_index() ) ).empty() ) {
        return false;
    }
...
{
    const cata::optional<vpart_reference> loader = part_with_feature( "FURNITURE_TIEDOWN", true );
    return loader && loader->part().get_base().has_var( "tied_down_furniture" );
}

void vehicle_part::load_furniture( map &here, const tripoint &from )
{
    if( base.has_var( "tied_down_furniture" ) || !here.has_furn( from ) ) {
        return;
    }

    base.set_var( "tied_down_furniture", here.furn( from ).obj().id.str() );
    here.furn_clear( from );
}

void vehicle_part::unload_furniture( map &here, const tripoint &to )
{
    if( !base.has_var( "tied_down_furniture" ) || here.has_furn( to ) ) {
        return;
    }

    const furn_str_id carried_furniture( base.get_var( "tied_down_furniture" ) );
    if( !carried_furniture.is_valid() ) {
        debugmsg( "Invalid carried furniture %s", carried_furniture.str() );
        return;
    }

    base.erase_var( "tied_down_furniture" );
    here.furn_set( to, carried_furniture );
}
...
{
    const bool was_unavailable = pt.is_unavailable();
    ...
    if( was_unavailable != pt.is_unavailable() ) {
...
bool vehicle::mod_hp( vehicle_part &pt, int qty, damage_type dt )
{
    ...
    if( dur > 0 ) {
        const bool was_unavailable = pt.is_unavailable();
        const bool destroyed = pt.base.mod_damage( -( pt.base.max_damage() * qty / dur ), dt );
        if( was_unavailable != pt.is_unavailable() ) {
            recalculate_enchantment_cache();
        }
        return destroyed;
