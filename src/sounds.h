...
struct sound_effect_choice {
    std::string id;
    std::string variant;
};

/** Ordered exact matches followed by the unconditional legacy fallback. */
std::vector<sound_effect_choice> gun_sound_candidates( const std::string &weapon_id,
        const std::string &ammo_type, const std::string &suffix,
        const std::string &legacy_suppressed_variant );

/** Size/skill fallback used when a soundpack has no exact melee-weapon sample. */
std::string melee_sound_fallback_variant( const std::string &skill, int weapon_volume );

/** Material family used for a melee impact. */
std::string melee_hit_sound_id( bool target_is_monster, const std::string &material );
...
void generate_melee_sound( const item &weapon, const tripoint &source,
                           const tripoint &target, bool hit,
