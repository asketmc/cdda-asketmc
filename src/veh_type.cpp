...
void vpart_info::load_toolkit( cata::optional<vpslot_toolkit> &tkptr, const JsonObject &jo )
{
...
void vpart_info::load( const JsonObject &jo, const std::string &src )
{
    ...
    if( jo.has_array( "enchantments" ) ) {
        int enchantment_number = 0;
        for( JsonValue value : jo.get_array( "enchantments" ) ) {
            std::string enchantment_name = "INLINE_ENCH_VPART_" + def.id.str() + "_" +
                                           std::to_string( enchantment_number++ );
            def.enchantments.push_back( enchantment::load_inline_enchantment(
                                            value, src, enchantment_name ) );
        }
    }
...
void vpart_info::finalize()
{
    ...
    for( auto &e : vpart_info_all ) {
        ...
        if( e.second.has_flag( VPFLAG_APPLIANCE ) ) {
            ...
            e.second.location = vpart_location_structure;
        }

        if( e.second.toolkit_info.has_value() ) {
            std::set<itype_id> &allowed = e.second.toolkit_info->allowed_types;
            for( auto iter = allowed.begin(); iter != allowed.end(); ) {
                if( !iter->is_valid() ) {
                    debugmsg( "invalid itype_id '%s' in allowed_tools of vpart '%s'",
                              iter->str(), e.first.str() );
                    iter = allowed.erase( iter );
                } else {
                    ++iter;
                }
            }
                                    ...
                                    int width ) const
{
    ...
    if( get_toolkit_info() && !get_toolkit_info()->allowed_types.empty() ) {
        append_desc( std::string( "\n" ) + _( "Allows connecting:" ) + " <color_cyan>" +
                     enumerate_as_string( get_toolkit_info()->allowed_types.begin(),
                                          get_toolkit_info()->allowed_types.end(),
        []( const itype_id & tool ) {
            return tool->nname( 1 );
        } ) + "</color>" );
    }
...
const cata::optional<vpslot_toolkit> &vpart_info::get_toolkit_info() const
{
    return toolkit_info;
}
...
void vehicle_prototype::load( const JsonObject &jo )
{
    ...
    const auto add_part_obj = [&]( const JsonObject & part, point pos ) {
        ...
        if( part.has_array( "tools" ) ) {
            for( const JsonValue tool_value : part.get_array( "tools" ) ) {
                part_def::tool_def tool;
                if( tool_value.test_string() ) {
                    // Backwards-compatible, compact form used by hand-written prototypes.
                    tool.type = itype_id( tool_value.get_string() );
                } else if( tool_value.test_object() ) {
                    // Item types are not finalized while prototypes are loaded.  Keep the JSON
                    // intact and deserialize it later in vehicle_prototype::finalize().
                    tool.state = tool_value.get_object().str();
                } else {
                    tool_value.throw_error( "vehicle prototype tool must be an item id or object" );
                }
                pt.tools.emplace_back( std::move( tool ) );
            }
        }
...
void vehicle_prototype::save_vehicle_as_prototype( const vehicle &veh, JsonOut &json )
{
    ...
    int mount_min_x = std::numeric_limits<int>::max();
    int mount_max_x = std::numeric_limits<int>::min();
    int mount_min_y = std::numeric_limits<int>::max();
    int mount_max_y = std::numeric_limits<int>::min();

    // Preserve structural parts first at each mount point, then sort the remaining parts below.
    for( const vpart_reference &vpr : veh.get_all_parts() ) {
        const vehicle_part &part = vpr.part();
        if( part.is_fake ) {
            continue;
        }
        mount_min_x = std::min( mount_min_x, part.mount.x );
        mount_max_x = std::max( mount_max_x, part.mount.x );
        mount_min_y = std::min( mount_min_y, part.mount.y );
        mount_max_y = std::max( mount_max_y, part.mount.y );
        if( part.info().location == vpart_location_structure ) {
            vp_map[part.mount].push_front( &part );
        } else {
            vp_map[part.mount].push_back( &part );
        }
    }

    // Work out a loadable part order before writing anything.  Some runtime-only parts (for
    // example attached cables) cannot legally occur in a prototype.  Failing before JsonOut is
    // touched lets write_to_file discard its temporary file instead of publishing truncated JSON.
    for( auto &vp_pos : vp_map ) {
        vehicle ordering_vehicle;
        std::list<const vehicle_part *>::iterator iter = vp_pos.second.begin();
        size_t failed_attempts = 0;
        while( iter != vp_pos.second.end() ) {
            const vehicle_part candidate = **iter;
            if( ordering_vehicle.can_mount( point_zero, candidate.info().get_id() ).success() ) {
                ordering_vehicle.install_part( point_zero, candidate );
                ++iter;
                failed_attempts = 0;
                continue;
            }

            const std::list<const vehicle_part *>::iterator next = std::next( iter );
            vp_pos.second.splice( vp_pos.second.end(), vp_pos.second, iter );
            iter = next;
            ++failed_attempts;
            if( failed_attempts >= static_cast<size_t>( std::distance( iter,
                    vp_pos.second.end() ) ) ) {
                throw std::runtime_error( string_format(
                                              "mount point (%d,%d) has an illegal vehicle part sequence",
    ...
    json.start_array();
    if( !vp_map.empty() ) {
        // Vehicle JSON blueprints are printed from the front toward the rear.
        for( int x = mount_max_x; x >= mount_min_x; --x ) {
            std::string row;
            for( int y = mount_min_y; y <= mount_max_y; ++y ) {
                const int displayed_part = veh.part_displayed_at( point( x, y ), false, true, true );
                row += displayed_part < 0 ? ' ' : veh.part_sym( displayed_part, true, false );
            }
            json.write( row );
        }
    }
    ...
    json.member( "parts" );
    json.start_array();
    const auto print_vpart_with_variant = [&json]( const vehicle_part & part ) {
        const vpart_info &part_info = part.info();
        if( part.variant.empty() ) {
            json.write( part_info.get_id().str() );
        } else {
            json.write( part_info.get_id().str() + "#" + part.variant );
        }
    };

    for( const auto &vp_pos : vp_map ) {
        json.start_object();
        json.member( "x", vp_pos.first.x );
        json.member( "y", vp_pos.first.y );
        json.member( "parts" );
        json.start_array();
        for( const vehicle_part *part : vp_pos.second ) {
            const bool has_fuel = part->is_tank() && part->ammo_remaining() > 0;
            const bool has_turret_ammo = part->is_turret() && part->ammo_remaining() > 0;
            const std::vector<item> &attached_tools = veh.get_tools( *part );
            if( has_fuel || has_turret_ammo || !attached_tools.empty() ) {
                json.start_object();
                json.member( "part" );
                print_vpart_with_variant( *part );
                if( has_fuel ) {
                    json.member( "fuel", part->ammo_current().str() );
                } else if( has_turret_ammo ) {
                    json.member( "ammo", 100 );
                    json.member( "ammo_types", part->ammo_current().str() );
                    json.member( "ammo_qty" );
                    json.start_array();
                    json.write( part->ammo_remaining() );
                    json.write( part->ammo_remaining() );
                    json.end_array();
                }
                if( !attached_tools.empty() ) {
                    json.member( "tools" );
                    json.start_array();
                    for( const item &tool : attached_tools ) {
                        json.write( tool );
                    }
                    json.end_array();
                }
    ...
    // Vehicle prototype item spawns cannot represent nested item state, so only export top-level IDs.
    json.member( "items" );
    json.start_array();
    for( const vpart_reference &vp : veh.get_any_parts( "CARGO" ) ) {
        ...
        const vehicle_stack stack = veh.get_items( static_cast<int>( vp.part_index() ) );
        if( stack.empty() ) {
            continue;
        }
        json.start_object();
    ...
    json.member( "zones" );
    json.start_array();
    for( const auto &zone : veh.loot_zones ) {
        json.start_object();
        json.member( "x", zone.first.x );
        json.member( "y", zone.first.y );
        json.member( "type", zone.second.get_type().str() );
        json.end_object();
    }
    json.end_array();
    json.end_object();
    ...
    for( auto &vp : vtypes ) {
        ...
        for( part_def &pt : proto.parts ) {
            ...
            if( part_idx < 0 ) {
                ...
                vehicle_part &installed_part = blueprint.part( part_idx );
                for( const part_def::tool_def &tool : pt.tools ) {
                    if( tool.state.empty() ) {
                        blueprint.get_tools( installed_part ).emplace_back( tool.type,
                                calendar::turn_zero );
                    } else {
                        item restored;
                        restored.deserialize( json_loader::from_string( tool.state ).get_object() );
                        blueprint.get_tools( installed_part ).emplace_back( std::move( restored ) );
                    }
                }
            }
