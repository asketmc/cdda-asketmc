...
bool avatar::wield( item &target, const int obtain_cost )
{
    ...
    // EVENT EOCs may remove the newly wielded item.  Dispatch only after every
    // caller-side use of the item_location has completed.
    cata::event event = cata::event::make<event_type::character_wields_item>( getID(), last_item );
...
void avatar::export_as_npc( const cata_path &path )
{
    ...
    try {
        ...
        // Export performs file I/O after moving the live avatar into shadow_npc.
        // Always restore the avatar before propagating an export failure.
        swap_character( get_shadow_npc() );
        throw;
    }
    swap_character( get_shadow_npc() );
