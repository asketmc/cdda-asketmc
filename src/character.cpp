...
float Character::fine_detail_vision_mod( const tripoint &p ) const
{
    ...
    // 0.G uses tripoint_zero as the default-position sentinel, while the backported
    // NPC crafting API passes tripoint_min.  Treat both as "this character's tile"
    // so all crafting and reading paths consult the same calculated lightmap.
    const tripoint &check_p = p == tripoint_zero || p == tripoint_min ? pos() : p;
...
float Character::get_skill_level_with_progress( const skill_id &ident ) const
{
    return enchantment_cache->modify_value( ident, _skills->get_skill_level( ident ) +
                                            _skills->get_progress_level( ident ) );
}

float Character::get_skill_level_with_progress( const skill_id &ident,
        const item &context ) const
{
    return enchantment_cache->modify_value( ident, _skills->get_skill_level( ident, context ) +
                                            _skills->get_progress_level( ident, context ) );
}
...
void Character::prevent_death()
{
    for( const bodypart_id &bp : get_all_body_parts( get_body_part_flags::only_main ) ) {
        if( bp->is_vital && get_part_hp_cur( bp ) <= 0 ) {
...
{
    // Weight now modifies effective lifestyle directly instead of imposing a
    // much harsher cap on every positive daily-health contribution.
    const float bmi = get_bmi();
    const int over_factor = std::round( std::max(
                                0.0f, 5 * ( bmi - character_weight_category::obese ) ) );
    const int under_factor = std::round( std::max(
                                 0.0f, 50 * ( character_weight_category::normal - bmi ) ) );
                                   ...
                                   int charges_in_it, bool bulk_cost ) const
{
    ...
    // Hand encumbrance is setup overhead. Charge it for the first operation,
    // but not repeatedly for a contiguous bulk transfer.
    if( !bulk_cost ) {
                                ...
                                int base_cost ) const
{
    ...
    const float lvl = get_skill_level_with_progress(
                          it.is_gun() ? it.gun_skill() : it.melee_skill() );
                        ...
                        float /*difficulty*/, dealt_projectile_attack const *const proj )
{
    ...
    const optional_vpart_position veh_part = get_map().veh_at( pos() );
...
void Character::assign_activity( const player_activity &act, bool allow_resume, bool suspend_current )
{
    ++activity_generation;
    bool resuming = false;
    ...
    if( it.is_container() ) {
        ...
        for( item_pocket::pocket_type ptype : {
            ...
            for( item *contained : it.all_items_top( ptype, true ) ) {
                if( previous != nullptr && previous->stacks_with( *contained ) ) {
                    moves += std::max( item_handling_cost( *contained, true, 0, -1, true ), 1 );
                } else {
                    moves += item_handling_cost( *contained );
                }
                previous = contained;
            }
                                ...
                                int base_cost )
{
    ...
    cata::event event = cata::event::make<event_type::character_wields_item>( getID(), weapon.typeId() );
    get_event_bus().send_with_talker( this, &wielded, event );
