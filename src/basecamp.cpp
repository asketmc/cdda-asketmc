                            ...
                            bool player_founded )
{
    ...
    if( om_cur.find( base_camps::prefix ) == std::string::npos ) {
        ...
        if( player_founded ) {
            ...
            const oter_id bcid( direction.empty() ? "faction_base_camp_0" :
...
std::unordered_set<recipe_id> basecamp::recipe_deck_all( const inventory *supplies ) const
{
    ...
    for( const std::pair<const point, expansion_data> &expansion : expansions ) {
        for( const std::pair<const std::string, int> &provided : expansion.second.provides ) {
            for( const std::pair<const recipe_id, translation> &camp_recipe :
                 recipe_group::get_recipes_by_id( provided.first ) ) {
                recipes.insert( camp_recipe.first );
            }
        }
    }
    return recipes;
...
{
    faction *owner_faction = g->faction_manager_ptr->get( owner, false );
    if( owner_faction != nullptr ) {
        return owner_faction;
    }

    // Camps from older saves have no owner.  Deserialization normally migrates
    // them, but retain a safe fallback for camps created by legacy code paths.
    return get_player_character().get_faction();
}

void basecamp::set_owner( const faction_id &new_owner )
{
    // Faction instances are created lazily, so validity cannot be required here.
    owner = new_owner;
}

faction_id basecamp::get_owner() const
{
    return owner;
}

faction *basecamp::get_owner_faction() const
{
    return fac();
}

bool basecamp::allowed_access_by( const Character &guy, bool water_request ) const
{
    faction *owner_faction = fac();
    faction *visitor_faction = guy.get_faction();
    if( owner_faction == nullptr || visitor_faction == nullptr ) {
        return false;
    }
    if( owner_faction == visitor_faction ) {
        return true;
    }
    if( owner_faction->has_relationship( visitor_faction->id,
                                         npc_factions::share_my_stuff ) ) {
        return true;
    }
    return water_request && owner_faction->has_relationship(
               visitor_faction->id, npc_factions::share_public_goods );
}
...
bool basecamp::form_storage_zones( map &here, const tripoint_abs_ms &abspos )
{
    ...
    tripoint src_loc = here.getlocal( abspos );
    bool found_storage_tile = false;
    ...
    if( mgr.has_near( zone_type_CAMP_STORAGE, abspos, 60 ) ) {
        const std::unordered_set<tripoint_abs_ms> &storage_tiles =
            mgr.get_near( zone_type_CAMP_STORAGE, abspos, 60 );
        const std::vector<tripoint_abs_ms> sorted_tiles =
            get_sorted_tiles_by_distance( abspos, storage_tiles );
        for( const tripoint_abs_ms &tile : sorted_tiles ) {
            const tripoint local_tile = here.getlocal( tile );
            if( !here.inbounds( local_tile ) ) {
                continue;
            }
            if( !found_storage_tile ) {
                src_loc = local_tile;
                found_storage_tile = true;
            }
            if( here.has_flag_ter_or_furn( ter_furn_flag::TFLAG_LIQUIDCONT, local_tile ) ) {
                possible_liquid_dumps.push_back( tile );
            }
        }
    }

    set_dumping_spot( here.getglobal( src_loc ) );
    set_liquid_dumping_spots( possible_liquid_dumps );
    return found_storage_tile;
}
