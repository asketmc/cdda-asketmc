...
class vpart_info
{
    public:
        ...
        static void load_toolkit( cata::optional<vpslot_toolkit> &tkptr, const JsonObject &jo );
        ...
        */
        ...
        const cata::optional<vpslot_toolkit> &get_toolkit_info() const;

        const std::vector<enchantment_id> &get_enchantments() const {
            return enchantments;
        }
    ...
    private:
        ...
        cata::optional<vpslot_toolkit> toolkit_info;
...
struct vehicle_prototype {
    struct part_def {
        struct tool_def {
            ...
            // Full serialized item state for newly exported prototypes.  Empty for the legacy
            // string-only form, which remains supported for hand-written vehicle JSON.
            std::string state;
        };
