...
void pocket_consumption_entry::deserialize( const JsonObject &jo )
{
    ...
    if( qty < 1 ) {
        jo.throw_error_at( "qty", "multi-pocket consumption qty must be at least 1" );
    }
}

static std::vector<pocket_consumption_entry> read_consumption_entries( const JsonValue &value )
{
    if( !value.test_array() ) {
        value.throw_error( "multi-pocket consumption entries must be an array" );
    }
    std::vector<pocket_consumption_entry> result;
    for( const JsonValue &element : value.get_array() ) {
        if( !element.test_object() ) {
            element.throw_error( "multi-pocket consumption entry must be an object" );
        }
        pocket_consumption_entry entry;
        JsonObject entry_json = element.get_object();
        entry.deserialize( entry_json );
        result.push_back( entry );
    }
    if( result.empty() ) {
        value.throw_error( "multi-pocket consumption entries must not be empty" );
    }
    return result;
}

void firing_requirement_set::deserialize_firing_requirements( const JsonObject &jo,
        const std::string &member )
{
    per_mode.clear();
    JsonObject modes = jo.get_object( member );
    for( const JsonMember mode : modes ) {
        const std::string mode_name = mode.name();
        if( mode_name.compare( 0, 2, "//" ) == 0 ) {
            continue;
        }
        per_mode[gun_mode_id( mode_name )] = read_consumption_entries( mode );
    }
    modes.allow_omitted_members();
    if( per_mode.empty() ) {
        jo.throw_error_at( member, "firing_requirements must define at least one mode" );
    }
}

void firing_requirement_set::deserialize_consumption_per_use( const JsonObject &jo,
        const std::string &member )
{
    per_mode.clear();
...
void Item_factory::check_definitions() const
{
    ...
    for( const auto &elem : m_templates ) {
        ...
        if( mag_pocket_number > 1 && type->firing_requirements.empty() ) {
            msg += "cannot have more than one pocket that handles ammo (MAGAZINE or MAGAZINE_WELL)\n";
        }

        std::set<std::string> pocket_ids;
        for( const pocket_data &data : type->pockets ) {
            if( !data.pocket_id.empty() && !pocket_ids.insert( data.pocket_id ).second ) {
                msg += string_format( "duplicate pocket id \"%s\"\n", data.pocket_id );
            }
        }
        if( !type->firing_requirements.empty() ) {
            if( !type->gun && !type->tool ) {
                msg += "multi-pocket consumption requires a gun or tool\n";
            }
            if( type->gun && type->gun->ammo_to_fire != 1 ) {
                msg += "firing_requirements cannot be combined with non-default ammo_to_fire\n";
            }
            if( type->tool && type->tool->charges_per_use != 0 ) {
                msg += "consumption_per_use cannot be combined with charges_per_use\n";
            }
            for( const std::pair<const gun_mode_id, std::vector<pocket_consumption_entry>> &mode :
                 type->firing_requirements.per_mode ) {
                if( type->gun && !type->gun->modes.count( mode.first ) ) {
                    msg += string_format( "firing_requirements references unknown mode \"%s\"\n",
                                          mode.first.c_str() );
                }
                for( const pocket_consumption_entry &entry : mode.second ) {
                    const auto found = std::find_if( type->pockets.begin(), type->pockets.end(),
                    [&entry]( const pocket_data & data ) {
                        return data.pocket_id == entry.pocket;
                    } );
                    if( found == type->pockets.end() ) {
                        msg += string_format( "multi-pocket consumption references missing pocket \"%s\"\n",
                                              entry.pocket );
                    } else if( found->type != item_pocket::pocket_type::MAGAZINE &&
                               found->type != item_pocket::pocket_type::MAGAZINE_WELL ) {
                        msg += string_format( "multi-pocket consumption pocket \"%s\" is not ammunition storage\n",
                                              entry.pocket );
                    }
                }
            }
...
void Item_factory::load_basic_info( const JsonObject &jo, itype &def, const std::string &src )
{
    ...
    if( jo.has_member( "firing_requirements" ) ) {
        def.firing_requirements.deserialize_firing_requirements( jo, "firing_requirements" );
    }
    if( jo.has_member( "consumption_per_use" ) ) {
        if( jo.has_member( "firing_requirements" ) ) {
            jo.throw_error( "an item cannot define both firing_requirements and consumption_per_use" );
        }
        def.firing_requirements.deserialize_consumption_per_use( jo, "consumption_per_use" );
    }
