...
static bool safe_transform_can_reach_target( const Character &guy, const trait_id &switched,
        const trait_id &target )
{
    const mutation_branch &target_data = target.obj();
    const auto has_after_source_removed = [&]( const trait_id & candidate ) {
        return candidate != switched && guy.has_trait( candidate );
    };
    const auto has_any = [&]( const std::vector<trait_id> & candidates ) {
        return std::any_of( candidates.begin(), candidates.end(), has_after_source_removed );
    };

    // mutate_towards would perform a different intermediate operation or reject
    // these targets.  Reject before mutation effects can destroy or displace gear.
    if( guy.has_child_flag( target ) || target_data.threshold || target_data.profession ||
        target_data.dummy || ( !target_data.prereqs.empty() && !has_any( target_data.prereqs ) ) ||
        ( !target_data.prereqs2.empty() && !has_any( target_data.prereqs2 ) ) ||
        ( !target_data.threshreq.empty() && !has_any( target_data.threshreq ) ) ) {
        return false;
    }

    for( const bionic_id &bid : guy.get_bionics() ) {
        if( bid->mutation_conflicts.count( target ) != 0 ||
            std::find( bid->canceled_mutations.begin(), bid->canceled_mutations.end(), target ) !=
            bid->canceled_mutations.end() ) {
            return false;
        }
    }

    std::vector<trait_id> conflicts = target_data.cancels;
    const std::vector<trait_id> all_prereqs = get_all_mutation_prereqs( target );
    for( const trait_id &same_type : get_mutations_in_types( target_data.types ) ) {
        if( std::find( all_prereqs.begin(), all_prereqs.end(), same_type ) == all_prereqs.end() ) {
            conflicts.push_back( same_type );
        }
    }
    for( const trait_id &conflict : conflicts ) {
        if( has_after_source_removed( conflict ) && !guy.purifiable( conflict ) ) {
            return false;
        }
    }
    return true;
...
bool Character::switch_mutations( const trait_id &switched, const trait_id &target,
                                  bool start_powered, bool safe )
{
    if( safe && !safe_transform_can_reach_target( *this, switched, target ) ) {
        return false;
    }
    const std::unordered_map<trait_id, trait_data> previous_mutations = my_mutations;
    ...
    if( !has_trait( target ) ) {
        // mutate_towards may fail or add only a prerequisite.  Restore the
        // complete pre-transform mutation state instead of consuming the source.
        std::vector<trait_id> added_mutations;
        for( const std::pair<const trait_id, trait_data> &entry : my_mutations ) {
            if( previous_mutations.count( entry.first ) == 0 ) {
                added_mutations.push_back( entry.first );
            }
        }
        for( const trait_id &added : added_mutations ) {
            unset_mutation( added );
        }
        for( const std::pair<const trait_id, trait_data> &entry : previous_mutations ) {
            if( !has_trait( entry.first ) ) {
                set_mutation( entry.first, entry.second.variant );
            }
            my_mutations[entry.first] = entry.second;
        }
        do_mutation_updates();
        return false;
    }
...
void Character::activate_mutation( const trait_id &mut )
{
    ...
    if( !mut->activated_eocs.empty() ) {
        ...
        // Activation EOCs are one-shot unless they explicitly set up a processed mutation.
        tdata.powered = mut->activated_is_setup;
    }

    if( mdata.transform ) {
        const cata::value_ptr<mut_transform> trans = mdata.transform;
        if( switch_mutations( mut, trans->target, trans->active, trans->safe ) ) {
