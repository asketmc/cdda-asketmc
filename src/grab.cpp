...
bool game::grabbed_veh_move_stairs( const tripoint &dp )
{
    ...
    if( !grabbed_vehicle->handle_potential_theft( get_avatar() ) ) {
        return false;
    }
    ...
    if( dp.z > 0 && u.get_arm_str() < max_str_req ) {
    ...
    point maximums;
    for( const vpart_reference &vpr : grabbed_vehicle->get_all_parts() ) {
        const point mount = vpr.mount();
        if( first_part ) {
            minimums = maximums = mount;
            first_part = false;
        } else {
            minimums.x = std::min( minimums.x, mount.x );
            minimums.y = std::min( minimums.y, mount.y );
            maximums.x = std::max( maximums.x, mount.x );
            maximums.y = std::max( maximums.y, mount.y );
        }
    }
    if( maximums.x > minimums.x && maximums.y > minimums.y ) {
        add_msg( _( "The %s is too wide to get through the stairs." ),
                 grabbed_vehicle->disp_name() );
        u.grab( object_type::NONE );
        return true;
    }

    const tripoint target_relative = u.grab_point + dp;
    const point away_step( ( target_relative.x > 0 ) - ( target_relative.x < 0 ),
                           ( target_relative.y > 0 ) - ( target_relative.y < 0 ) );
    if( away_step == point_zero ) {
        add_msg( _( "You need to push the %s away from the stairs." ),
                 grabbed_vehicle->disp_name() );
        u.grab( object_type::NONE );
        return true;
    }

    tripoint final_dp = dp;
    const auto overlaps_player = [&]() {
        for( const tripoint &p : grabbed_vehicle->get_points( true ) ) {
            if( p + final_dp == u.pos() ) {
                return true;
            }
        }
    ...
    for( size_t offset = 0; overlaps_player() &&
         offset <= static_cast<size_t>( grabbed_vehicle->num_parts() ); ++offset ) {
        final_dp += tripoint( away_step, 0 );
    }
    if( overlaps_player() ) {
        add_msg( _( "There is no room to drag the %s through the stairs." ),
                 grabbed_vehicle->disp_name() );
        u.grab( object_type::NONE );
        return true;
    }
    ...
    for( const tripoint &p : grabbed_vehicle->get_points( true ) ) {
        const tripoint destination = p + final_dp;
        const optional_vpart_position other_vehicle = m.veh_at( destination );
        if( m.impassable( destination ) ||
            get_creature_tracker().creature_at<Creature>( destination ) != nullptr ||
            ( other_vehicle && &other_vehicle->vehicle() != grabbed_vehicle ) ) {
            add_msg( _( "The %s will not fit through the stairs." ), grabbed_vehicle->disp_name() );
            u.grab( object_type::NONE );
            return true;
        }
    }

    if( !m.displace_vehicle( *grabbed_vehicle, final_dp ) ) {
        add_msg( _( "You fail to drag the %s through the stairs." ), grabbed_vehicle->disp_name() );
        u.grab( object_type::NONE );
        return true;
    }
...
bool game::grabbed_veh_move( const tripoint &dp )
{
    ...
    // Vehicle movement: strength check.  Wheels and traction make heavy
    // vehicles practical to move, while dragging the same mass still uses the
    // deliberately severe worst-case requirement.
    int mc = 0;
    const int max_str_req = grabbed_vehicle->total_mass() / 10_kilogram;
    int str_req = 0;
    ...
    if( grabbed_vehicle->valid_wheel_config() ) {
        ...
        for( int p : wheel_indices ) {
            ...
            // Multiply first so a light vehicle with several wheels does not
            // lose all terrain cost to integer truncation.
        ...
        str_req /= grabbed_vehicle->k_traction( m.vehicle_wheel_traction( *grabbed_vehicle ) );
        // If rolling is worse than dragging on this terrain, use the bounded
        // no-wheel requirement instead.
