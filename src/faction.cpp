    ...
    , wealth( jsobj.get_int( "wealth" ) )
{
    ...
    if( jsobj.has_object( "fac_food_supply" ) ) {
        jsobj.read( "fac_food_supply", food_supply, true );
    } else {
        // Legacy faction JSON stores kilocalories as a single integer.
        food_supply.calories = jsobj.get_int( "food_supply", 0 ) * 1000;
    }
            ...
            if( !elem.second.validated ) {
                for( const faction_template &fac_temp : npc_factions::all_templates ) {
                    if( fac_temp.id == id ) {
                        ...
                        elem.second.consumes_food = fac_temp.consumes_food;
