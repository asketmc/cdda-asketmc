...
float Character::get_hit_weapon( const item &weap ) const
{
    ...
    float skill = get_skill_level_with_progress( weap.melee_skill() );
    ...
    return ( skill / 3.0f ) + ( get_skill_level_with_progress( skill_melee ) / 2.0f ) +
           weap.type->m_to_hit;
}
                                       ...
                                       bool allow_unarmed )
{
    ...
    if( monster *m = t.as_monster() ) {
        cata::event event = cata::event::make<event_type::character_melee_attacks_monster>(
                                getID(), cur_weap.typeId(), hits, m->type->id );
        get_event_bus().send_with_talker( this, m, event );
    } else if( Character *c = t.as_character() ) {
        cata::event event = cata::event::make<event_type::character_melee_attacks_character>(
    ...
    if( !hits ) {
        ...
        sfx::generate_melee_sound( cur_weap, pos(), t.pos(), false, false );
        ...
        if( !t.is_dead_state() ) {
            ...
            sfx::generate_melee_sound( cur_weap, pos(), t.pos(), true, t.is_monster(), material );
...
double Character::crit_chance( float roll_hit, float target_dodge, const item &weap ) const
{
    ...
    if( weap.is_null() ) {
        ...
        weapon_crit_chance = 0.5 + 0.05 * get_skill_level_with_progress( skill_unarmed );
    }
...
int Character::attack_speed( const item &weap ) const
{
    ...
    const float melee_skill = has_active_bionic( bionic_id( bio_cqb ) ) ? BIO_CQB_LEVEL :
                              get_skill_level_with_progress( skill_melee );
