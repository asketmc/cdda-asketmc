...
{
    if( myorigin->part( part_num ).get_base().has_var( "tied_down_furniture" ) ) {
        return max_volume();
    }
    return item_stack::stored_volume();
}
                     ...
                     float percent_of_parts_to_affect, point damage_origin, float damage_size )
{
    for( vehicle_part &part : parts ) {
        ...
        for( int &square_part_index : parts_in_square ) {
            if( part_info( square_part_index ).location == vpart_location_structure ) {
                structures_found++;
            }
        }
    ...
    for( int p = static_cast<int>( parts.size() ) - 1; p >= 0; p-- ) {
        ...
        for( int other_i = static_cast<int>( parts_here.size() ) - 1; other_i >= 0; other_i -- ) {
            ...
            if( p_info.get_id() == other_p_info.get_id() ||
                ( !p_info.location.is_empty() && p_info.location == other_p_info.location ) ) {
 ...
 */
ret_val<void> vehicle::can_mount( const point &dp, const vpart_id &id,
                                  const bool allow_noinstall ) const
{
    ...
    if( part.has_flag( "NOINSTALL" ) && !allow_noinstall ) {
    ...
    if( parts_in_square.empty() &&  part.location != vpart_location_structure &&
    ...
    for( const int &elem : parts_in_square ) {
        ...
        if( part.get_id() == other_part.get_id() ||
            ( !part.location.is_empty() && part.location == other_part.location ) ) {
            return ret_val<void>::make_failure(
                       _( "The installed %1$s part shares the %2$s location." ), other_part.name(),
                       other_part.location->name.translated() );
        }
...
bool vehicle::can_unmount( const int p, std::string &reason ) const
{
    ...
    if( vp_to_remove.info().location != vpart_location_structure ) {
    ...
    for( const int &elem : parts_here ) {
        if( part_info( elem ).location != vpart_location_structure ) {
...
{
    enchantment_cache = enchant_cache();

    for( const std::pair<const efftype_id, effect> &entry : effects ) {
        for( const enchantment_id &enchantment_ident : entry.first->enchantments ) {
            const enchantment &ench = enchantment_ident.obj();
            if( ench.is_active( *this, true ) && ench.is_vehicle_relevant() ) {
                enchantment_cache.force_add( ench, *this );
            }
        }
    }

    for( const vehicle_part &part_entry : parts ) {
        if( part_entry.removed || part_entry.is_unavailable() ) {
            continue;
        }
        for( const enchantment_id &enchantment_ident : part_entry.info().get_enchantments() ) {
            const enchantment &ench = enchantment_ident.obj();
            if( ench.is_active( *this, part_entry.enabled ) && ench.is_vehicle_relevant() ) {
                enchantment_cache.force_add( ench, *this );
            }
        }
    }
...
void vehicle::add_effect( const effect_source &source, const efftype_id &effect_ident,
                          const time_duration &duration, bool permanent, int intensity )
{
    if( !effect_ident.is_valid() ) {
        debugmsg( "Invalid effect, ID: %s", effect_ident.c_str() );
        return;
    }

    for( const std::pair<const efftype_id, effect> &entry : effects ) {
        for( const efftype_id &blocked_effect : entry.second.get_blocks_effects() ) {
            if( blocked_effect == effect_ident ) {
                return;
            }
        }
    }

    const effect_type &type = effect_ident.obj();
    std::map<efftype_id, effect>::iterator found = effects.find( effect_ident );
    if( found != effects.end() ) {
        effect &existing = found->second;
        existing.mod_duration( duration * existing.get_dur_add_perc() / 100 );
        if( permanent ) {
            existing.pause_effect();
        }
        if( existing.get_int_dur_factor() <= 0_turns ) {
            if( intensity > 0 ) {
                existing.set_intensity( intensity );
            } else if( existing.get_int_add_val() != 0 ) {
                existing.mod_intensity( existing.get_int_add_val() );
            }
        }
    } else {
        effect added( effect_source( source ), &type, duration, bodypart_str_id::NULL_ID(), permanent,
                      intensity, calendar::turn );
        if( added.get_int_dur_factor() != 0_turns ) {
            added.set_intensity( added.get_duration() / added.get_int_dur_factor() + 1 );
        }
        if( added.get_intensity() < 1 ) {
            added.set_intensity( 1 );
        } else if( added.get_intensity() > added.get_max_intensity() ) {
            added.set_intensity( added.get_max_intensity() );
        }
        effects.emplace( effect_ident, added );
    }
    recalculate_enchantment_cache();
}
...
{
    std::vector<efftype_id> remove_ids;
    std::vector<bodypart_id> remove_bodyparts;
    for( std::pair<const efftype_id, effect> &entry : effects ) {
        for( const efftype_id &removed_effect : entry.second.get_removes_effects() ) {
            remove_ids.push_back( removed_effect );
            remove_bodyparts.push_back( bodypart_str_id::NULL_ID() );
        }
        entry.second.decay( remove_ids, remove_bodyparts, calendar::turn, false );
    }
    for( const efftype_id &effect_ident : remove_ids ) {
        effects.erase( effect_ident );
    }
    recalculate_enchantment_cache();
}

void vehicle::remove_effect( const efftype_id &effect_ident )
{
    effects.erase( effect_ident );
    recalculate_enchantment_cache();
}

bool vehicle::has_effect( const efftype_id &effect_ident ) const
{
    return effects.count( effect_ident ) != 0;
}

bool vehicle::has_visible_effect() const
{
    return std::any_of( effects.begin(), effects.end(), []( const std::pair<const efftype_id, effect> &entry ) {
        return entry.first->is_show_in_info();
    } );
}

std::vector<std::reference_wrapper<const effect>> vehicle::get_effects() const
{
    std::vector<std::reference_wrapper<const effect>> result;
...
bool vehicle::remove_part( const int p, RemovePartHandler &handler )
{
    ...
    if( !magic ) {
        for( const item &tool : parts[p].tools ) {
            handler.add_item_or_charges( part_loc, tool, false );
        }
    }
    parts[p].tools.clear();
                                      ...
                                      const std::vector<int> &racks )
{
    ...
    if( split_vehicles( here, { carried_parts }, { new_vehicle }, { new_mounts } ) ) {
        ...
        new_vehicle->recalculate_enchantment_cache();
        ...
        while( !search_queue.empty() ) {
            ...
            for( const point &offset : four_adjacent_offsets ) {
                ...
                for( int p : all_neighbor_parts ) {
                    ...
                    if( part_info( p ).location == vpart_location_structure ) {
...
cata::optional<vpart_reference> vpart_position::part_with_tool( const itype_id &tool_type ) const
{
    for( const int idx : vehicle().parts_at_relative( mount(), false ) ) {
        ...
        if( std::any_of( tools.begin(), tools.end(), [&tool_type]( const auto & entry ) {
        return entry.first.typeId() == tool_type;
        } ) ) {
            return vp;
        }
    }
...
{
    std::map<item, input_event> tools;
    for( const int part_idx : this->vehicle().parts_at_relative( this->mount(), false ) ) {
        const vehicle_part &vp = this->vehicle().part( part_idx );
        if( vp.is_broken() ) {
            continue;
        }
        const std::map<item, input_event> part_tools = this->vehicle().prepare_tools( vp );
        tools.insert( part_tools.begin(), part_tools.end() );
    }
    return tools;
}
...
std::map<item, input_event> optional_vpart_position::get_tools() const
{
    return has_value() ? value().get_tools() : std::map<item, input_event>();
}
                                ...
                                bool roof ) const
{
    ...
    for( size_t index = 0; index < parts_in_square.size(); index++ ) {
        ...
        int test_z_order = part_info( test_index ).location->z_order;
...
int vehicle::roof_at_part( const int part ) const
{
    ...
    for( const int p : parts_in_square ) {
        if( part_info( p ).location == vpart_location_on_roof || part_flag( p, "ROOF" ) ) {
            return p;
        }
    }
...
{
    return connected_battery_power_level().first > 0;
}
...
void vehicle::idle( bool on_map )
{
    ...
    // Mounted energy weapons and other active turret items need normal item processing so
    // heat can dissipate.  Only send status messages when the player is at controls or beside it.
    const bool player_at_controls = !get_parts_at( player_character.pos(), "CONTROLS",
...
bool vehicle::has_available_electric_engine() const
{
    for( int i = 0; i < static_cast<int>( engines.size() ); ++i ) {
        if( is_engine_type( i, fuel_type_battery ) &&
...
{
    const auto force_controller_off = []() {
        const std::string option = get_option<std::string>(
                                       "FORCE_SMART_CONTROLLER_OFF_ON_ENGINE_STOP" );
        if( option == "enabled" ) {
            return true;
        }
        if( option == "disabled" ) {
        ...
        if( option != "ask" ) {
            debugmsg( "Unexpected option \"%s\" for FORCE_SMART_CONTROLLER_OFF_ON_ENGINE_STOP", option );
        }
        return query_yn( _( "The smart controller may restart this engine.  Disable the controller?" ) );
    };

    if( has_enabled_smart_controller && !has_available_electric_engine() && force_controller_off() ) {
...
std::vector<item> &vehicle::get_tools( vehicle_part &part )
{
    return part.tools;
}

const std::vector<item> &vehicle::get_tools( const vehicle_part &part ) const
{
    return part.tools;
}

std::map<item, input_event> vehicle::prepare_tools( const vehicle_part &part ) const
{
    std::map<item, input_event> result;
    for( const std::pair<itype_id, int> &entry : part.info().get_pseudo_tools() ) {
        item tool( entry.first, calendar::turn );
        prepare_tool( tool );
        const input_event hotkey = entry.second > 0
                                   ? input_event( entry.second, input_event_t::keyboard_char )
                                   : input_event( -1, input_event_t::error );
        result.emplace( std::move( tool ), hotkey );
    }
    for( const item &attached : part.tools ) {
        item tool( attached );
        prepare_tool( tool );
        const input_event hotkey = attached.invlet > 0
                                   ? input_event( attached.invlet, input_event_t::keyboard_char )
                                   : input_event();
        result.emplace( std::move( tool ), hotkey );
    }
    return result;
...
void vehicle::dump_items_from_part( const size_t index )
{
    ...
    vp.items.clear();
    for( item &tool : vp.tools ) {
        here.add_item_or_charges( global_part_pos3( vp ), tool );
    }
    vp.tools.clear();
}
...
void vehicle::refresh( const bool remove_fakes )
{
    ...
    struct sort_veh_part_vector {
        ...
        inline bool operator()( const int p1, const int p2 ) const {
            return veh->part_info( p1 ).location->list_order <
                   veh->part_info( p2 ).location->list_order;
        }
    } svpv = { this };
...
int vehicle::damage( map &here, int p, int dmg, damage_type type, bool aimed )
{
    ...
    if( armor_part < 0 ) {
        ...
        bool overhead = part_flag( target_part, "ROOF" ) ||
                        part_info( target_part ).location == vpart_location_on_roof;
                                      ...
                                      int &quantity, const std::function<bool( const item & )> &filter, bool in_tools )
{
    ...
    if( tool_vp ) { // handle vehicle tools
        const itype_id tool_fuel = type->tool_slot_first_ammo();
        const itype_id fuel_type = tool_fuel.is_null() ? type : tool_fuel;
...
void vehicle::calc_mass_center( bool use_precalc ) const
{
    ...
    for( const vpart_reference &vp : get_all_parts() ) {
        ...
        if( vp.has_feature( "FURNITURE_TIEDOWN" ) &&
            ...
            const furn_str_id carried_furniture(
                vp.part().get_base().get_var( "tied_down_furniture" ) );
            if( carried_furniture.is_valid() ) {
