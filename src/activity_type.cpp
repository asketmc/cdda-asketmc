...
void activity_type::check_consistency()
{
    for( const auto &pair : activity_type_all ) {
        ...
        const auto check_eoc = [&pair]( const effect_on_condition_id & eoc,
                                       const char *member ) {
            if( eoc.is_null() ) {
                return;
            }
            if( !eoc.is_valid() ) {
                debugmsg( "%s has invalid %s %s", pair.second.id_.c_str(), member, eoc.c_str() );
            } else if( eoc->type != eoc_type::ACTIVATION ) {
                debugmsg( "%s uses non-ACTIVATION EOC %s for %s", pair.second.id_.c_str(),
                          eoc.c_str(), member );
            }
        };
        check_eoc( pair.second.completion_EOC, "completion_eoc" );
        check_eoc( pair.second.do_turn_EOC, "do_turn_eoc" );
    }
