...
static void draw_can_craft_indicator( const catacurses::window &w, const recipe &rec,
                                      Character &crafter, bool camp_crafting );
 ...
 * return < 0 if error happens or nobody is chosen.
 */
...
{
struct availability {
        explicit availability( Character &_crafter, const inventory &_inventory,
                               const recipe *r, int batch_size = 1 ) :
            crafter( _crafter ), crafting_inv( _inventory ) {
            rec = r;
            ...
            if( r->is_nested() ) {
                can_craft = check_can_craft_nested( crafter, crafting_inv, *r );
            } else {
                can_craft = ( !r->is_practice() || has_all_skills ) && has_proficiencies &&
                            req.can_make_with_inventory( crafting_inv, all_items_filter, batch_size,
            ...
            would_use_rotten = !req.can_make_with_inventory( crafting_inv, no_rotten_filter,
                               batch_size, craft_flags::start_only );
            would_use_favorite = !req.can_make_with_inventory( crafting_inv, no_favorite_filter,
                                 batch_size,
                                 craft_flags::start_only );
            ...
            apparently_craftable =
                ...
                simple_req.can_make_with_inventory( crafting_inv, all_items_filter,
                        batch_size, craft_flags::start_only );
        ...
        const inventory &get_crafting_inventory() const {
            return crafting_inv;
        }
    private:
    ...
    public:
        ...
        static bool check_can_craft_nested( Character &crafter, const inventory &crafting_inv,
                                            const recipe &r ) {
            ...
            for( const recipe_id &nested_r : r.nested_category_data ) {
                // if nested recur
                availability av( crafter, crafting_inv, &nested_r.obj() );
    ...
    const std::vector<Character *> &crafting_group )
{
    ...
    const double failure_percent = 100.0 * guy.crafting_failure_chance( recp );
    const char *failure_color = failure_percent > 75.0 ? "red" :
                                failure_percent > 50.0 ? "light_red" :
                                failure_percent > 25.0 ? "yellow" : "light_green";
    oss << string_format( _( "Minor failure chance: <color_%s>%.2f%%</color>\n" ),
                          failure_color, failure_percent );
    ...
    const inventory &crafting_inv = avail.get_crafting_inventory();
    if( recp.result() ) {
    ...
    if( time_maluses != 1.0 && fail_maluses != 1.0 ) {
        oss << string_format( _( "<color_yellow>This recipe will take %.1fx as long as normal, "
                                 "and be %.1fx more likely to incur failures, because the crafter "
                                 "lacks some of the proficiencies used.</color>\n" ), time_maluses, fail_maluses );
    } else if( time_maluses != 1.0 ) {
        oss << string_format( _( "<color_yellow>This recipe will take %.1fx as long as normal, "
                                 "because the crafter lacks some of the proficiencies used.</color>\n" ), time_maluses );
...
class recipe_result_info_cache
{
    ...
    public:
        ...
        void set_crafter( Character &_crafter ) {
            if( crafter != &_crafter ) {
                crafter = &_crafter;
                last_recipe = nullptr;
            }
...
static void recursively_expance_recipes( std::vector<const recipe *> &current,
        ...
        Character &crafter, const inventory &crafting_inv,
        ...
        if( available_recipes.contains( &nested.obj() ) &&
            ...
            if( !availability_cache.count( &nested.obj() ) ) {
                availability_cache.emplace( &nested.obj(),
                                            availability( crafter, crafting_inv,
                                                    &nested.obj() ) );
            }
        }
    }
    ...
    for( size_t i = 0; i < current.size(); ++i ) {
        if( current[i]->is_nested() &&
            ...
            recursively_expance_recipes( current, indent, availability_cache, i, crafter,
                                         crafting_inv, unread_recipes_first,
...
static std::string list_nested( Character &crafter, const inventory &crafting_inv,
                                const recipe *rec,
                                const recipe_subset &available_recipes,
                                int indent = 0 )
{
    std::string description;
    availability avail( crafter, crafting_inv, rec );
    if( rec->is_nested() ) {
        description += colorize( std::string( indent,
                                              ' ' ) + rec->result_name() + ":\n", avail.color() );
        for( const recipe_id &r : rec->nested_category_data ) {
            description += list_nested( crafter, crafting_inv, &r.obj(), available_recipes,
                                        indent + 2 );
        }
...
std::pair<Character *, const recipe *> select_crafter_and_crafting_recipe( int &batch_size_out,
        ...
        bool camp_crafting, const inventory *inventory_override,
        const recipe_subset *recipes_override )
{
    if( crafter == nullptr ) {
    ...
    const std::vector<Character *> crafting_group = camp_crafting ?
            std::vector<Character *>( { crafter } ) : crafter->get_crafting_group();
    int crafter_i = static_cast<int>( std::find( crafting_group.begin(), crafting_group.end(),
                                      crafter ) - crafting_group.begin() );

    const auto crafting_inventory_for = [inventory_override]( Character & who ) -> const inventory & {
        return inventory_override != nullptr ? *inventory_override : who.crafting_inventory();
    };

    // Get everyone's recipes
    recipe_subset assemble_available_recipes;
    if( recipes_override != nullptr ) {
        assemble_available_recipes = *recipes_override;
    } else {
        for( Character *guy : crafting_group ) {
            const inventory &group_inventory = crafting_inventory_for( *guy );
            assemble_available_recipes.include( guy->get_available_recipes( group_inventory,
                                                &crafting_group ) );
        }
    }
    ...
    ui.on_redraw( [&]( ui_adaptor & ui ) {
        ...
        if( !current.empty() ) {
            ...
            draw_can_craft_indicator( w_head_info, recp, *crafter, camp_crafting );
        ...
        if( isWide && !current.empty() ) {
            ...
            if( cur_recipe->is_practice() ) {
                ...
                desc += list_nested( *crafter, crafting_inventory_for( *crafter ), cur_recipe,
                                     available_recipes );
                fold_and_print( w_iteminfo, point_zero, item_info_width, c_light_gray, desc );
                scrollbar().offset_x( item_info_width - 1 ).offset_y( 0 ).content_size( 1 ).viewport_size( getmaxy(
                            w_iteminfo ) ).apply( w_iteminfo );
    ...
    do {
        if( recalc ) {
            ...
            if( batch ) {
                ...
                for( int i = 1; i <= 50; i++ ) {
                    ...
                    available.emplace_back( *crafter, crafting_inventory_for( *crafter ), chosen, i );
                }
                indent.assign( current.size(), 0 );
            } else {
                static_popup popup;
                ...
                for( const recipe *e : current ) {
                    if( !availability_cache->count( e ) ) {
                        availability_cache->emplace( e,
                                                     availability( *crafter,
                                                             crafting_inventory_for( *crafter ), e ) );
                    }
                }
                ...
                indent.assign( current.size(), 0 );
                expand_recipes( current, indent, *availability_cache, *crafter,
                                crafting_inventory_for( *crafter ),
        ...
        if( action == "SELECT" ) {
            ...
            recalc = true;
        } else if( action == "CHOOSE_CRAFTER" && !camp_crafting ) {
            ...
            if( new_crafter_i >= 0 && new_crafter_i != crafter_i ) {
                ...
                // next line also inserts empty cache for crafter->getID() if nonexistent
...
const recipe *select_crafting_recipe( int &batch_size_out, const recipe_id &goto_recipe,
                                      Character *crafter, bool camp_crafting,
                                      const inventory *inventory_override,
                                      const recipe_subset *recipes_override )
{
    return select_crafter_and_crafting_recipe( batch_size_out, goto_recipe, crafter, "",
            camp_crafting, inventory_override, recipes_override ).second;
}
                    ...
                    const recipe *rec, bool rec_valid )
{
    ...
    for( Character *chara : crafting_group ) {
        ...
        if( rec_valid ) {
            const inventory &crafting_inv = chara->crafting_inventory();
            availability avail( *chara, crafting_inv, rec );
            std::vector<std::string> reasons;

            bool has_stuff = rec->deduped_requirements().can_make_with_inventory(
                                 crafting_inv, rec->get_component_filter( recipe_filter_flags::none ), 1,
                                 craft_flags::start_only );
            if( !has_stuff ) {
            ...
            entry.emplace_back( colorize( join( reasons, ", " ), c_red ) );
        }
...
{
    if( camp_crafting ) {
        right_print( w, 0, 1, i_green, _( "Camp crafting" ) );
    } else if( crafter.lighting_craft_speed_multiplier( rec ) <= 0.0f ) {
