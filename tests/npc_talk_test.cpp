...
TEST_CASE( "popping the final categorized dialogue topic is safe", "[npc][dialogue]" )
{
    dialogue d;

    SECTION( "a single categorized topic empties the stack" ) {
        d.topic_stack.emplace_back( "TALK_MISSION_START" );
        d.pop_topic();
        CHECK( d.topic_stack.empty() );
    }

    SECTION( "adjacent topics in the same category are popped together" ) {
        d.topic_stack.emplace_back( "TALK_FRIEND" );
        d.topic_stack.emplace_back( "TALK_MISSION_START" );
        d.topic_stack.emplace_back( "TALK_MISSION_OFFER" );
        d.pop_topic();
        REQUIRE( d.topic_stack.size() == 1 );
        CHECK( d.topic_stack.back().id == "TALK_FRIEND" );
    }
}
