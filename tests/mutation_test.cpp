...
static const trait_id trait_TEST_SAFE_TRANSFORM_CONFLICT( "TEST_SAFE_TRANSFORM_CONFLICT" );
static const trait_id trait_TEST_SAFE_TRANSFORM_GEAR_PREREQ( "TEST_SAFE_TRANSFORM_GEAR_PREREQ" );
static const trait_id trait_TEST_SAFE_TRANSFORM_GEAR_SOURCE( "TEST_SAFE_TRANSFORM_GEAR_SOURCE" );
static const trait_id trait_TEST_SAFE_TRANSFORM_GEAR_TARGET( "TEST_SAFE_TRANSFORM_GEAR_TARGET" );
static const trait_id trait_TEST_SAFE_TRANSFORM_BLOCKED_SOURCE( "TEST_SAFE_TRANSFORM_BLOCKED_SOURCE" );
static const trait_id trait_TEST_SAFE_TRANSFORM_BLOCKED_TARGET( "TEST_SAFE_TRANSFORM_BLOCKED_TARGET" );
static const trait_id trait_TEST_SAFE_TRANSFORM_PREREQ( "TEST_SAFE_TRANSFORM_PREREQ" );
static const trait_id trait_TEST_SAFE_TRANSFORM_PREREQ_SOURCE( "TEST_SAFE_TRANSFORM_PREREQ_SOURCE" );
static const trait_id trait_TEST_SAFE_TRANSFORM_PREREQ_TARGET( "TEST_SAFE_TRANSFORM_PREREQ_TARGET" );
static const trait_id trait_TEST_SAFE_TRANSFORM_SOURCE( "TEST_SAFE_TRANSFORM_SOURCE" );
static const trait_id trait_TEST_SAFE_TRANSFORM_TARGET( "TEST_SAFE_TRANSFORM_TARGET" );
...
static const trait_id trait_TEST_UNSAFE_TRANSFORM_SOURCE( "TEST_UNSAFE_TRANSFORM_SOURCE" );
...
TEST_CASE( "safe mutation transforms follow normal conflict handling", "[mutations][transform]" )
{
    npc dummy;
    dummy.set_mutation( trait_TEST_SAFE_TRANSFORM_CONFLICT );

    SECTION( "safe transform removes a conflict before adding its target" ) {
        dummy.set_mutation( trait_TEST_SAFE_TRANSFORM_SOURCE );
        dummy.activate_mutation( trait_TEST_SAFE_TRANSFORM_SOURCE );

        CHECK_FALSE( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_SOURCE ) );
        CHECK_FALSE( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_CONFLICT ) );
        CHECK( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_TARGET ) );
    }

    SECTION( "legacy transform remains a direct replacement" ) {
        dummy.set_mutation( trait_TEST_UNSAFE_TRANSFORM_SOURCE );
        dummy.activate_mutation( trait_TEST_UNSAFE_TRANSFORM_SOURCE );

        CHECK_FALSE( dummy.has_trait( trait_TEST_UNSAFE_TRANSFORM_SOURCE ) );
        CHECK( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_CONFLICT ) );
        CHECK( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_TARGET ) );
    }
}

TEST_CASE( "failed safe mutation transforms restore their source", "[mutations][transform]" )
{
    npc dummy;
    dummy.set_moves( 1000 );

    SECTION( "rejected target leaves the source and moves unchanged" ) {
        dummy.set_mutation( trait_TEST_SAFE_TRANSFORM_BLOCKED_SOURCE );
        dummy.activate_mutation( trait_TEST_SAFE_TRANSFORM_BLOCKED_SOURCE );

        CHECK( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_BLOCKED_SOURCE ) );
        CHECK_FALSE( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_BLOCKED_TARGET ) );
        CHECK( dummy.get_moves() == 1000 );
    }

    SECTION( "prerequisite-only progress is rolled back" ) {
        dummy.set_mutation( trait_TEST_SAFE_TRANSFORM_PREREQ_SOURCE );
        dummy.activate_mutation( trait_TEST_SAFE_TRANSFORM_PREREQ_SOURCE );

        CHECK( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_PREREQ_SOURCE ) );
        CHECK_FALSE( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_PREREQ ) );
        CHECK_FALSE( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_PREREQ_TARGET ) );
        CHECK( dummy.get_moves() == 1000 );
    }

    SECTION( "destructive prerequisite is rejected before it can destroy gear" ) {
        const itype_id helmet( "helmet_ball" );
        REQUIRE( dummy.wear_item( item( helmet ), false, false ) );
        REQUIRE( dummy.amount_worn( helmet ) == 1 );
        dummy.set_mutation( trait_TEST_SAFE_TRANSFORM_GEAR_SOURCE );

        dummy.activate_mutation( trait_TEST_SAFE_TRANSFORM_GEAR_SOURCE );

        CHECK( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_GEAR_SOURCE ) );
        CHECK_FALSE( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_GEAR_PREREQ ) );
        CHECK_FALSE( dummy.has_trait( trait_TEST_SAFE_TRANSFORM_GEAR_TARGET ) );
        CHECK( dummy.amount_worn( helmet ) == 1 );
        CHECK( dummy.get_moves() == 1000 );
    }
}
