...
TEST_CASE( "vehicle_auto_drink_zone_uses_faucet_tank",
           "[vehicle][vehicle_parts][zones][auto_drink]" )
{
    clear_avatar();
    clear_map();
    clear_vehicles();
    ...
    REQUIRE( found_tank );
    REQUIRE( veh.fuel_left( itype_water_clean, true ) == water_charges );

    cata::optional<vpart_reference> faucet;
    cata::optional<vpart_reference> cargo_without_faucet;
    for( const vpart_reference &part : veh.get_all_parts() ) {
        if( !faucet ) {
            faucet = part.part_with_tool( itype_water_faucet );
        }
        if( part.has_feature( "CARGO" ) && !part.part_with_tool( itype_water_faucet ) ) {
            cargo_without_faucet = part;
        }
    }
    REQUIRE( faucet.has_value() );
    REQUIRE( cargo_without_faucet.has_value() );

    const bool zone_on_faucet = GENERATE( true, false );
    const tripoint zone_pos = zone_on_faucet ? faucet->pos() : cargo_without_faucet->pos();
    character.setpos( zone_pos );
    character.set_thirst( 300 );

    const tripoint_abs_ms zone_abs = here.getglobal( zone_pos );
    zone_manager::get_manager().add( "Auto drink", zone_type_AUTO_DRINK,
                                     faction_your_followers, false, true, zone_abs.raw(), zone_abs.raw(),
                                     nullptr, false, true );

    const int moves = get_auto_consume_moves( character, false );
    if( zone_on_faucet ) {
        CHECK( moves != 0 );
        CHECK( veh.fuel_left( itype_water_clean, true ) == water_charges - 1 );
    } else {
        CHECK( moves == 0 );
        CHECK( veh.fuel_left( itype_water_clean, true ) == water_charges );
    }
}
