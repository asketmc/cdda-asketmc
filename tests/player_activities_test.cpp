...
static const itype_id itype_test_ebook_scan_one_page( "test_ebook_scan_one_page" );
static const itype_id itype_test_ebook_scan_thirty_pages( "test_ebook_scan_thirty_pages" );
static const itype_id itype_test_ebook_scan_two_pages( "test_ebook_scan_two_pages" );
...
struct ebook_scan_test_items {
    item_location ereader;
    item_location one_page;
    item_location two_pages;
    item_location thirty_pages;
};

static ebook_scan_test_items setup_ebook_scan_test( avatar &dummy )
{
    clear_avatar();
    dummy.worn.wear_item( dummy, item( itype_test_backpack ), false, false );
    ...
    return { ereader, dummy.i_add( item( itype_test_ebook_scan_one_page ) ),
             dummy.i_add( item( itype_test_ebook_scan_two_pages ) ),
             dummy.i_add( item( itype_test_ebook_scan_thirty_pages ) ) };
}

TEST_CASE( "multi-book e-reader scanning preserves speed-adjusted progress",
           "[activity][ebooksave][backport]" )
{
    avatar &dummy = get_avatar();
    const ebook_scan_test_items items = setup_ebook_scan_test( dummy );
    dummy.activity = player_activity( ebooksave_activity_actor(
                                          { items.one_page, items.two_pages }, items.ereader ) );
    dummy.activity.start_or_resume( dummy, false );
    REQUIRE( dummy.activity.moves_total == to_moves<int>( 15_seconds ) );

    SECTION( "fast character crosses one book boundary without losing remainder" ) {
        dummy.moves = 1250;
        dummy.activity.do_turn( dummy );
        CHECK( items.ereader->ebooks().size() == 1 );
        CHECK( dummy.activity.moves_left == 250 );

        dummy.moves = 100;
        dummy.activity.do_turn( dummy );
        CHECK( items.ereader->ebooks().size() == 1 );
        CHECK( dummy.activity.moves_left == 150 );

        dummy.moves = 150;
        dummy.activity.do_turn( dummy );
        CHECK( items.ereader->ebooks().size() == 2 );
        CHECK( dummy.activity.id() == ACT_NULL );
    }

    SECTION( "slow character completes a book only at its move boundary" ) {
        for( int turn = 0; turn < 19; ++turn ) {
            dummy.moves = 50;
            dummy.activity.do_turn( dummy );
        }
        CHECK( items.ereader->ebooks().empty() );
        CHECK( dummy.activity.moves_left == 550 );

        dummy.moves = 50;
        dummy.activity.do_turn( dummy );
        CHECK( items.ereader->ebooks().size() == 1 );
        CHECK( dummy.activity.moves_left == 500 );
    }
}

TEST_CASE( "legacy single-book e-reader activity keeps persisted progress",
           "[activity][ebooksave][save][backport]" )
{
    avatar &dummy = get_avatar();
    const ebook_scan_test_items items = setup_ebook_scan_test( dummy );
    ...
    JsonOut json( saved );
    json.start_object();
    json.member( "type", ACT_EBOOKSAVE );
    json.member( "actor" );
    json.start_object();
    json.member( "actor_type", ACT_EBOOKSAVE );
    json.member( "actor_data" );
    json.start_object();
    json.member( "book", items.two_pages );
    json.member( "ereader", items.ereader );
    json.end_object();
    json.end_object();
    json.member( "moves_total", to_moves<int>( 10_seconds ) );
    json.member( "moves_left", 300 );
    json.end_object();

    JsonObject saved_activity = json_loader::from_string( saved.str() ).get_object();
    dummy.activity.deserialize( saved_activity );
    dummy.activity.start_or_resume( dummy, true );
    REQUIRE( dummy.activity.id() == ACT_EBOOKSAVE );
    REQUIRE( dummy.activity.moves_left == 300 );

    dummy.moves = 100;
    dummy.activity.do_turn( dummy );
    CHECK( items.ereader->ebooks().empty() );
    CHECK( dummy.activity.moves_left == 200 );
}

TEST_CASE( "e-reader scan charge cost follows persisted page progress",
           "[activity][ebooksave][charges][save][backport]" )
{
    restore_on_out_of_scope<time_point> restore_calendar_turn( calendar::turn );
    avatar &dummy = get_avatar();
    const ebook_scan_test_items items = setup_ebook_scan_test( dummy );
    const int initial_charges = items.ereader->ammo_remaining();

    REQUIRE( ebooksave_activity_actor::required_charges( { items.thirty_pages },
             items.ereader ) == 1 );
    dummy.activity = player_activity( ebooksave_activity_actor(
                                          { items.thirty_pages }, items.ereader ) );
    dummy.activity.start_or_resume( dummy, false );

    SECTION( "fast scanning consumes the displayed charge at a page boundary" ) {
        calendar::turn = calendar::turn_zero + 37_seconds;
        dummy.moves = to_moves<int>( 150_seconds );
        dummy.activity.do_turn( dummy );

        CHECK( dummy.activity.id() == ACT_NULL );
        CHECK( items.ereader->ammo_remaining() == initial_charges - 1 );
    }

    SECTION( "slow scanning is independent of wall-clock alignment" ) {
        calendar::turn = calendar::turn_zero + 73_seconds;
        for( int turn = 0; turn < 150 && dummy.activity.id() != ACT_NULL; ++turn ) {
            dummy.moves = 100;
            dummy.activity.do_turn( dummy );
    ...
    SECTION( "saving before a boundary resumes without losing or duplicating progress" ) {
        calendar::turn = calendar::turn_zero + 19_seconds;
        dummy.moves = to_moves<int>( 120_seconds );
        dummy.activity.do_turn( dummy );
        REQUIRE( dummy.activity.moves_left == to_moves<int>( 30_seconds ) );
        REQUIRE( items.ereader->ammo_remaining() == initial_charges );

        std::ostringstream saved;
        JsonOut json( saved );
        dummy.activity.serialize( json );
        JsonObject saved_activity = json_loader::from_string( saved.str() ).get_object();
        dummy.activity.deserialize( saved_activity );
        dummy.activity.start_or_resume( dummy, true );

        dummy.moves = to_moves<int>( 5_seconds );
        dummy.activity.do_turn( dummy );
        CHECK( items.ereader->ammo_remaining() == initial_charges - 1 );
        CHECK( dummy.activity.moves_left == to_moves<int>( 25_seconds ) );

        dummy.moves = to_moves<int>( 25_seconds );
        dummy.activity.do_turn( dummy );
        CHECK( dummy.activity.id() == ACT_NULL );
        CHECK( items.ereader->ammo_remaining() == initial_charges - 1 );
    }
...
TEST_CASE( "legacy e-reader saves initialize missing page progress safely",
           "[activity][ebooksave][charges][save][backport]" )
{
    avatar &dummy = get_avatar();
    const ebook_scan_test_items items = setup_ebook_scan_test( dummy );
    ...
    json.member( "actor_data" );
    json.start_object();
    json.member( "books", std::vector<item_location> { items.thirty_pages } );
    json.member( "ereader", items.ereader );
    json.end_object();
    json.end_object();
    json.member( "moves_total", to_moves<int>( 150_seconds ) );
    json.member( "moves_left", to_moves<int>( 20_seconds ) );
    json.end_object();
    ...
    CHECK( dummy.activity.moves_left == to_moves<int>( 15_seconds ) );
    CHECK( items.ereader->ammo_remaining() == initial_charges );
}
