...
static const vpart_id vpart_test_vehicle_enchantment_part( "test_vehicle_enchantment_part" );

static const vproto_id vehicle_prototype_bicycle( "bicycle" );
static const vproto_id vehicle_prototype_car( "car" );
static const vproto_id vehicle_prototype_test_vehicle_enchantment_state(
    "test_vehicle_enchantment_state" );
...
TEST_CASE( "vehicle_part_enchantments_modify_vehicle_values", "[vehicle][enchantment]" )
{
    clear_map();
    map &here = get_map();
    vehicle *veh = here.add_vehicle( vehicle_prototype_car, tripoint( 60, 60, 0 ), 0_degrees,
                                     100, 0 );
    REQUIRE( veh != nullptr );

    veh->engine_on = true;
    veh->toggle_specific_engine( 0, true );
    const units::mass initial_mass = veh->total_mass();
    const units::energy initial_fuel_use = veh->engine_fuel_usage( 0 );
    const float initial_handling = veh->handling_difficulty();
    REQUIRE( initial_fuel_use > 0_J );

    const int installed = veh->install_part( point_zero, vpart_test_vehicle_enchantment_part,
                                             "", true );
    REQUIRE( installed >= 0 );

    CHECK( veh->total_mass().value() == Approx( initial_mass.value() * 0.9 ).margin( 1 ) );
    CHECK( veh->engine_fuel_usage( 0 ).value() ==
           Approx( initial_fuel_use.value() * 0.5 ).margin( 1 ) );
    CHECK( veh->handling_difficulty() == Approx( initial_handling + 2.0f ) );
}

TEST_CASE( "spawned_vehicle_part_enchantments_use_the_final_enabled_state",
           "[vehicle][enchantment]" )
{
    const vehicle &blueprint = *vehicle_prototype_test_vehicle_enchantment_state->blueprint;
    REQUIRE( blueprint.reactors.size() == 1 );
    REQUIRE_FALSE( blueprint.part( blueprint.reactors.front() ).enabled );
    CHECK( blueprint.handling_difficulty() == Approx( 7.0f ) );

    clear_map();
    vehicle *spawned = get_map().add_vehicle( vehicle_prototype_test_vehicle_enchantment_state,
                                             tripoint( 60, 60, 0 ), 0_degrees, 100, 0 );
    REQUIRE( spawned != nullptr );
    REQUIRE( spawned->reactors.size() == 1 );
    REQUIRE( spawned->part( spawned->reactors.front() ).enabled );
    CHECK( spawned->handling_difficulty() == Approx( 3.0f ) );
}

TEST_CASE( "broken_vehicle_part_enchantments_do_not_modify_vehicle_values",
           "[vehicle][enchantment]" )
{
    clear_map();
    map &here = get_map();
    ...
    const int installed = veh->install_part( point_zero, vpart_test_vehicle_enchantment_part,
                                             "", true );
    REQUIRE( installed >= 0 );
    vehicle_part &enchanted_part = veh->part( installed );
    REQUIRE( veh->mod_hp( enchanted_part, -( enchanted_part.hp() + 1 ), damage_type::BASH ) );
    REQUIRE( enchanted_part.is_broken() );

    CHECK( veh->total_mass() == initial_mass );
    CHECK( veh->engine_fuel_usage( 0 ) == initial_fuel_use );
    CHECK( veh->handling_difficulty() == initial_handling );
}

TEST_CASE( "carried_vehicle_part_enchantments_only_modify_the_carried_vehicle",
           "[vehicle][enchantment][bike_rack]" )
{
    clear_map();
    map &here = get_map();
    const tripoint carrier_pos( 60, 60, 0 );
    const tripoint carried_pos( 56, 60, 0 );
    vehicle *carrier = here.add_vehicle( vehicle_prototype_car, carrier_pos, 0_degrees, 100, 0 );
    vehicle *carried = here.add_vehicle( vehicle_prototype_bicycle, carried_pos, 90_degrees,
                                        100, 0 );
    REQUIRE( carrier != nullptr );
    REQUIRE( carried != nullptr );

    std::vector<int> rack_parts;
    for( int y = -1; y <= 2; ++y ) {
        const int installed = carrier->install_part( point( -3, y ), vpart_bike_rack );
        REQUIRE( installed >= 0 );
        rack_parts.push_back( installed );
    }

    const int enchanted = carried->install_part( point_zero, vpart_test_vehicle_enchantment_part,
                                             "", true );
    REQUIRE( enchanted >= 0 );
    const units::mass enchanted_carried_mass = carried->total_mass();

    carrier->engine_on = true;
    carrier->toggle_specific_engine( 0, true );
    const units::energy initial_fuel_use = carrier->engine_fuel_usage( 0 );
    REQUIRE( initial_fuel_use > 0_J );

    const std::vector<vehicle::rackable_vehicle> rackables =
        carrier->find_vehicles_to_rack( rack_parts.front() );
    ...
    carrier->recalculate_enchantment_cache();
    CHECK( carrier->engine_fuel_usage( 0 ) == initial_fuel_use );

    const std::vector<vehicle::unrackable_vehicle> unrackables =
        carrier->find_vehicles_to_unrack( rack_parts.front() );
    REQUIRE( unrackables.size() == 1 );
    const vehicle::unrackable_vehicle &unrackable = unrackables.front();
    REQUIRE( carrier->remove_carried_vehicle( unrackable.parts, unrackable.racks ) );

    const optional_vpart_position unracked_position = here.veh_at( carried_pos );
    REQUIRE( unracked_position.has_value() );
    vehicle &unracked = unracked_position->vehicle();
    REQUIRE( &unracked != carrier );
    CHECK( unracked.total_mass() == enchanted_carried_mass );
}

TEST_CASE( "vehicle_effects_decay", "[vehicle][effect]" )
{
    clear_map();
    vehicle *veh = get_map().add_vehicle( vehicle_prototype_car, tripoint( 60, 60, 0 ),
                                          0_degrees, 100, 0 );
    REQUIRE( veh != nullptr );
    REQUIRE_FALSE( veh->has_effect( effect_grabbed ) );

    veh->add_effect( effect_source::empty(), effect_grabbed, 1_turns );
    REQUIRE( veh->has_effect( effect_grabbed ) );
    veh->process_effects();
    REQUIRE( veh->has_effect( effect_grabbed ) );
    veh->process_effects();
    CHECK_FALSE( veh->has_effect( effect_grabbed ) );
}
