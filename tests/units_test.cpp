...
TEST_CASE( "angles_to_tileset_directions", "[units][tiles][vehicle]" )
{
    CHECK( angle_to_dir4( 0_degrees ) == 0 );
    CHECK( angle_to_dir4( 90_degrees ) == 1 );
    CHECK( angle_to_dir4( 180_degrees ) == 2 );
    CHECK( angle_to_dir4( 270_degrees ) == 3 );
    CHECK( angle_to_dir4( 360_degrees ) == 0 );

    // Current tileset releases are normalized to the 0.G sprite contract at
    // import time, so vehicle rendering keeps the original 0.G handedness.
    CHECK( angle_to_dir4( 270_degrees - 0_degrees ) == 3 );
    CHECK( angle_to_dir4( 270_degrees - 90_degrees ) == 2 );
    CHECK( angle_to_dir4( 270_degrees - 180_degrees ) == 1 );
    CHECK( angle_to_dir4( 270_degrees - 270_degrees ) == 0 );
}
