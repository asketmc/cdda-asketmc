                            ...
                            bool fov_3d )
{
    ...
    if( fov_3d ) {
        ...
        get_map().seen_cache_process_ledges( seen_squares, floor_cache,
                                             ORIGIN - tripoint( 0, 0, OVERMAP_DEPTH ) );
    } else {
        castLightAll<float, float, sight_calc, sight_check, update_light, accumulate_transparency>(
...
TEST_CASE( "ledge coverage increases for a lower viewing profile",
           "[shadowcasting][fov_3d][pr67434][pr72293]" )
{
    map &here = get_map();
    const tripoint viewer( 60, 60, 1 );
    const tripoint target( 64, 60, 0 );
    level_cache &upper_cache = here.access_cache( viewer.z );

    std::array<bool, 5> previous_floor;
    for( int x = viewer.x; x <= target.x; ++x ) {
        previous_floor[x - viewer.x] = upper_cache.floor_cache[x][viewer.y];
        upper_cache.floor_cache[x][viewer.y] = false;
    }
    upper_cache.floor_cache[62][viewer.y] = true;

    CHECK( here.ledge_coverage( viewer, viewer, 1.0f ) == 0 );
    const int standing_coverage = here.ledge_coverage( viewer, target, 1.0f );
    const int crouched_coverage = here.ledge_coverage( viewer, target, 0.5f );
    CHECK( standing_coverage > 100 );
    CHECK( crouched_coverage > standing_coverage );

    for( int x = viewer.x; x <= target.x; ++x ) {
        upper_cache.floor_cache[x][viewer.y] = previous_floor[x - viewer.x];
    }
}

TEST_CASE( "lower-floor drawing preserves ramps and rejects off-map points",
           "[shadowcasting][fov_3d][pr67434][pr72293]" )
{
    map &here = get_map();
    const tripoint ramp( 60, 60, 0 );
    const ter_id previous_terrain = here.ter( ramp );

    here.ter_set( ramp, ter_id( "t_ramp_down_high" ) );
    here.invalidate_map_cache( ramp.z );
    here.build_floor_cache( ramp.z );

    CHECK_FALSE( here.dont_draw_lower_floor( ramp ) );
    CHECK( here.dont_draw_lower_floor( tripoint( -1, ramp.y, ramp.z ) ) );
    CHECK( here.dont_draw_lower_floor( tripoint( MAPSIZE_X, ramp.y, ramp.z ) ) );

    here.ter_set( ramp, previous_terrain );
    here.invalidate_map_cache( ramp.z );
    here.build_floor_cache( ramp.z );
}

TEST_CASE( "remote z-level dirtiness only rebuilds 3D seen cache",
           "[shadowcasting][fov_3d][pr68660][pr82240]" )
{
    map &here = get_map();
    avatar &player = get_avatar();
    const int active_z = player.posz();
    const int remote_z = active_z < OVERMAP_HEIGHT ? active_z + 1 : active_z - 1;
    restore_on_out_of_scope<bool> restore_fov_3d( fov_3d );
    restore_on_out_of_scope<int> restore_fov_3d_z_range( fov_3d_z_range );
    const move_mode_id previous_move_mode = player.current_movement_mode();
    on_out_of_scope restore_move_mode( [&player, previous_move_mode]() {
        player.set_movement_mode( previous_move_mode );
    } );
    player.set_movement_mode( move_mode_id( "walk" ) );

    // Stabilize all caches and the position/range state held by build_map_cache.
    fov_3d = false;
    here.build_map_cache( active_z, true );
    here.access_cache( active_z ).seen_cache_dirty = false;
    here.access_cache( remote_z ).seen_cache_dirty = true;

    here.build_map_cache( active_z, true );
    CHECK( here.access_cache( remote_z ).seen_cache_dirty );

    fov_3d = true;
    fov_3d_z_range = 1;
    here.build_map_cache( active_z, true );
    CHECK_FALSE( here.access_cache( remote_z ).seen_cache_dirty );
}
