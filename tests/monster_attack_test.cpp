...
TEST_CASE( "monster_attack_dialogue_conditions", "[mattack][eoc]" )
{
    clear_map();
    clear_creatures();
    ...
    Character &you = get_player_character();
    clear_avatar();
    you.setpos( attacker_location + tripoint_east );

    monster &test_monster = spawn_test_monster( "mon_test_mattack_dialogue", attacker_location );
    test_monster.set_dest( you.get_location() );
    const mtype_special_attack &attack =
        test_monster.type->special_attacks.at( "test_dialogue_condition" );

    CHECK_FALSE( attack->call( test_monster ) );

    // A true dialogue condition does not bypass the legacy effect requirements.
    test_monster.add_effect( effect_took_flumed, 1_days );
    REQUIRE( test_monster.has_effect( effect_took_flumed ) );
    CHECK_FALSE( attack->call( test_monster ) );
    test_monster.add_effect( effect_grabbing, 1_days );
    REQUIRE( test_monster.has_effect( effect_grabbing ) );
    ...
    test_monster.add_effect( effect_invisibility, 1_days );
    REQUIRE( test_monster.has_flag( json_flag_INVISIBLE ) );
