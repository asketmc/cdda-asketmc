...
TEST_CASE( "legacy_faction_food_supply_conversion_is_bounded", "[factions][savegame]" )
{
    const auto load_legacy_food_supply = []( int64_t legacy_kcal ) {
        const std::string data = "{\"id\":\"legacy_test\",\"food_supply\":" +
                                 std::to_string( legacy_kcal ) + "}";
        JsonValue value = json_loader::from_string( data );
        faction loaded;
        loaded.deserialize( value.get_object() );
        return loaded.food_supply.calories;
    };

    constexpr int max_calories = std::numeric_limits<int>::max();
    constexpr int64_t max_exact_legacy_kcal = max_calories / 1000;

    CHECK( load_legacy_food_supply( max_exact_legacy_kcal ) ==
           max_exact_legacy_kcal * 1000 );
    CHECK( load_legacy_food_supply( max_exact_legacy_kcal + 1 ) == max_calories );
    CHECK( load_legacy_food_supply( std::numeric_limits<int64_t>::max() ) == max_calories );
    CHECK( load_legacy_food_supply( -1 ) == 0 );
}
