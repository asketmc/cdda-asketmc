...
TEST_CASE( "thermoplastic riot armor can be repaired", "[iuse_actor][repair][armor]" )
{
    const item repair_tool( "soldering_iron" );
    const use_function *use = repair_tool.type->get_use( "repair_metal" );
    REQUIRE( use != nullptr );

    const repair_item_actor *actor = dynamic_cast<const repair_item_actor *>( use->get_actor_ptr() );
    REQUIRE( actor != nullptr );

    const std::vector<itype_id> riot_armor_ids = {
        itype_id( "armor_riot" ),
        itype_id( "armor_riot_torso" ),
        itype_id( "armor_riot_arm" ),
        itype_id( "armor_riot_leg" ),
        itype_id( "helmet_riot" ),
        itype_id( "helmet_riot_raised" )
    };

    for( const itype_id &armor_id : riot_armor_ids ) {
        CAPTURE( armor_id.str() );
        const std::set<itype_id> repair_materials =
            actor->get_valid_repair_materials( item( armor_id ) );
        CHECK( repair_materials.count( itype_id( "thermo_resin_chunk" ) ) == 1 );
    }
}
