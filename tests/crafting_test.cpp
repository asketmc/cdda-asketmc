...
static const recipe_id recipe_test_baseball( "test_baseball" );
...
TEST_CASE( "crafting_failure_chance_uses_0G_dice_formula", "[crafting][probability]" )
{
    clear_avatar();
    clear_map();
    Character &crafter = get_player_character();
    const recipe &rec = recipe_test_baseball.obj();

    crafter.set_skill_level( skill_fabrication, 0 );
    crafter.set_knowledge_level( skill_fabrication, 0 );

    // test_baseball is difficulty 1 with no secondary skills or proficiencies.
    // At fabrication 0 and intelligence 8 the unchanged 0.G roll is 4d24 / 8d24.
    CHECK( crafter.crafting_failure_chance( rec ) ==
           Approx( 0.9810579574507468 ).epsilon( 0.000000000001 ) );

    crafter.toggle_trait( trait_DEBUG_CNF );
    CHECK( crafter.crafting_failure_chance( rec ) == 0.0 );
    CHECK( crafter.crafting_success_roll( rec ) == 1.0 );
}

TEST_CASE( "crafting_failure_display_counts_fractional_skill_progress",
           "[crafting][probability][h6]" )
{
    clear_avatar();
    clear_map();
    Character &crafter = get_player_character();
    const recipe &rec = recipe_armguard_metal.obj();

    crafter.set_skill_level( skill_fabrication, 4 );
    crafter.set_skill_level( skill_tailor, 4 );
    const double level_four_failure = crafter.crafting_failure_chance( rec );

    SkillLevel &fabrication = crafter.get_skill_level_object( skill_fabrication );
    fabrication.train( 50 * 100 * 5 * 5, 1.0f, 0.0f );
    REQUIRE( fabrication.level() == 4 );
    REQUIRE( fabrication.exercise() == 50 );
    const double halfway_failure = crafter.crafting_failure_chance( rec );

    crafter.set_skill_level( skill_fabrication, 5 );
    const double level_five_failure = crafter.crafting_failure_chance( rec );

    CHECK( level_four_failure > halfway_failure );
    CHECK( halfway_failure > level_five_failure );
}
...
TEST_CASE( "allied_characters_share_known_crafting_recipes" )
{
    const recipe *r = &recipe_brew_mead.obj();
    standard_npc crafter( "crafter", tripoint_zero, {}, 0 );
    standard_npc recipe_knower( "recipe knower", tripoint_zero, {}, 0 );
    inventory crafting_inv;
    std::vector<Character *> crafting_group = { &crafter, &recipe_knower };

    REQUIRE( r->skill_used );
    REQUIRE_FALSE( crafter.knows_recipe( r ) );

    SECTION( "a recipe memorized by an ally" ) {
        recipe_knower.learn_recipe( r );

        CHECK_FALSE( crafter.get_available_recipes( crafting_inv, &crafting_group ).contains( r ) );

        crafter.set_skill_level( r->skill_used, r->difficulty );
        CHECK( crafter.get_available_recipes( crafting_inv, &crafting_group ).contains( r ) );
    }

    SECTION( "a recipe book carried by an ally" ) {
        item &cookbook = recipe_knower.inv->add_item( item( "brewing_cookbook" ) );
        REQUIRE( cookbook.is_book() );
        crafter.identify( cookbook );
...
TEST_CASE( "npc_does_not_resume_a_craft_claimed_by_the_avatar" )
{
    clear_avatar();
    clear_map();

    avatar &you = get_avatar();
    standard_npc crafter( "crafter" );
    const tripoint test_origin( 60, 60, 0 );
    you.setpos( test_origin );
    crafter.setpos( test_origin );

    const recipe *r = &recipe_cudgel_simple.obj();
    item in_progress_craft( r, 1, {}, {} );
    in_progress_craft.set_var( "crafter", crafter.name );

    item_location claimed_craft;
    SECTION( "in the NPC inventory" ) {
        claimed_craft = crafter.i_add( in_progress_craft );
    }
    SECTION( "on an adjacent map tile" ) {
        map &here = get_map();
        const tripoint craft_pos = test_origin + tripoint_east;
        item &craft_on_map = here.add_item_or_charges( craft_pos, in_progress_craft );
        claimed_craft = item_location( map_cursor( craft_pos ), &craft_on_map );
    }

    REQUIRE( claimed_craft );
    you.assign_activity( player_activity( ACT_CRAFT ) );
    you.activity.targets.emplace_back( claimed_craft );

    CHECK_FALSE( crafter.get_item_to_craft() );
...
TEST_CASE( "liquid container sorting ignores a loaded battery pocket",
           "[crafting][container][regression]" )
{
    item multi_cooker( itype_multi_cooker );
    item battery_cell( itype_medium_battery_cell );
    REQUIRE( multi_cooker.put_in( battery_cell,
                                  item_pocket::pocket_type::MAGAZINE_WELL ).success() );
    REQUIRE_FALSE( multi_cooker.empty() );
    REQUIRE( multi_cooker.num_item_stacks() == 0 );

    item half_full_bottle( "bottle_plastic" );
    REQUIRE( half_full_bottle.put_in( item( itype_water_clean, calendar::turn_zero, 1 ),
                                      item_pocket::pocket_type::CONTAINER ).success() );
    REQUIRE( half_full_bottle.num_item_stacks() == 1 );

    std::vector<const item *> containers = { &multi_cooker, &half_full_bottle };
    const std::string debug_message = capture_debugmsg_during( [&containers]() {
        std::sort( containers.begin(), containers.end(), item_ptr_compare_by_charges );
    } );

    CHECK( debug_message.empty() );
    CHECK( containers.front() == &half_full_bottle );
    CHECK( containers.back() == &multi_cooker );
}
