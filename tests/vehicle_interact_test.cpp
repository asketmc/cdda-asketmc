...
struct veh_interact_test_helper {
    static bool selected_tile_is_empty( vehicle &veh, const point &cursor ) {
        veh_interact interaction( veh, cursor );
        return interaction.cpart < 0;
    }

    static bool update_install_requirements( vehicle &veh, const point &cursor,
            const vpart_id &part ) {
        veh_interact interaction( veh, cursor );
        interaction.w_msg = catacurses::newwin( 10, 80, point_zero );
        interaction.sel_vpart_info = &part.obj();
        return interaction.update_part_requirements();
    }
};
...
TEST_CASE( "vehicle_install_requirements_on_empty_adjacent_tile", "[vehicle]" )
{
    clear_avatar();
    clear_map();
    ...
    Character &player_character = get_player_character();
    player_character.setpos( test_origin );
    player_character.set_mutation( trait_id( "DEBUG_HS" ) );

    vehicle *veh_ptr = get_map().add_vehicle( vehicle_prototype_bicycle,
                       test_origin + tripoint_south_east, -90_degrees, 0, 0 );
    REQUIRE( veh_ptr != nullptr );

    // A bicycle occupies mount x=-1..1, so x=2 is the empty adjacent extension tile.
    const point cursor( -2, 0 );
    REQUIRE( veh_interact_test_helper::selected_tile_is_empty( *veh_ptr, cursor ) );
    REQUIRE( veh_ptr->can_mount( point( 2, 0 ), vpart_frame ).success() );
    CHECK( veh_interact_test_helper::update_install_requirements( *veh_ptr, cursor,
            vpart_frame ) );
    player_character.unset_mutation( trait_id( "DEBUG_HS" ) );
}
