...
TEST_CASE( "explicit multi-pocket consumption is isolated by pocket id",
           "[ammo][multimag][vehicle]" )
{
    item gun( "test_multimag_turret_gun" );
    REQUIRE( gun.uses_firing_requirements() );

    item magazine( "glockmag" );
    magazine.ammo_set( itype_9mm, 10 );
    REQUIRE( gun.pocket_by_id( "ammo" )->insert_item( magazine ).success() );

    item battery_magazine( "heavy_battery_cell" );
    battery_magazine.ammo_set( itype_battery, 40 );
    REQUIRE( gun.pocket_by_id( "power" )->insert_item( battery_magazine ).success() );

    const gun_mode_id default_mode( "DEFAULT" );
    CHECK( gun.feasible_shots( default_mode, 20 ) == 8 );
    CHECK( gun.consume_shots( default_mode, 2, tripoint_zero, nullptr ) == 2 );
    CHECK( gun.ammo_remaining_in_pocket( "ammo" ) == 8 );
    CHECK( gun.ammo_remaining_in_pocket( "power" ) == 30 );
}
