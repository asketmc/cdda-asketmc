...
{
    const item described( itype_test_snippet_description, calendar::turn_zero );

    REQUIRE( described.has_var( "description" ) );
    const std::string expanded = described.get_var( "description" );
    CHECK( expanded.find( "<number>" ) == std::string::npos );
    CHECK( expanded.find( "Serial " ) == 0 );

    const item copy = described;
    CHECK( copy.get_var( "description" ) == expanded );
}
